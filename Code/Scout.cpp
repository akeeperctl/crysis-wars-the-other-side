...
#include "TheOtherSide/Actor Files/Aliens/ScoutBombHandler.h"
//~TheOtherSide
...
CScoutBombHandler* CScout::CreateBombHandler()
{
	return nullptr;
}
...
{
	const float fGlowPower = energy / 2 + 5;

	const int nBodySlot = 0;
	const int nHeadSlot = 1;
	const int nScoutTentaclesSlot = 2;
...
bool CScout::InitSearchBeam()
{
	m_searchbeam.itemId = 0;
	m_searchbeam.pAttachment = nullptr;
	...
	//const auto itemId = GetInventory()->GetItemByClass(pBeamClass);

	//if (!itemId || !GET_ENTITY(m_searchbeam.itemId))
		//m_searchbeam.itemId = g_pGame->GetIGameFramework()->GetIItemSystem()->GiveItem(this, "ScoutSearchBeam", false, false, false);
...
void CScout::UpdateGrab(float frameTime)
{
	...
	const bool showBeam(m_pGrabHandler && m_pGrabHandler->GetStats()->grabId > 0);
...
void CScout::ProcessMovementNew(float frameTime)
{
	...
	{	// normal action
		...
		//TheOtherSide
		const bool isGyro = g_pControlSystem->GetAbilitiesSystem()->scout.isGyroEnabled;
		const bool isSearch = g_pControlSystem->GetAbilitiesSystem()->scout.IsSearch;
		...
		const float desiredSpeed = m_desiredVelocity.GetLength();
		const float fCurSpeed = m_stats.speed;

		const float rate1 = min(maxSpeed, fabsf(desiredSpeed - fCurSpeed));
		const float rateVelocity = (rate1 > 0.0f) ? min(frameTime * 2.5f, 5.0f / (rate1 * rate1)) : frameTime * 2.5f;
		const float rateRotation = (rate1 > 0.0f) ? min(frameTime * 2.5f, 2.5f / (rate1 * rate1)) : frameTime * 2.5f;
		...
		if (desiredSpeed > 0.0f && fCurSpeed > 0.0f)
		{
			...
			const Vec3 vNew = m_desiredVelocity.GetNormalized();
	...
	{	// for the emergency situation...set speed immidiately
		...
		//TheOtherSide
		m_desiredVelocity = m_input.movementVector;
		m_desiredVelocity += m_viewMtx.GetColumn(0) * m_input.deltaMovement.x * 30.0f;
		m_desiredVelocity += m_viewMtx.GetColumn(1) * m_input.deltaMovement.y * 30.0f;
		m_desiredVelocity += m_viewMtx.GetColumn(2) * m_input.deltaMovement.z * 30.0f;
		//~TheOtherSide
		...
		const Quat modelRot(GetEntity()->GetRotation());
	...
	if (showDebugInfo && (name == "movement_debug" || IsLocalOwner()))
	{
		float color[] = { 1,1,1,1 };
		...
		gEnv->pRenderer->Draw2dLabel(20, 80, 1.3f, color, false, "m_desiredVelocity (%f, %f, %f)", m_desiredVelocity.x, m_desiredVelocity.y, m_desiredVelocity.z);
		gEnv->pRenderer->Draw2dLabel(20, 100, 1.3f, color, false, "m_input.movementVector (%f, %f, %f)", m_input.movementVector.x, m_input.movementVector.y, m_input.movementVector.z);
	}
}
...
bool CScout::NetSerialize(TSerialize ser, EEntityAspects aspect, uint8 profile, int flags)
{
	...
	float color[] = { 1,1,1,1 };
	gEnv->pRenderer->Draw2dLabel(20, 20, 1.15f, color, false, "[Scout][NetSerialize Mode 2]");
...
void CAlienHealBomb::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	if (!m_bPlanted)
	{
		...
		const Vec3 vPos = GetEntity()->GetWorldPos();
		const Vec3 vDir = GetEntity()->GetWorldTM().GetColumn2() * -5000.f;
		...
		const float fDistToGround = rayHit.dist;
	...
	else
	{
		...
		const int	numEnts = pWorld->GetEntitiesInBox(min, max, ppList, ent_living);
		for (int i = 0; i < numEnts; ++i)
		{
			//CryLogAlways("Entities in box");
			const EntityId id = pWorld->GetPhysicalEntityId(ppList[i]);
			if (CActor* pActor = dynamic_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(id)))
			{
				//CryLogAlways("Actor");
...
bool CAlienHealBomb::Reset()
{
	...
	IEntityRenderProxy* pRenderProxy = dynamic_cast<IEntityRenderProxy*>(GetEntity()->GetProxy(ENTITY_PROXY_RENDER));
	if (pRenderProxy)
	{
