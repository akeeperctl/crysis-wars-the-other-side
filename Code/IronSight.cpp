...
bool CIronSight::StartZoom(bool stayZoomed, bool fullZoomout, int zoomStep)
{
	...
	if (pActor && pActor->GetScreenEffects() != 0)
	{
		...
		if (pPlayer->IsOnlyThirdPerson())
		{
			if (pPlayer == g_pGame->GetIGameFramework()->GetClientActor())
			{
				if (pPlayer->IsThirdPerson())
				{
					pPlayer->ToggleThirdPerson();

					uint32 flags = pPlayer->GetEntity()->GetCharacter(0)->GetFlags();
					flags &= ~ENTITY_SLOT_RENDER;
					pPlayer->GetEntity()->GetCharacter(0)->SetFlags(flags);
...
void CIronSight::OnLeaveZoom()
{
	...
	if (pPlayer && pPlayer->IsOnlyThirdPerson())
	{
		if (pPlayer == g_pGame->GetIGameFramework()->GetClientActor())
		{
			if (!pPlayer->IsThirdPerson())
