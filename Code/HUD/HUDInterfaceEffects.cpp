...
void CHUD::IndicateDamage(EntityId weaponId, Vec3 direction, bool onVehicle)
{
	...
	if (!g_pControlSystem->GetControlClient()->GetControlledActor())
		pPlayerActor = static_cast<CActor*>(g_pGame->GetIGameFramework()->GetClientActor());
	else
		pPlayerActor = g_pControlSystem->GetControlClient()->GetControlledActor();
...
void CHUD::UpdateCrosshairVisibility()
{
	...
	//TheOtherSide
	if (g_pGameCVars->goc_enable || (g_pControlSystem->GetControlClient() && g_pControlSystem->GetControlClient()->GetControlledActor()))
	{
