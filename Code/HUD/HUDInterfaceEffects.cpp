...
void CHUD::UpdateMissionObjectiveIcon(EntityId objective, int friendly, FlashOnScreenIcon iconType, bool forceNoOffset, Vec3 rotationTarget, bool characterhead, bool forceshow)
{
	...
	if (nearCenter && (m_pHUDScopes->IsBinocularsShown()))
	{
...
void CHUD::UpdateAllMissionObjectives()
{
	...
	if (g_pGame->GetHUD()->GetCurrentGameRules() != EHUD_INSTANTACTION && m_pHUDScopes->IsBinocularsShown())
	{
		if (m_missionObjectiveValues.size() && gEnv->bMultiplayer)
		{
			m_animMissionObjective.SetVisible(true);
...
void CHUD::TrackProjectiles(IActor* pPlayerActor)
{
...
//void CHUD::IndicateControlDamage(EntityId weaponId, Vec3 direction, bool onVehicle)
//{
//	Vec3 vlookingDirection = FORWARD_DIRECTION;
//	CGameFlashAnimation* pAnim = NULL;
//	//TheOtherSide
//	CActor* pPlayerActor = g_pControlSystem->GetControlClient()->GetControlledActor();
//
//	if (!pPlayerActor)
//		return;
//
//	//CryLogAlways("PlayerHit %s", pPlayerActor->GetEntity()->GetName());
//
...
void CHUD::IndicateDamage(EntityId weaponId, Vec3 direction, bool onVehicle)
{
	...
	CGameFlashAnimation* pAnim = NULL;
	//TheOtherSide
	CActor* pPlayerActor = 0;
	if (!g_pControlSystem->GetLocalControlClient()->GetControlledActor())
		pPlayerActor = static_cast<CActor*>(g_pGame->GetIGameFramework()->GetClientActor());
	else
		pPlayerActor = g_pControlSystem->GetLocalControlClient()->GetControlledActor();
...
{
	//CryLogAlways("Indicate Hit %1.f %s", (float)enemyIndicator, pEntity ? (string)pEntity->GetName() : (string)"Null entity");
	//TheOtherSide
	//TheOtherSide
	/*CActor* pPlayer;
	if (!g_pControlSystem->GetEnabled())
		pPlayer = static_cast<CActor*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
	else
		pPlayer = g_pControlSystem->GetControlClient()->GetControlledActor();*/
...
void CHUD::Targetting(EntityId pTargetEntity, bool bStatic)
{
	...
	if (!gEnv->bMultiplayer && GetScopes()->IsBinocularsShown())
	{
...
void CHUD::UpdateCrosshairVisibility()
{
	...
	//TheOtherSide
	if (g_pGameCVars->goc_enable || (g_pControlSystem->GetLocalControlClient() && g_pControlSystem->GetLocalControlClient()->GetControlledActor()))
	{
