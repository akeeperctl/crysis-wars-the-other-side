...
int CHUDPowerStruggle::GetPlayerPP()
{
	...
	const auto pActor = g_pGame->GetIGameFramework()->GetClientActor();
	if (!pActor)
		return 0;
	...
	if (isConquest)
	{
		pp = pConqueror->GetActorPoints(pActor->GetEntity());
		return pp;
	}
	//~TheOtherSide
	...
	if (pScriptTable)
	{
		...
		//TheOtherSide CrashFix GetClientActor() is null
		if (pActor)
			pGameRules->GetSynchedEntityValue(pActor->GetEntityId(), TSynchedKey(key), pp);
		//~TheOtherSide
	}
	return pp;
}
...
void CHUDPowerStruggle::DetermineCurrentBuyZone(bool sendToFlash, bool force /* = false*/)
{
	...
	if (isConquest)
	{
		auto pArea = pConqueror->GetClientArea();
		if (!pArea)
			return;

		if (pArea->IsBuyZoneActived(pConqueror->GetClientSpecies()))
		{
			SBuyZoneInfo info;
			pArea->GetBuyZoneInfo(info);
			
			if (info.flags & eBZO_Ammo)
			{
				if (factory == E_LOADOUT)
					factory = E_AMMO;
				availableBuyZones.push_back(E_AMMO);
			}
			if (info.flags & eBZO_Equipment)
			{
				if (factory == E_LOADOUT)
					factory = E_EQUIPMENT;
				availableBuyZones.push_back(E_EQUIPMENT);
			}
			if (info.flags & eBZO_Prototypes)
			{
				if (factory == E_LOADOUT)
					factory = E_PROTOTYPES;
				availableBuyZones.push_back(E_PROTOTYPES);
			}
			if (info.flags & eBZO_Vehicles)
			{
				if (factory == E_LOADOUT)
					factory = E_VEHICLES;
				availableBuyZones.push_back(E_VEHICLES);
			}
			if (info.flags & eBZO_Weapons)
			{
				if (factory == E_LOADOUT)
					factory = E_WEAPONS;
