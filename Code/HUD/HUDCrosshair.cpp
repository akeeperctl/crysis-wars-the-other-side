...
void CHUDCrosshair::Update(float fDeltaTime)
{
	...
	if (!g_pControlSystem->GetEnabled())
		...
		pClientActor = g_pControlSystem->GetControlClient()->GetControlledActor();
	...
	if (pWeapon)
	{
		...
		if (pPlayer && !g_pControlSystem->GetEnabled())
		{
...
bool CHUDCrosshair::IsFriendlyEntity(IEntity* pEntity)
{
	//TheOtherSide
	IActor* pClientActor;
	if (g_pControlSystem->GetEnabled() && g_pControlSystem->GetControlClient()->GetControlledActor())
		pClientActor = g_pControlSystem->GetControlClient()->GetControlledActor();
	else
