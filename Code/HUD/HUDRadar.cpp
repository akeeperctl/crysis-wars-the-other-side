...
CHUDRadar::CHUDRadar()
{
	...
	//TheOtherSide
	//m_flashMouseCoords = Vec2(0, 0);
	//m_animFlashMouseLayer = &g_pGame->GetHUD()->m_animMapMouseLayer;

	//XmlNodeRef xmlVehicles = GetISystem()->LoadXmlFile("../Mods/TheOtherSide/vehicles.xml");// .. - root of game

	//if (xmlVehicles != NULL)
	//{
	//	CryLogAlways("Load Successful");
	//	CryLogAlways("Tag: %s",  (string)xmlVehicles->getTag()); // in <Mod name="" team="" version="" screenshot="" url="" description=""/> return "Mod"
	//	//CryLogAlways("ChildCount: %s" ,(string)xmlVehicles->getContent());
	//	/*for (int tag = 0; tag < xmlVehicles->getChildCount(); ++tag)
	//	{
	//		XmlNodeRef mission = xmlVehicles->getChild(tag);
	//		const char* attrib;
	...
	//	for (int j = 0; j < xmlVehicles->getNumAttributes(); ++j)
	//	{
	//		const char* vehicleNum, * vehicleClsName;
	//		if (xmlVehicles->getAttributeByIndex(j, &vehicleNum, &vehicleClsName))
	//		{
	//			int vehicleIndex = j;

	//			if (IEntityClass *pAddonVehicleCls = pEntityClassRegistry->FindClass(vehicleClsName))
	//				m_AddonVehiclesMap[vehicleIndex] = pAddonVehicleCls;

	//

	//			//assert(m_AddonVehiclesMap[vehicleIndex]);

	//			//m_statusMessagesMap[string(attrib)] = string(value);
	//		//CryLogAlways(string(vehicleNum)+" "+ string(vehicleName));
	//		}
	//
	//	}
	//}
	...
	//m_pPlayerTrooper = pEntityClassRegistry->FindClass("PlayerTrooper");
...
void CHUDRadar::Update(float fDeltaTime)
{
	...
	{
		//TheOtherSide
		if (gEnv->bMultiplayer || g_pControlSystem->GetConquerorSystem()->IsGamemode())
		{
			//render the mini map
			if (m_renderBigMap)
				RenderBigMap();
	...
	pClientActor = g_pControlSystem->GetClientActor();

	//CompassStealth***************************************************************
		UpdateCompassStealth(pClientActor, fDeltaTime);
	//~CompassStealth**************************************************************

	//binocs***********************************************************************
		UpdateBinoculars(pClientActor, fDeltaTime);
	//~binocs**********************************************************************

	//jammer***********************************************************************
		UpdateRadarJammer(pClientActor);
	//~jammer**********************************************************************
	...
	//*********************************MAIN ENTITY UPDATE*************************
	auto pClientPlayer = static_cast<CPlayer*> (g_pGame->GetIGameFramework()->GetClientActor());
	auto playerViewMtxInverted = pClientPlayer->GetViewMatrix().GetInverted();

	UpdateMinimapEntities(pClientActor, fRadius, playerViewMtxInverted, numOfValues, &entityValues);
	//****************************************************************************
	...
	for (; it != end; ++it)
	{
		...
		if (pEntity && !pEntity->IsHidden())
		{
			...
			numOfValues += ::FillUpDoubleArray(&entityValues, 
				pEntity->GetId(), 
				5 /* MO */, 
				(fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY, 
				180 + RAD2DEG(fAngle), 
				faction, 
				75.0f, 
				fAlpha * 100.0f);
	...
	if (isMultiplayer && g_pGameCVars->hud_showRadarObjectiveIcons)
	{
		...
		float fY = 0.0;
		float sizeScale = 1.0;
		int playerTeam = pGameRules->GetTeam(pClientActor->GetEntityId());
			...
			if (id)
			{
				...
				if (pEntity)
				{
					...
					numOfValues += ::FillUpDoubleArray(&entityValues, 
						...
						(fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY,
						0.0f, 
						FriendOrFoe(gEnv->bMultiplayer, playerTeam, pEntity, g_pGame->GetGameRules()), 
						sizeScale * 100.0f, 
						100.0f);
				}
			}
	...
	for (int t = 0; t < m_tempEntitiesOnRadar.size(); ++t)
	{
		...
		if (pEntity && !pEntity->IsHidden())
		{
			...
			int playerTeam = g_pGame->GetGameRules()->GetTeam(pClientActor->GetEntityId());
			...
			numOfValues += ::FillUpDoubleArray(&entityValues, 
				...
				FriendOrFoe(gEnv->bMultiplayer, playerTeam, pEntity,
					g_pGame->GetGameRules()), 
				sizeScale * 25.0f, 
				fAlpha * 100.0f);
		}
	}
	...
	//TheOtherSide
	//draw capturable areas
	auto pConqueror = g_pControlSystem->GetConquerorSystem();
	for (int i = 0; i < pConqueror->m_strategicAreas.size(); ++i)
	{
		auto pArea = pConqueror->m_strategicAreas[i];
		if (pArea && pArea->IsEnabled())
		{
			const auto isFriendly = pConqueror->GetClientSpecies() == pArea->GetSpecies();

			EntityId id = pArea->GetEntityId();
			if (id)
			{
				IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id);
				if (pEntity)
				{
					...
					FlashRadarType type = ESpawnPoint;
					if (!pArea->IsHaveFlag(EAreaFlag::SoldierSpawner))
						type = EFactoryPrototype;

					if (pArea->IsHaveFlag(EAreaFlag::Base) && !isFriendly)
						continue;
	...
	//draw story entities on minimap
	for (int i = 0; i < m_storyEntitiesOnRadar.size(); ++i)
	{
		EntityId id = m_storyEntitiesOnRadar[i].m_id;
		if (id)
		{
			IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id);
			if (pEntity)
			{
				//0 - draw as movable unit (with scale and rotation)
				//1 - draw as building unit (with static scale and player-related rotation)
				//2 - draw as waypoint (mission objective)
				int drawMethod = 1;		
				...
				FlashRadarType type = m_storyEntitiesOnRadar[i].m_type;
				FlashRadarFaction faction = ENeutral;
	...
	//draw extended story entities on minimap
	for (int i = 0; i < m_extendedStoryEntitiesOnRadar.size(); ++i)
	{
		EntityId id = m_extendedStoryEntitiesOnRadar[i].m_id;
		if (id)
		{
			IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id);
			if (pEntity)
			{
				...
				FlashRadarType type = m_extendedStoryEntitiesOnRadar[i].m_type;
				FlashRadarFaction faction = m_extendedStoryEntitiesOnRadar[i].m_faction;
	...
	//render the bigmap (big map on M)
	if (m_renderBigMap)
		RenderBigMap();
	...
	if (pClientActor)
	{
		GetPosOnMap(pClientActor->GetEntity(), playerX, playerY, true);

		SFlashVarValue args[3] = { playerX, playerY, 180.0f - GetRadarRotation(pClientActor) };
...
void CHUDRadar::UpdateMinimapEntities(CActor* pClientActor, float& fRadius, Matrix34& playerViewMtxInverted, int& numOfValues, ArrayFillHelper<double, FVAT_Double, NUM_ARRAY_FILL_HELPER_SIZE>* entityValues)
{
	...
	//TheOtherSide
	bool isConquest = g_pControlSystem->GetConquerorSystem()->IsGamemode();
	if (isConquest)
	{
		auto species = g_pControlSystem->GetConquerorSystem()->GetClientSpecies();

		m_conquerorTeamMates.clear();
		g_pControlSystem->GetConquerorSystem()->GetSpeciesTeammates(species, m_conquerorTeamMates);
	}
	...
	for (int i = 0; i < amount; ++i)
	{
		...
		if (pEntity)
		{
			...
			if (!isOnRadar && stl::find(m_teamMates, id))
				isOnRadar = friendlyBool = true;

			//TheOtherSide conqueror team mates 
			if (!isOnRadar && stl::find(m_conquerorTeamMates, id))
				isOnRadar = friendlyBool = true;
			//~TheOtherSide
			...
			{
				auto pTempAI = pEntity->GetAI();
				//if (pTemp && AIOBJECT_VEHICLE != pTemp->GetAIType() && pTemp->IsHostile(pClientActor->GetEntity()->GetAI(), false))
				if (pTempAI && AIOBJECT_VEHICLE != pTempAI->GetAIType() && TOS_AI::IsHostile(pTempAI, pClientActor->GetEntity()->GetAI(),false))
				{
					isOnRadar = true;
			...
			//TheOtherSide
			auto pActor = static_cast<CActor*> (tempActor);
			auto pNanoActor = static_cast<CPlayer*> (tempActor);
			auto pAITemp = pEntity->GetAI();
		//	bool isHostile = pAITemp && pAITemp->IsHostile(pClientActor->GetEntity()->GetAI(), false);
			bool isHostile = pAITemp && TOS_AI::IsHostile(pAITemp, pClientActor->GetEntity()->GetAI(), false);
			
			if (isHostile)
			{
				if (pNanoActor && pNanoActor->GetActorClass() == CPlayer::GetActorClassType())
				{
					auto pNanoSuit = pNanoActor->GetNanoSuit();
					if (pNanoSuit)
					{
				...
				else if (pActor && pActor->IsAlien())
				{
					auto pAlien = static_cast<CAlien*> (tempActor);

					if (pAlien->IsUseCloakAbility())
						isOnRadar = false;
				}
			...
			int friendly = friendlyBool ? EFriend : ENeutral;
			...
			if (pAIObject)
			{
				...
				{
					if (checkDriver || friendly)	//probably the own player
						friendly = EFriend;
					else
						friendly = ENeutral;
				}
			...
			else if (gEnv->bMultiplayer)	//treats factions in multiplayer
			{
				if (tempActor)
				{
					...
					if (!friendly && !unknownEnemyActor) //probably MP scanned enemy
					{
						friendly = EEnemy;
						scannedEnemy = true;
					}
...
void CHUDRadar::UpdateCompassStealth(CActor* pActor, float fDeltaTime)
{
	...
	auto pPlayerActor = static_cast<CPlayer*>(pActor);
	...
	if (m_fLastStealthValue != fStealthValue)
	{
		...
		if (!gEnv->bMultiplayer)
			g_pGame->GetHUD()->SetStealthExposure(fStealthValue);
	}
...
#define COMPASS_EPSILON (0.01f)
	...
	if (gEnv->bMultiplayer || g_pControlSystem->GetConquerorSystem()->IsGamemode())	//shows the player coordinates in MP
	{
		float fX, fY;
		fX = fY = -1;
...
void CHUDRadar::Reset()
{
	...
	//TheOtherSide
	m_extendedStoryEntitiesOnRadar.clear();
	m_conquerorTeamMates.clear();
	//~TheOtherSide
}
...
void CHUDRadar::RenderBigMap()
{
	...
	EntityId iCurrentSpawnPoint = pGameRules->GetPlayerSpawnGroup(pClientActor);
	...
	int team = pGameRules->GetTeam(pClientActor->GetEntityId());

	//TheOtherSide
	//draw capturable area's on big map
	auto pConqueror = g_pControlSystem->GetConquerorSystem();
	for (int i = 0; i < pConqueror->m_strategicAreas.size(); ++i)
	{
		auto pArea = pConqueror->m_strategicAreas[i];
		if (pArea && pArea->IsEnabled())
		{
			...
			if (id)
			{
				...
				if (GetPosOnMap(pEntity, fX, fY))
				{
					...
					{
						if (pArea->GetSpecies() == ESpeciesType::eST_NEUTRAL)
							faction = ENeutral;
						else
							faction = EEnemy;
					}
					...
					//if (addBuilding == true)
					{
						...
						{
							underAttack = 
								(pArea->GetCaptureState() == ECaptureState::UNCAPTURING ||
								pArea->GetCaptureState() == ECaptureState::CONTESTED) &&
								faction == EFriend;
						}
						...
						numOfValues += FillUpDoubleArray(&entityValues, 
							...
							100, 
							100, 
							iOnScreenObjective == pEntity->GetId(),
							iCurrentSpawnPoint == pEntity->GetId(),
							underAttack);
					}
				}
			}
				...
				if (pEntity->GetScriptTable() && pEntity->GetScriptTable()->GetValue("Properties", props))
				{
					...
					if (props->GetValue("bCapturable", addBuilding))
					{
						if (!addBuilding)
						{
							...
							{
								if (pGameRules->GetTeam(pClientActor->GetEntityId()) == pGameRules->GetTeam(pEntity->GetId()))
								{
									addBuilding = true;
								}
							}
				...
				if (friendly == EFriend || friendly == ENeutral)
				{
					if (type == EAmmoTruck && stl::find_in_map(drawnVehicles, mEntity.entityId, false))
					{
						numOfValues += FillUpDoubleArray(
							...
							pEntity->GetId(), 
							EBarracks, 
							fX, fY, 270.0f - RAD2DEG(pEntity->GetWorldAngles().z), 
							friendly, 
							100, 100, 
						...
						numOfValues += FillUpDoubleArray(
							...
							pEntity->GetId(), 
							type, 
							fX, fY, 270.0f - RAD2DEG(pEntity->GetWorldAngles().z), friendly, 
							100, 100, 
							iOnScreenObjective == mEntity.entityId, 
							iCurrentSpawnPoint == mEntity.entityId);
				}
	...
	//draw tagged guys and vehicles...
	{
		for (int i = 0; i < NUM_TAGGED_ENTITIES; ++i)
		{
			auto id = m_taggedEntities[i];
			if (id)
			{
				if (pTempActor = m_pActorSystem->GetActor(id))
				{
					if (auto pVehicle = pTempActor->GetLinkedVehicle())
					{
						if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
						{
							...
							numOfValues += FillUpDoubleArray(
								&entityValues, 
								pVehicle->GetEntity()->GetId(), 
								ETaggedEntity, fX, fY, 
								270.0f - RAD2DEG(pVehicle->GetEntity()->GetWorldAngles().z), ENeutral, 
								100, 100, 
								iOnScreenObjective == pVehicle->GetEntity()->GetId(), 
								iCurrentSpawnPoint == pVehicle->GetEntity()->GetId());
					...
					else
					{
						...
						numOfValues += FillUpDoubleArray(
							&entityValues, 
							pClientActor->GetEntity()->GetId(), 
							ETaggedEntity, 
							fX, fY, 
							270.0f - RAD2DEG(pTempActor->GetEntity()->GetWorldAngles().z), 
							ENeutral, 
							100, 100, 
							iOnScreenObjective == pClientActor->GetEntity()->GetId(), 
							iCurrentSpawnPoint == pClientActor->GetEntity()->GetId());
					}
				}
				else if (auto pVehicle = m_pVehicleSystem->GetVehicle(id))
				{
					if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
					{
						GetPosOnMap(pVehicle->GetEntity(), fX, fY);
	...
	//draw temporarily tagged units
	{
		for (int e = 0; e < m_tempEntitiesOnRadar.size(); ++e)
		{
			auto id = m_tempEntitiesOnRadar[e].m_id;
			if (pTempActor = m_pActorSystem->GetActor(id))
			{
				if (auto pVehicle = pTempActor->GetLinkedVehicle())
				{
					if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
					{
						...
						numOfValues += FillUpDoubleArray(
							&entityValues, 
							pVehicle->GetEntity()->GetId(), 
							ChooseType(pVehicle->GetEntity(), false), 
							fX, fY,
							270.0f - RAD2DEG(pVehicle->GetEntity()->GetWorldAngles().z), 
							FriendOrFoe(isMultiplayer, team, pVehicle->GetEntity(), pGameRules), 
							100, 100, 
							iOnScreenObjective == pVehicle->GetEntity()->GetId(), 
							iCurrentSpawnPoint == pVehicle->GetEntity()->GetId());
				...
				else
				{
					...
					numOfValues += FillUpDoubleArray(
						&entityValues, 
						pClientActor->GetEntity()->GetId(), 
						ChooseType(pTempActor->GetEntity(), false), 
						fX, fY,
						270.0f - RAD2DEG(pTempActor->GetEntity()->GetWorldAngles().z), 
						FriendOrFoe(isMultiplayer, team, pTempActor->GetEntity(), pGameRules), 
						100, 100, 
						iOnScreenObjective == pTempActor->GetEntity()->GetId(), 
						iCurrentSpawnPoint == pTempActor->GetEntity()->GetId());
				}
			}
			else if (auto pVehicle = m_pVehicleSystem->GetVehicle(id))
			{
	...
	//draw position of teammates ...
	{
					...
					if (auto pVehicle = pTempActor->GetLinkedVehicle())
					{
						if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
						{
							...
							numOfValues += FillUpDoubleArray(
								&entityValues,
								pVehicle->GetEntity()->GetId(),
								ChooseType(pVehicle->GetEntity()),
								fX, fY, 270.0f - RAD2DEG(pVehicle->GetEntity()->GetWorldAngles().z),
								EFriend,
								100, 100,
								iOnScreenObjective == pVehicle->GetEntity()->GetId(),
								iCurrentSpawnPoint == pVehicle->GetEntity()->GetId());
		...
		else if (pConqueror->IsGamemode())
		{
			auto it = m_conquerorTeamMates.begin();
			auto end = m_conquerorTeamMates.end();
			for (; it != end; ++it)
			{
				pTempActor = m_pActorSystem->GetActor(*it);
				if (pTempActor && pTempActor != pClientActor && (FriendOrFoe(false, 0, pTempActor->GetEntity(), pGameRules) == EFriend))
				{
					if (auto pVehicle = pTempActor->GetLinkedVehicle())
					{
						if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
	...
	//draw radar entities on the big map (scanned enemies and vehicles)
	...
	for (int i = 0; i < m_entitiesOnRadar.size(); ++i)
	{
		...
		{
			//TheOtherSide		
			//auto pClientAI = pClientActor->GetEntity()->GetAI();
			//auto pTempAI = pTempActor->GetEntity()->GetAI();
			//if (pClientAI && pTempAI)
			//{
			//	if (pTempAI->IsHostile(pClientAI, false))
			//	{
			//		RemoveFromRadar(uiEntityId);
			//		continue;
			//	}
			...
			auto pVehicle = pTempActor->GetLinkedVehicle();
			if (pVehicle && !pVehicle->IsDestroyed())
			{
				if (!stl::find_in_map(drawnVehicles, pVehicle->GetEntityId(), false))
				{
					...
					numOfValues += FillUpDoubleArray(
						&entityValues, 
						pVehicle->GetEntity()->GetId(), 
						ChooseType(pVehicle->GetEntity()), fX, fY, 
						270.0f - RAD2DEG(pVehicle->GetEntity()->GetWorldAngles().z), 
						friendly,
						100, 100, 
						iOnScreenObjective == pVehicle->GetEntity()->GetId(), 
	...
	//TheOtherSide
	//draw extended story entities (icons with text) on big map
	{
		for (int e = 0; e < m_extendedStoryEntitiesOnRadar.size(); ++e)
		{
			EntityId id = m_extendedStoryEntitiesOnRadar[e].m_id;
			if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id))
			{
				GetPosOnMap(pEntity, fX, fY);
				numOfValues += FillUpDoubleArray(&entityValues, 
					id, 
					m_extendedStoryEntitiesOnRadar[e].m_type,
					fX, 
					fY,
					270.0f - RAD2DEG(pEntity->GetWorldAngles().z), 
					m_extendedStoryEntitiesOnRadar[e].m_faction,
					100, 
					100, 
					iOnScreenObjective == id, 
					iCurrentSpawnPoint == id);
				if (!m_extendedStoryEntitiesOnRadar[e].m_text.empty())
					textOnMap[id] = m_extendedStoryEntitiesOnRadar[e].m_text;
			}
		}
	}
	//~TheOtherSide
...
void CHUDRadar::ComputePositioning(Vec2 playerpos, std::vector<double>* doubleArray)
{
	...
	Vec2 vMapPos = Vec2(
		(254.0f - playerpos.x * fMapSize) + m_vPDAMapTranslation.x, 
		(254.0f + (1.0f - playerpos.y) * fMapSize) + m_vPDAMapTranslation.y);

	Vec3 vMapWorldPos = Vec3();
	...
	//if(bUpdate || m_initMap)
	{
		...
		//m_flashPDA->GetFlashPlayer()->GetViewport(x0, y0, width, heigth, ratio);
		//m_animFlashMouseLayer->GetFlashPlayer()->SetViewport(m_endOnScreenX, m_endOnScreenY, width, heigth, ratio);
		//m_animFlashMouseLayer->SetVisible(true);

		//SFlashVarValue m_mouseX = (float)0;
		//SFlashVarValue m_mouseY = (float)0;

		//m_animFlashMouseLayer->GetFlashPlayer()->GetVariable("m_mouseX", &m_mouseX);
		//m_animFlashMouseLayer->GetFlashPlayer()->GetVariable("m_mouseY", &m_mouseY);

		//Vec2 lowerBound = Vec2(m_fX - fRadarSizeOverTwo, m_fY - fRadarSizeOverTwo);	//used for flash radar position computation

		//Vec2 mapViewport = Vec2(x0,y0);

		//m_flashMouseCoords.x = g_pControlSystem->GetMouseScreenPos().x;
		//m_flashMouseCoords.y = g_pControlSystem->GetMouseScreenPos().y;

		//gEnv->pRenderer->Draw2dLabel(100, 100, 1.3f, color, false, "Map_G (%1.f,%1.f)",
		//	(vMapPos.x + vOffset.x), (vMapPos.y + vOffset.y));
		//gEnv->pRenderer->Draw2dLabel(100, 120, 1.3f, color, false, "vMapPos (%1.f,%1.f)",
		//	vMapPos.x, vMapPos.y);
		//gEnv->pRenderer->Draw2dLabel(100, 140, 1.3f, color, false, "vOffset (%1.f,%1.f)",
		//	vOffset.x, vOffset.y);
		//gEnv->pRenderer->Draw2dLabel(100, 160, 1.3f, color, false, "m_vPDAMapTranslation (%1.f,%1.f)",
		//	m_vPDAMapTranslation.x, m_vPDAMapTranslation.y);
		//gEnv->pRenderer->Draw2dLabel(100, 180, 1.3f, color, false, "m_vPDATempMapTranslation (%1.f,%1.f)",
		//	m_vPDATempMapTranslation.x, m_vPDATempMapTranslation.y);
		//gEnv->pRenderer->Draw2dLabel(100, 200, 1.3f, color, false, "m_fPDAZoomFactor (%1.f)",
		//	m_fPDAZoomFactor);
		//gEnv->pRenderer->Draw2dLabel(100, 220, 1.3f, color, false, "m_fPDATempZoomFactor (%1.f)",
		//	m_fPDATempZoomFactor); 
		//gEnv->pRenderer->Draw2dLabel(100, 180, 1.3f, color, false, "m_flashMouseCoords - lowerBound = (%f,%f)",
		//	m_flashMouseCoords.x - lowerBound.x,
		//	m_flashMouseCoords.y - lowerBound.y);

		//gEnv->pRenderer->Draw2dLabel(100, 200, 1.3f, color, false, "mapViewport (%f,%f)",
		//	mapViewport.x, mapViewport.y);
		//gEnv->pRenderer->Draw2dLabel(100, 220, 1.3f, color, false, "m_flashMouseCoords (%f,%f)",
		//	m_flashMouseCoords.x, m_flashMouseCoords.y);
		//gEnv->pRenderer->Draw2dLabel(100, 240, 1.3f, color, false, "m_startOnScreenX (%i)",
		//	m_startOnScreenX);
		//gEnv->pRenderer->Draw2dLabel(100, 260, 1.3f, color, false, "m_startOnScreenY (%i)",
		//	m_startOnScreenY);
		//gEnv->pRenderer->Draw2dLabel(100, 280, 1.3f, color, false, "m_endOnScreenX (%i)",
		//	m_endOnScreenX);
		//gEnv->pRenderer->Draw2dLabel(100, 300, 1.3f, color, false, "m_endOnScreenY (%i)",
		//	m_endOnScreenY);
		//gEnv->pRenderer->Draw2dLabel(100, 320, 1.3f, color, false, "m_mapDimX (%i)",
		//	m_mapDimX);
		//gEnv->pRenderer->Draw2dLabel(100, 340, 1.3f, color, false, "m_mapDimY (%i)",
		//	m_mapDimY);
		//gEnv->pRenderer->Draw2dLabel(100, 360, 1.3f, color, false, "m_mapDivX (%f)",
		//	m_mapDivX);
		//gEnv->pRenderer->Draw2dLabel(100, 380, 1.3f, color, false, "m_mapDivY (%f)",
		//	m_mapDivY);
	...
	//calculate icon positions in flash
	for (int i(0); i < doubleArray->size(); i += 11)
	{
		if (doubleArray->size() > i + 6)
		{			
			//x(508) = a - (( b(1.0) - c * d(508) ) + e + f)
			float xOnMap = (*doubleArray)[i + 2];
			float yOnMap = (*doubleArray)[i + 3];

			//icon x coord in flash
			(*doubleArray)[i + 2] = 254.0f - ((playerpos.x - xOnMap) * fMapSize) + m_vPDAMapTranslation.x + vOffset.x;
			
			//icon y coord in flash
			(*doubleArray)[i + 3] = 254.0f - ((playerpos.y - yOnMap) * fMapSize) + m_vPDAMapTranslation.y + vOffset.y;
			
			//TheOtherSide
			const auto pEntity = gEnv->pEntitySystem->GetEntity((EntityId)(*doubleArray)[i]);
			...
			{
				const auto playerWorldPos1 = g_pGame->GetIGameFramework()->GetClientActor()->GetEntity()->GetWorldPos();
				const auto playerMapPos1 = playerpos;

				const auto flashIconPos1 = Vec2((*doubleArray)[i + 2], (*doubleArray)[i + 3]);

				auto playerMapPos2 = Vec2(0, 0);
				auto playerWorldPos2 = Vec3(0, 0, 0);

				ConvertFromFlashToMap(flashIconPos1, playerMapPos2, playerpos);
				ConvertFromMapToWorld(playerMapPos2, playerWorldPos2);

				float color[] = { 1,1,1,1 };
				gEnv->pRenderer->Draw2dLabel(100, 90, 1.5f, color, false, "1");
				gEnv->pRenderer->Draw2dLabel(380, 90, 1.5f, color, false, "2");

				gEnv->pRenderer->Draw2dLabel(100, 100, 1.3f, color, false, "Player World Position1 (%1.f,%1.f,%1.f)",
					playerWorldPos1.x, playerWorldPos1.y, playerWorldPos1.z);

				gEnv->pRenderer->Draw2dLabel(100, 120, 1.3f, color, false, "Player Map Position1 (%.2f,%.2f)",
					playerMapPos1.x, playerMapPos1.y);

				gEnv->pRenderer->Draw2dLabel(100, 140, 1.3f, color, false, "Flash Player Position1 (%1.f,%1.f)",
					flashIconPos1.x, flashIconPos1.y);

				gEnv->pRenderer->Draw2dLabel(350, 100, 1.3f, color, false, "Player World Position2 (%1.f,%1.f,%1.f)",
					playerWorldPos2.x, playerWorldPos2.y, playerWorldPos2.z);

				gEnv->pRenderer->Draw2dLabel(350, 120, 1.3f, color, false, "Player Map Position2 (%.2f,%.2f)",
					playerMapPos2.x, playerMapPos2.y);
			}
			//~TheOtherSide
...
void CHUDRadar::Serialize(TSerialize ser)
{
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		//TheOtherSide
		amount = m_extendedStoryEntitiesOnRadar.size();
		ser.Value("AmountOfExtendedStoryEntities", amount);
		if (ser.IsReading())
		{
			m_extendedStoryEntitiesOnRadar.clear();
			m_extendedStoryEntitiesOnRadar.resize(amount);
		}
		for (int s = 0; s < amount; ++s)
		{
			ser.BeginGroup("ExtendedStoryEntity");
			ser.Value("id", m_extendedStoryEntitiesOnRadar[s].m_id);
			ser.EnumValue("type", m_extendedStoryEntitiesOnRadar[s].m_type, EFirstType, ELastType);
			ser.Value("spawnTime", m_extendedStoryEntitiesOnRadar[s].m_spawnTime);
			ser.Value("timeLimit", m_extendedStoryEntitiesOnRadar[s].m_timeLimit);
			ser.Value("text", m_extendedStoryEntitiesOnRadar[s].m_text);
			ser.EnumValue("faction", m_extendedStoryEntitiesOnRadar[s].m_faction , ENeutral, ESelf);
			ser.EndGroup();
		}

		amount = m_conquerorTeamMates.size();
		ser.Value("conquerorTeamMatesAmount", amount);
		if (amount)
		{
			if (ser.IsReading())
			{
				m_conquerorTeamMates.clear();
				m_conquerorTeamMates.resize(amount);
			}

			for (int i = 0; i < amount; ++i)
			{
				ser.BeginGroup("conquerorTeamMateId");
				ser.Value("mateID", m_conquerorTeamMates[i]);
				ser.EndGroup();
			}
		}
		//~TheOtherSide
...
void CHUDRadar::AddExtendedStoryEntity(EntityId id, FlashRadarType type /*= EWayPoint*/, const char* text /*= NULL*/, FlashRadarFaction faction /*= ENeutral*/)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id);
	if (pEntity)
	{
		std::vector<TempRadarEntityExtended>::iterator it = m_extendedStoryEntitiesOnRadar.begin();
		std::vector<TempRadarEntityExtended>::iterator end = m_extendedStoryEntitiesOnRadar.end();
		for (; it != end; ++it)
			if ((*it).m_id == id)
				return;
		TempRadarEntityExtended temp(id, type, 0.0f, text, faction);
		m_extendedStoryEntitiesOnRadar.push_back(temp);
	}
}

void CHUDRadar::RemoveExtendedStoryEntity(EntityId id)
{
	std::vector<TempRadarEntityExtended>::iterator it = m_extendedStoryEntitiesOnRadar.begin();
	std::vector<TempRadarEntityExtended>::iterator end = m_extendedStoryEntitiesOnRadar.end();
	for (; it != end; ++it)
	{
		if ((*it).m_id == id)
		{
			m_extendedStoryEntitiesOnRadar.erase(it);
			return;
		}
	}
}
...
void CHUDRadar::GetMemoryStatistics(ICrySizer* s)
{
	...
	//TheOtherSide
	s->AddContainer(m_extendedStoryEntitiesOnRadar);
	//~TheOtherSide
	...
	for (int iEntity = 0; iEntity < query.nCount; iEntity++)
	{
		...
		if (pEntity && !pEntity->IsHidden() && pEntity != pActorEntity)
		{
			...
			if (g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(id))
				...
				m_entitiesInProximity.push_back(id);
...
void CHUDRadar::ConvertFromMapToWorld(const Vec2& mapIn, Vec3& worldOut)
{
	const float xFromFlash = mapIn.x * (float)m_mapDimX + (float)m_startOnScreenX;
	const float yFromFlash = mapIn.y * (float)m_mapDimY + (float)m_startOnScreenY;

	const float xFromMap = (xFromFlash - (float)m_startOnScreenX) / (m_mapDivX * (float)m_mapDimX);
	const float yFromMap = (yFromFlash - (float)m_startOnScreenY) / (m_mapDivY * (float)m_mapDimY);
...
void CHUDRadar::ConvertFromFlashToMap(const Vec2& flashIn, Vec2& mapOut, const Vec2& playerPos)
{
	const float mapSize = 508.0f /** m_fPDAZoomFactor*/;

	const float zeroPoint = 0.0f;//254.0f

	const float xOnMap = ((flashIn.x + zeroPoint - playerPos.x /*+ m_vPDAMapTranslation.x + mapOffset.x*/) / mapSize);
	const float yOnMap = ((flashIn.y + zeroPoint - playerPos.y  /*+ m_vPDAMapTranslation.y + mapOffset.y*/) / mapSize);
