...
void CHUDRadar::Update(float fDeltaTime)
{
	...
	//CompassStealth***************************************************************
	if (CActor* pCtrlActor = pPlayer->GetControlClient()->GetControlledActor())
		UpdateCompassStealth(pCtrlActor, fDeltaTime);
	else
	...
	if (CActor* pCtrlActor = pPlayer->GetControlClient()->GetControlledActor())
		UpdateBinoculars(pCtrlActor, fDeltaTime);
	else
	...
	//*********************************MAIN ENTITY UPDATE*************************
	if (CActor* pCtrlActor = pPlayer->GetControlClient()->GetControlledActor())
		UpdateRadarEntities(pCtrlActor, fRadius, playerViewMtxInverted, numOfValues, &entityValues);
	else
...
void CHUDRadar::UpdateRadarEntities(CActor* pActor, float& fRadius, Matrix34& playerViewMtxInverted, int& numOfValues, ArrayFillHelper<double, FVAT_Double, NUM_ARRAY_FILL_HELPER_SIZE>* entityValues)
{
	...
	for (int i = 0; i < amount; ++i)
	{
		...
		if (pEntity)
		{
			...
			if (isOnRadar)
			{
				CPlayer* pNanoActor = static_cast<CPlayer*> (tempActor);
				IAIObject* pAITemp = pEntity->GetAI();

				if (pNanoActor && pAITemp && pAITemp->IsHostile(pActor->GetEntity()->GetAI(), false))
				{
					if (pNanoActor->GetActorClass() == CPlayer::GetActorClassType())
					{
						CNanoSuit* pNanoSuit = pNanoActor->GetNanoSuit();
						if (pNanoSuit)
						{
...
void CHUDRadar::RenderMapOverlay()
{
	...
	for (int i = 0; i < m_entitiesOnRadar.size(); ++i)
	{
		...
		{
			//TheOtherSide
			//CPlayer* pTempPlayer = static_cast<CPlayer*>(pTempActor);
			//if (pTempPlayer)
			//{
			//	IAIObject* pPlayerAI = pActor->GetEntity()->GetAI();
			//	IAIObject* pTempAI = pEntity->GetAI();

			//	CNanoSuit* pSuit = pTempPlayer->GetNanoSuit();

			//	if (pSuit && pTempAI && pPlayerAI)
			//	{
			//		if (pSuit->GetMode() == NANOMODE_CLOAK && pTempAI->IsHostile(pPlayerAI, false))
			//		{
			//			RemoveFromRadar(uiEntityId);
			//			break;
			//		}
...
FlashRadarType CHUDRadar::ChooseType(IEntity* pEntity, bool radarOnly)
{
	...
	if (pCls == m_pPlayerClass || pCls == m_pGrunt)
		returnType = EPlayer;
	else if (pCls == m_pAlien || pCls == m_pTrooper || pCls == m_pPlayerTrooper)
		returnType = EPlayer;
