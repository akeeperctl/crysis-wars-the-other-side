...
bool CHUD::Init()
{
	...
	if (g_pControlSystem)
	{
		...
		{
			g_pControlSystem->GetAbilitiesSystem()->InitHUD(true);
			//g_pControlSystem->GetAbilitiesSystem()->ShowHUD(true);
			//g_pControlSystem->GetAbilitiesSystem()->LoadHUDAbilities("Default");
			//g_pControlSystem->GetAbilitiesSystem()->ShowHUD(false);
		}
	}
	//~TheOtherSide
...
void CHUD::SetHUDColor()
{
	...
	else if (g_pControlSystem->GetEnabled() &&
		g_pControlSystem->GetControlClient() &&
		g_pControlSystem->GetControlClient()->GetControlledActor())
		UpdateHealth(g_pControlSystem->GetControlClient()->GetControlledActor());
	//~TheOtherSide
...
void CHUD::ResetPostSerElements()
{
	...
	if (!g_pControlSystem->GetEnabled())
		UpdateHealth(g_pGame->GetIGameFramework()->GetClientActor());
	else if (g_pControlSystem->GetEnabled() && g_pControlSystem->GetControlClient()->GetControlledActor())
		UpdateHealth(g_pControlSystem->GetControlClient()->GetControlledActor());
	//~TheOtherSide
...
bool CHUD::OnAction(const ActionId& action, int activationMode, float value)
{
	...
	if (action == rGameActions.hud_suit_menu && activationMode == eIS_Pressed && !pActor->IsHaveSlave())
	{
		//~TheOtherSide
...
void CHUD::OnPostUpdate(float frameTime)
{
	...
	CPlayer* pPlayer;
	//TheOtherSide
	if (g_pControlSystem->GetEnabled() && g_pControlSystem->GetControlClient() && g_pControlSystem->GetControlClient()->GetControlledActor())
		pPlayer = static_cast<CPlayer*>(g_pControlSystem->GetControlClient()->GetControlledActor());
	else
		pPlayer = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
	...
	if (g_pGameCVars->cl_hud <= 0 || m_cineHideHUD)
	{
		...
		if (!m_bInMenu && m_cineHideHUD && m_pHUDScopes->IsBinocularsShown() && m_pHUDScopes->m_bShowBinocularsNoHUD && pPlayer && pLocalPlayer && m_pHUDScopes->m_animBinoculars.GetVisible())
		{
			m_pHUDScopes->DisplayBinoculars(pLocalPlayer);
	...
	if (!g_pControlSystem->GetEnabled())
	{
		...
		UpdatePlayerAmmo(g_pControlSystem->GetControlClient()->GetControlledActor());
	//~TheOtherSide
...
void CHUD::SetFireMode(IItem* pItem, IFireMode* pFM, bool forceUpdate)
{
	...
	if (!pItem || !pFM)
	{
		if (!pItem)
		{
			...
			{
				if (IActor* pClient = g_pControlSystem->GetControlClient()->GetControlledActor())
					pItem = pClient->GetCurrentItem(false);
			}
			if (!pItem)
				return;
