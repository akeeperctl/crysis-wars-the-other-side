	...
	if (g_pControlSystem->GetLocalEnabled())
	{
		if (CControlClient* pCC = g_pControlSystem->GetLocalControlClient())
		{
			if (pCC->GetControlledActor())
				actor = pCC->GetControlledActor();
		}
		else
			actor = gEnv->pGame->GetIGameFramework()->GetClientActor();
	}
	else
		actor = gEnv->pGame->GetIGameFramework()->GetClientActor();
	//~TheOtherSide
...
bool CHUD::Init()
{
	...
	//TheOtherSide
	//Hunter
	m_hudFireModes["HunterSweepMOAR_PLAY"] = 15;
	m_hudFireModes["MOAR_PLAY"] = 15;
	m_hudFireModes["SingularityCannon_PLAY"] = 4;
	//~Hunter

	//Scout
	m_hudFireModes["ScoutMOAC_PLAY"] = 14; //Moac, Moar
	m_hudFireModes["Scout_MOAR_PLAY"] = 15; //Moac, Moar
	m_hudFireModes["ScoutSingularity_PLAY"] = 15;
	//~Scout

	//Trooper
	m_hudFireModes["Trooper_Melee_PLAY"] = 8;
	m_hudFireModes["FastLightMOAC_PLAY"] = 14;
	m_hudFireModes["FastLightMOAR_PLAY"] = 15;
	//~Trooper

	//Alien
	m_hudFireModes["LightMOAC_PLAY"] = 14;
	//~Alien
	//~TheOtherSide
	...
	//TheOtherSide
	//I dont want use listeners)
	if (g_pControlSystem)
	{
		if (g_pControlSystem->GetSquadSystem())
			g_pControlSystem->GetSquadSystem()->InitHUD(true);

		if (g_pControlSystem->GetConquerorSystem())
			g_pControlSystem->GetConquerorSystem()->InitHUD(true);
	}

	//m_animMapMouseLayer.Load("Libs/UI/HUD_Map_Mouse_Layer.swf", eFD_Right, eFAF_Visible);
...
void CHUD::SetHUDColor()
{
	...
	//TheOtherSide
	//if (!g_pControlSystem->GetLocalEnabled())
	//	UpdateHealth(g_pGame->GetIGameFramework()->GetClientActor());
	//else if (g_pControlSystem->GetLocalEnabled() &&
	//	g_pControlSystem->GetLocalControlClient() &&
	//	g_pControlSystem->GetLocalControlClient()->GetControlledActor())
	//	UpdateHealth(g_pControlSystem->GetLocalControlClient()->GetControlledActor());
	UpdateHealth(g_pControlSystem->GetClientActor());
	//~TheOtherSide
...
void CHUD::DisplayTimedOverlayFlashMessage(const char* label, float time, const ColorF& col /*= Col_White*/, bool formatWStringWithParams /*= false*/, const char* paramLabel1 /*= 0*/, const char* paramLabel2 /*= 0*/, const char* paramLabel3 /*= 0*/, const char* paramLabel4 /*= 0*/)
{
	if (!label)
		return;
	...
	if (m_fOverlayTextLineTimeout <= 0.0f)
		m_fOverlayTextLineTimeout = gEnv->pTimer->GetFrameStartTime().GetSeconds() + time;
...
//TheOtherSide
void CHUD::OnToggleHumanMode(IActor* pActor, bool bHumanMode)
{
	...
	if (pPlayer)
	{
		if (!bHumanMode)
		{
			...
			m_animPlayerStats.Invoke("setEnergy", (int)101);
		}

		if (bHumanMode)
		{
			m_animPlayerStats.Load("Libs/UI/HUD_AmmoHealthEnergyHuman.gfx", eFD_Right, eFAF_Visible | eFAF_ThisHandler);
...
void CHUD::SetModalHUD(CGameFlashAnimation* pAnim)
{
	m_pModalHUD = pAnim;
}
	...
	if (!pCurrent || pCurrent->GetEntityId() != m_uiWeapondID)
	{
		...
		if (pItem)
		{
			...
			if (curCategory && curClass)
				ShowInventoryOverview(pActor, curCategory, curClass, false);
		}
	}
...
void CHUD::HandleFSCommand(const char* szCommand, const char* szArgs)
{
	if (g_pGameCVars->g_debug_fscommand)
		CryLogAlways("HandleFSCommand : %s %s\n", szCommand, szArgs);
	...
	if (pSquadSystem)
	{
		auto pSquad = pSquadSystem->GetClientSquad();
		if (pSquad)
			pSquad->HandleMouseFSCommand(szCommand);
	}

	auto pConqueror = g_pControlSystem->GetConquerorSystem();
	if (pConqueror)
		pConqueror->HandleFSCommand(szCommand, szArgs);
	//~TheOtherSide
}
...
bool CHUD::OnAction(const ActionId& action, int activationMode, float value)
{
	...
	//TheOtherSide fix shit that created by Crytek
	if (GetCurrentGameRules() == EHUD_POWERSTRUGGLE)
	{
	...
	//TheOtherSide
	CPlayer* pActor = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
	if (action == rGameActions.hud_suit_menu && activationMode == eIS_Pressed && !pActor->IsHaveSlave() && !pActor->IsHumanMode())
	{
		//~TheOtherSide
	...
	else if (action == rGameActions.hud_suit_mod)
	{
		//TheOtherSide
		CPlayer* pPlayer = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
		if (!pPlayer->IsHaveSlave() && !pPlayer->IsHumanMode())
		{
			if (activationMode == 1)
...
bool CHUD::ShowPDA(bool show, bool buyMenu)
{
	...
	if (isConquest && pConqueror->m_gameOver)
	{
		if (m_bShow)
		{
	...
	//TheOtherSide
	if (isConquest && pConqueror->GetLobbyInfo().state != EConquerLobbyState::IN_LOBBY)
	{
		if (!buyMenu && show && m_pModalHUD == NULL)
		{
			ShowObjectives(true);
	...
	if (buyMenu)
	{
		if (!m_animBuyMenu.IsLoaded())
		{
			CryLogAlways("%s[C++][ERROR][Buy menu can't be showed][Cause: !m_animBuyMenu.IsLoaded]", STR_RED);
			return false;
		}
		if (m_pModalHUD == &m_animPDA)
			ShowPDA(false, false);
		...
		if (m_pHUDPowerStruggle && m_pHUDPowerStruggle->m_currentBuyZones.size() > 0)
		{
	...
	if (show && m_pModalHUD == NULL)
	{
		...
		if (pActor->GetHealth() <= 0 && !(isConquest || gEnv->bMultiplayer))
			return false;
		...
		//TheOtherSide
		SConquerLobbyInfo info = pConqueror->GetLobbyInfo();

		if (isConquest)
		{
			anim->SetVariable("GameRules", SFlashVarValue("InstantAction"));
	...
	else if (!show && ((buyMenu) ? m_pModalHUD == &m_animBuyMenu : m_pModalHUD == &m_animPDA))
	{
		...
		if (buyMenu)
		{
			const SFlashVarValue cVal(gEnv->bMultiplayer || isConquest ? "MP" : "SP");
...
{
	//TheOtherSide
	g_pControlSystem->OnHardwareMouseEvent(iX, iY, eHardwareMouseEvent);
	//~TheOtherSide
	...
	if (g_pControlSystem->GetConquerorSystem())
	{
		auto* anim = &g_pControlSystem->GetConquerorSystem()->m_animConquerorLobby;
		if (anim && anim->IsLoaded())
		{
			int x(iX), y(iY);
			anim->GetFlashPlayer()->ScreenToClient(x, y);
			anim->GetFlashPlayer()->SendCursorEvent(SFlashCursorEvent(eCursorState, x, y));
	...
	if (g_pControlSystem->GetSquadSystem())
	{
		auto* anim = &g_pControlSystem->GetSquadSystem()->m_animSquadMouseOrders;
		if (anim && anim->IsLoaded())
		{
			int x(iX), y(iY);
			anim->GetFlashPlayer()->ScreenToClient(x, y);
...
void CHUD::OnPostUpdate(float frameTime)
{
	...
	//TheOtherSide
	//if (g_pControlSystem->GetLocalEnabled() && g_pControlSystem->GetLocalControlClient() && g_pControlSystem->GetLocalControlClient()->GetControlledActor())
		//pPlayer = static_cast<CPlayer*>(g_pControlSystem->GetLocalControlClient()->GetControlledActor());
	//else
		//pPlayer = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
	...
	{
		//CPlayer* pLocalPlayer = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetClientActor());
		// possibility to show the binoculars during cutscene
		if (!m_bInMenu && m_cineHideHUD && m_pHUDScopes->IsBinocularsShown() && m_pHUDScopes->m_bShowBinocularsNoHUD && pClientActor && m_pHUDScopes->m_animBinoculars.GetVisible())
		{
			m_pHUDScopes->DisplayBinoculars(pClientActor);
	...
	if (gEnv->bMultiplayer && !pClientActor->IsAlien())
	{
	...
	if (!gEnv->pGame->GetIGameFramework()->IsGameStarted() || !pClientActor)
	{
	...
	//if (!g_pControlSystem->GetLocalEnabled())
	//{
		//UpdatePlayerAmmo(g_pGame->GetIGameFramework()->GetClientActor());
	//}
	//else if (g_pControlSystem->GetLocalEnabled() && g_pControlSystem->GetLocalControlClient()->GetControlledActor())
	...
	if (m_pModalHUD == &m_animQuickMenu)
	{
		SPlayerStats* stats = static_cast<SPlayerStats*>(pClientActor->GetActorStats());
		if ((stats && (stats->bSprinting || (pClientActor->GetStance() == STANCE_PRONE && stats->speedFlat > 0.01f))) || (GetCurrentWeapon() && GetCurrentWeapon()->IsBusy()))
		{
	...
	if (!m_bInMenu)
	{
		...
		if (pGameRules)
		{
			if (m_animSpawnCycle.IsLoaded())
			{
				...
				if (m_fRemainingReviveCycleTime == -1.0f)
	...
	{
		CWeapon* pCurrentWeapon = GetCurrentWeapon();
		if (pClientActor && pCurrentWeapon)
		{
	...
	if (m_bShow && (m_cineState == eHCS_None || gEnv->bMultiplayer) && pClientActor && !m_bInMenu)
	{
		CWeapon* pCurrentWeapon = GetCurrentWeapon();
		...
		// Grenade detector
		TrackProjectiles(g_pControlSystem->GetClientActor());

		// Binoculars and Scope
		if (m_pHUDScopes->IsBinocularsShown())
			m_pHUDScopes->DisplayBinoculars(pClientActor);
		else if (m_pHUDScopes->GetCurrentScope() >= 0)
			m_pHUDScopes->DisplayScope(pClientActor);
		...
		if (gEnv->bMultiplayer && m_currentGameRules == EHUD_POWERSTRUGGLE)
		{
			if (g_pGame->GetGameRules() && g_pGame->GetGameRules()->IsPlayerActivelyPlaying(pClientActor->GetEntityId()))
			{
		...
		if (m_animAirStrike.IsLoaded() && m_animBuyMenu.GetVisible())
		{
		...
		CGameRules* pGameRules = g_pGame->GetGameRules();
		if ((gEnv->bMultiplayer) && pClientActor->GetSpectatorMode() /*|| (pGameRules && pGameRules->GetTeamCount() > 1 && pGameRules->GetTeam(pPlayer->GetEntityId()) == 0))*/) //SPECTATOR Mode
		{
			...
			if (pClientActor)
			{
				uint8 specMode = pClientActor->GetSpectatorMode();
				if (specMode >= CActor::eASM_FirstMPMode && specMode <= CActor::eASM_LastMPMode)
				{
					...
					if (m_prevSpectatorMode != specMode || m_prevSpectatorTarget != pClientActor->GetSpectatorTarget() || m_prevSpectatorHealth != pClientActor->GetSpectatorHealth() || m_prevSpectatorTeam != pGameRules->GetTeam(pClientActor->GetEntityId()))
					{
						m_prevSpectatorMode = specMode;
						m_prevSpectatorTarget = pClientActor->GetSpectatorTarget();
						m_prevSpectatorHealth = pClientActor->GetSpectatorHealth();
						m_prevSpectatorTeam = pGameRules->GetTeam(pClientActor->GetEntityId());
						bool blink = false;
						wstring mapText, functionalityText;
						// don't want the 'press m to...' text if waiting to respawn
						if (!pGameRules->IsPlayerActivelyPlaying(pClientActor->GetEntityId()))
						{
							blink = true;
						...
						if (specMode == CActor::eASM_Follow && pClientActor->GetSpectatorTarget() != 0)
						{
							IActor* pTarget = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pClientActor->GetSpectatorTarget());
							if (pTarget)
							{
								CryFixedStringT<128> text = pTarget->GetEntity()->GetName();
								text += " (%d)";
								int health = max(0, pClientActor->GetSpectatorHealth());
	...
	else if (!m_bInMenu && pClientActor)
	{
...
void CHUD::UpdateHealth(IActor* pActor)
{
	if (pActor)
	{
		float fHealth = (pActor->GetHealth() / (float)pActor->GetMaxHealth()) * 100.0f + 1.0f;
...
bool CHUD::UpdateTimers(float frameTime)
{
	...
	auto isEditor = gEnv->bEditor;
	//~TheOtherSide
	if (!isConquest && !isEditor && m_fPlayerDeathTime && m_animWarningMessages.IsLoaded())
	{
		if (g_pGame->GetMenu()->IsActive())
		...
		else
		{
			...
			if (diff > 14.0f)
			{
				...
				auto lastSave = g_pGame->GetMenu()->GetLastInGameSave();
...
void CHUD::SetFireMode(IItem* pItem, IFireMode* pFM, bool forceUpdate)
{
	...
	if (!pItem || !pFM)
	{
		if (!pItem)
		{
			...
			{
				if (IActor* pClient = g_pControlSystem->GetLocalControlClient()->GetControlledActor())
					pItem = pClient->GetCurrentItem(false);
			}
			if (!pItem)
				return;
...
void CHUD::ActorDeath(IActor* pActor)
{
	...
	if (pActor == g_pGame->GetIGameFramework()->GetClientActor())
	{
		...
		//TheOtherSide
		auto pSystem = g_pControlSystem->GetSquadSystem();
		if (pSystem)
		{
			auto pSquad = pSystem->GetClientSquad();
			if (pSquad)
				pSystem->m_animSquadMembers.SetVisible(false);
		}
		//~TheOtherSide
	}
}
...
void CHUD::ActorRevive(IActor* pActor)
{
	...
	if (pActor == g_pGame->GetIGameFramework()->GetClientActor())
	{
		...
		if (pSystem)
		{
			auto pSquad = pSystem->GetClientSquad();
			if (pSquad)
				pSystem->m_animSquadMembers.SetVisible(true);
		}
		//~TheOtherSide
	}
}
...
void CHUD::UpdatePlayerAmmo(IActor* pActor)
{
...
void CHUD::SetWeaponName(const char* text)
{
...
void CHUD::GameOver(int localWinner, int winnerTeam, EntityId id)
{
	...
	const SFlashVarValue args[2] = { localizedText, false };
	m_animMPMessages.Invoke("addKillLog", args, 2);
}

//TheOtherSide
void CHUD::ShowInventoryOverview(IActor* pActor, const char* curCategory, const char* curItem, bool grenades)
{
	if (!curCategory || !curItem)
		return;
	...
	if (grenades)
	{
		const CPlayer* pPlayer = dynamic_cast<CPlayer*>(pActor);
		if (pPlayer)
		{
