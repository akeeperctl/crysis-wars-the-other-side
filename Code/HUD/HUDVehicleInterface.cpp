...
CHUDVehicleInterface::EVehicleHud CHUDVehicleInterface::ChooseVehicleHUD(const IVehicle* pVehicle) const
{
	...
	//TheOtherSide
	/*if ((bool)stl::find(pRadar->m_AddonVehiclesMap, cls))
	{
		if (IVehicleMovement* pMovement = pVehicle->GetMovement())
		{
			if (pMovement->GetMovementType() == IVehicleMovement::eVMT_Air)
				return EHUD_HELI;
			else if (pMovement->GetMovementType() == IVehicleMovement::eVMT_Amphibious)
				return EHUD_APC;
			else if (pMovement->GetMovementType() == IVehicleMovement::eVMT_Land)
				return EHUD_LTV;
			else if (pMovement->GetMovementType() == IVehicleMovement::eVMT_Sea)
				return EHUD_PATROLBOAT;
			else if (pMovement->GetMovementType() == IVehicleMovement::eVMT_Other)
				return EHUD_NONE;
		}
	}*/
	//~TheOtherSide
