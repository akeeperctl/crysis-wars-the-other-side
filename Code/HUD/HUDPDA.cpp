...
void CHUD::HandleFSCommandPDA(const char* strCommand, const char* strArgs)
{
	...
	else if ((0 == strcmp(strCommand, "Buy")) || (0 == strcmp(strCommand, "BuyAmmo")))
	{
		...
		if (isConquest)
		{
			auto pActor = reinterpret_cast<CActor*>(g_pGame->GetIGameFramework()->GetClientActor());
			pConqueror->GetShop()->Buy(pActor, strArgs);

			//if ((0 == strcmp(strCommand, "Buy")))
			//	pConqueror->GetShop()->BuyOther(pActor, strArgs);
			//else if ((0 == strcmp(strCommand, "BuyAmmo")))
			//	pConqueror->GetShop()->BuyAmmo(pActor, strArgs);
		}
		else if (strArgs && m_pHUDPowerStruggle)
		{
