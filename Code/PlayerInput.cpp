...
void CPlayerInput::OnAction(const ActionId& actionId, int activationMode, float value)
{
...

	{
		...
		if (!m_pPlayer->m_stats.spectatorMode)
		{
			...
			//TheOtherSide
			CControlClient* pCC = m_pPlayer->GetControlClient();
			if (pCC)
			{
				if (pCC->GetControlledActor())
				{
					pCC->OnAction(actionId, activationMode, value);
					m_actions = 0;
					m_deltaMovement.Set(0, 0, 0); //
				}

				//Update 28.07.2022: implemented with IInputEventListener
				//CSquadSystem* pSS = g_pControlSystem->GetSquadSystem();
				//if (pSS)
				//{
				//	CActor* pPlayer = static_cast<CActor*>(pCC->GetControlledActor());
				//	if (!pPlayer)
				//		pPlayer = static_cast<CActor*>(m_pPlayer);

				//	auto* pSquad = pSS->GetSquadFromMember(pPlayer, 1);
				//	if (pSquad && pSquad->GetLeader() != 0 && pSquad->GetMembersCount() > 0)
				//	{
				//		pSS->OnAction(actionId, activationMode, value);
				//	}
				//}
			}
			...
			if (pVehicle && !(m_pPlayer->m_pControlClient && m_pPlayer->m_pControlClient->GetControlledActor()))
			{
				if (m_pPlayer->m_pVehicleClient && !m_pPlayer->IsFrozen() && !g_pControlSystem->GetSquadSystem()->m_isCommandMode)
					m_pPlayer->m_pVehicleClient->OnAction(pVehicle, m_pPlayer->GetEntityId(), actionId, activationMode, value);

				m_actions = 0;
				m_deltaMovement.Set(0, 0, 0);
			}
			else if (!(m_pPlayer->m_pControlClient && m_pPlayer->m_pControlClient->GetControlledActor()) && m_pPlayer->GetHealth() > 0 && !m_pPlayer->m_stats.isFrozen.Value() && !m_pPlayer->m_stats.inFreefall.Value() && !m_pPlayer->m_stats.isOnLadder
				&& !m_pPlayer->m_stats.isStandingUp && m_pPlayer->GetGameObject()->GetAspectProfile(eEA_Physics) != eAP_Sleep)
			{
				...
				//TheOtherSide
				bool isCommandMode = g_pControlSystem->GetSquadSystem()->m_isCommandMode;
				//~TheOtherSide
				...
				if (!binoculars && (!scope || actionId == actions.use))
				{
					...
					if ((!pWeapon || !pWeapon->IsMounted()))
					{
						if ((actions.drop == actionId) && itemId)
						{
							...
							m_pPlayer->SelectNextItem(-1, true, 0);
						else if (actions.handgrenade == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.explosive == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.utility == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.small == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.medium == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.heavy == actionId && !isCommandMode)
							m_pPlayer->SelectNextItem(1, true, actionId.c_str());
						else if (actions.debug == actionId && !isCommandMode)
						{
							if (g_pGame)
							{
				...
				{
					if (actions.handgrenade == actionId && !isCommandMode)
						m_pPlayer->SelectNextItem(1, true, actionId.c_str());
					else if (actions.explosive == actionId && !isCommandMode)
						m_pPlayer->SelectNextItem(1, true, actionId.c_str());
					else if (actions.utility == actionId && !isCommandMode)
...
void CPlayerInput::PreUpdate()
{
	...
	//TheOtherSide
	if (m_pPlayer->GetControlClient() && m_pPlayer->GetControlClient()->GetControlledActor())
	{
		float iZoomScale = m_pPlayer->GetControlClient()->GetGenericParams().zoomScale;
		float oldSens = mouseSensitivity;

		bool isBino = m_pPlayer->GetControlClient()->GetGenericParams().isUsingBinocular;

		if (isBino && iZoomScale > 1)
			mouseSensitivity /= 3;
		else
			mouseSensitivity = oldSens;
	}
	//~TheOtherSide
	...
	deltaRotation = m_deltaRotation * mouseSensitivity;
	...
	{
		float color[] = { 1,1,1,1 };
		//gEnv->pRenderer->Draw2dLabel(20, 20, 2, color, false, "deltaRotation x%f y%f z%f", (float)deltaRotation.x, (float)deltaRotation.y, (float)deltaRotation.z);
		request.AddDeltaRotation(deltaRotation);
	}

	/*if (g_pControlSystem->GetEnabled())
		/g_pControlSystem->m_RotationAng = deltaRotation;*/
...
{
	//TheOtherSide
	if (activationMode == 1 && !m_pPlayer->GetSpectatorMode() && m_pPlayer->GetHealth() > 0 && !m_pPlayer->IsHaveSlave() && !m_pPlayer->IsHumanMode())
	{
		//~TheOtherSide
		if (m_pPlayer->GetNanoSuit())
		{
...
{
	//TheOtherSide
	if (m_pPlayer->GetNanoSuit() && !m_pPlayer->IsHaveSlave() && !m_pPlayer->IsHumanMode())
	{
		//~TheOtherSide
...
{
	//TheOtherSide
	if (g_pControlSystem->GetSquadSystem()->m_isCommandMode)
		return false;
	//~TheOtherSide
...
{
	if (g_pControlSystem->GetConquerorSystem()->IsGamemode() && !m_pPlayer->IsHaveSlave())
		return false;
...
{
	//if (m_pPlayer->m_pControlClient && m_pPlayer->m_pControlClient->GetControlledActor())
	//	return false;
...
{
	//TheOtherSide
	if (!m_pPlayer->m_stats.spectatorMode && !m_pPlayer->IsHaveSlave() && !m_pPlayer->m_stats.isHumanMode)
	{
		//~TheOtherSide
...
{
	//TheOtherSide
	if (!gEnv->pSystem->IsDevMode() || m_pPlayer->IsHaveSlave())
		return false;
	//~TheOtherSide
