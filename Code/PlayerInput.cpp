...
void CPlayerInput::OnAction(const ActionId& actionId, int activationMode, float value)
{
...

	{
		...
		if (!m_pPlayer->m_stats.spectatorMode)
		{
			...
			//TheOtherSide
			SControlClient* pCC = m_pPlayer->GetControlClient();
			if (pCC)
			{
				...
				SSquadSystem* pSS = g_pControlSystem->GetSquadSystem();
				if (pSS)
				{
					...
					SSquad& squad = pSS->GetSquadFromMember(pPlayer, 1);
...
{
	//TheOtherSide
	if (activationMode == 1 && !m_pPlayer->GetSpectatorMode() && m_pPlayer->GetHealth() > 0 && !m_pPlayer->IsHaveSlave())
	{
		//~TheOtherSide
		if (m_pPlayer->GetNanoSuit())
		{
...
{
	//TheOtherSide
	if (m_pPlayer->GetNanoSuit() && !m_pPlayer->IsHaveSlave())
	{
		//~TheOtherSide
...
{
	//TheOtherSide
	if (!m_pPlayer->m_stats.spectatorMode && !m_pPlayer->IsHaveSlave())
	{
		//~TheOtherSide
