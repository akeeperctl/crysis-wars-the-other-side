...
bool CBaseGrabHandler::SetGrab(SmartScriptTable& rParams)
{
	...
	if (m_grabStats.grabId < 1)
	{
		...
		if (pGrab)
		{
			...
			//TheOtherSide
			m_pActor->OnGrab(m_grabStats.grabId);

			auto pActor =static_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pGrab->GetId()));
			if (pActor)
				pActor->OnGrabbed(m_pActor->GetEntityId());
			//~TheOtherSide
...
bool CBaseGrabHandler::StartDrop()
{
	...
	if (pGrabPhys && m_grabStats.throwVector.len2() > 0.01f)
	{
		...
		//TheOtherSide
		if (CActor *pOwner = GetOwner())
		{
			string className = pOwner->GetEntity()->GetClass()->GetName();
			if (pOwner->IsHaveOwner() && className == "Hunter")
			{
				asv.w.x = -1.0f + (cry_rand() / (float)RAND_MAX * (float)(0.01f));
				asv.w.y = -1.0f + (cry_rand() / (float)RAND_MAX * (float)(0.01f));
				asv.w.z = -1.0f + (cry_rand() / (float)RAND_MAX * (float)(0.01f));
				asv.w.NormalizeSafe();
				asv.w *= 2.0f;

				CControlClient* pCC = g_pControlSystem->GetLocalControlClient();
				if (pCC)
				{
					Vec3 crosshairPos = pCC->GetCrosshairPos();
					Vec3 hunterPos = pOwner->GetEntity()->GetWorldPos();

					float len = (crosshairPos - hunterPos).GetLength();

					Vec3 throwDir = (crosshairPos - hunterPos).GetNormalized();
					throwDir.z -= 0.4f;
					throwDir.GetNormalized();
	...
	//TheOtherSide
	m_pActor->OnDrop(m_grabStats.grabId);
	
	auto pActor = static_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_grabStats.grabId));
	if (pActor)
		pActor->OnDropped(m_pActor->GetEntityId());
	//~TheOtherSide
...
bool CAnimatedGrabHandler::SetDrop(SmartScriptTable& rParams)
{
	...
	{
		//TheOtherSide
		//if (m_pActor->IsHaveOwner())
		//	m_pActor->GetAnimatedCharacter()->GetAnimationGraphState()->SetInput("Signal", m_grabStats.dropAnimGraphSignal);
		//else
