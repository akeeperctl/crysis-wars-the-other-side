...
void CTrooper::UpdateShields(float frametime)
{
	...
	case CTrooper::eShieldType_Owner:
	{
		...
		const float rechargeRate = m_shieldParams.energyRechargeRate = 120/*g_pGameCVars->ctrl_alienShieldRechargeRate*/;
		...
		if (isServer)
		{
			if (!GetOwnerActor())
			{
				if (GetHealth() != 0 && !m_empStats.isEmpState /*&& !m_shieldParams.isProjecting*/)
				{
		...
		if (pGuardian)
		{
			...
			isGuardianInEmp = pGuardian->m_empStats.isEmpState;
	...
	case CTrooper::eShieldType_Guardian:
	{
		...
		if (!IsHaveOwner())
		{
			SSquad& squad = g_pControlSystem->GetSquadSystem()->GetSquadFromMember(this, 1);
			if (squad.GetLeader() != 0)
			{
				...
				for (; it != end; it++)
				{
					...
					{
						if (IEntity* pEntity = squad.GetActor(member.actorId)->GetEntity())
						{
							string className = pEntity->GetClass()->GetName();
							bool isTrooper = className == "Trooper" || className == "PlayerTrooper";
			...
			else
			{
				...
				while (!pIt->IsEnd())
				{
					if (IEntity* pEntity = pIt->Next())
					{
						...
						const bool isTrooper = className == "Trooper" || className == "PlayerTrooper";
		...
		else
		{
			...
			if (squad.GetLeader() != 0)
			{
				...
				for (; it != end; it++)
				{
					...
					if (squad.GetActor(member.actorId))
					{
						if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(member.actorId))
						{
							const string className = pEntity->GetClass()->GetName();
							const bool isTrooper = className == "Trooper" || className == "PlayerTrooper";
...
void CTrooper::ProcessFlyControl(Vec3 &move, float frameTime)
{
	...
	if (pDude)
	{
		if (move.x || move.y)
		{
			Vec3 desiredVelocityClamped = m_pInput->deltaMovement;
...
void CTrooper::GetTrooperMovementVector(Vec3& move, float& speed, float& maxSpeed)
{
...
void CTrooper::ProcessMovement(float frameTime)
{
	...
	GetTrooperMovementVector(move, reqSpeed, maxSpeed);
	...
	//TheOtherSide
	IEntity* pControlledEntity = g_pControlSystem->GetControlClient()->GetControlledEntity();
...
*/
	...
	//TheOtherSide
	if (m_steerInertia > 0 && GetOwnerId() == 0)
	{
		//~TheOtherSide
		...
		{
			Vec3 desiredMovement(m_pInput->movementVector);
...
bool CTrooper::EnableLamLights(bool enable)
{
	...
	if (enable && m_empStats.isEmpState)
		return false;
...
void CTrooper::UpdateLamLights(float frametime, ICharacterInstance* pCharacter)
{
	...
	if (m_empStats.isEmpState && m_lamStats.isActive)
	{
		EnableLamLights(false);
		return;
	}
...
{
	m_lamStats.pLamOneEntity->SetWorldTM(Matrix34::CreateTranslationMat(pos1));
}
...
void CTrooper::STrooperRageMode::ToggleMode(const bool toggle, const float rageDuration)
{
	...
	{
		CSingle* pSingleFM = 0;
		CWeapon* pWeapon = g_pControlSystem->GetControlClient()->GetCurrentWeapon(pTrooper);
		if (pWeapon)
		{
