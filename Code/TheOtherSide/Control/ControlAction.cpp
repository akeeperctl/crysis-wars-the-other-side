...
void SControlClient::OnAction(const ActionId& action, int activationMode, float value)
{
...
void SControlClient::OnActionBinoculars(string classname, const ActionId& action, int activationMode, float value)
{
...
void SControlClient::OnActionAttack(string classname, const ActionId& action, int activationMode, float value)
{
	IInventory* pInventory = m_pControlledActor->GetInventory();
	if (!pInventory)
		return;
	...
	{
		if (pAlien->m_empStats.isEmpState)
			return;
	}
...
void SControlClient::OnActionLeanRight(string classname, const ActionId& action, int activationMode, float value)
{
	...
	else if (classname == "PlayerTrooper")
	{
		if (activationMode == eAAM_OnPress && m_trooper.canDodge && m_pControlledActor->GetActorStats()->onGround > 0)
		{
			...
			{
				PlayAnimAction("dodgeRightFwd", false);
			}
			else
			{
				PlayAnimAction("dodgeRight", false);
...
void SControlClient::OnActionLeanLeft(string classname, const ActionId& action, int activationMode, float value)
{
	if (m_generic.cannotMove)
		return;
	...
	else if (classname == "PlayerTrooper")
	{
		if (activationMode == eAAM_OnPress && m_trooper.canDodge && m_pControlledActor->GetActorStats()->onGround > 0)
		{
			...
			if (m_generic.moveDir.y > 0.0f)
			{
				PlayAnimAction("dodgeLeftFwd", false);
			}
			else
			{
				PlayAnimAction("dodgeLeft", false);
...
void SControlClient::OnActionAim(string classname, const ActionId& action, int activationMode, float value)
{
	CAlien* pAlien = static_cast<CAlien*>(m_pControlledActor);
		...
		if (activationMode == eAAM_OnPress)
		{
			...
			if (IInventory* pInventory = pAlien->GetInventory())
			{
				if (!m_pAbilitiesSystem->trooper.isCeiling)
				{
					...
					{
						if (PlayAnimAction("meleeAttack", false))
						{
					...
					else if (pAlien->GetActorStats()->inAir && m_trooper.canJumpMelee && pAlien->GetAlienEnergy() > TROOPER_MELEE_JUMP_ENERGY_COST)//when trooper melee in jump
					{
						//CryLogAlways("StoreCurrTime");
						pe_action_impulse pImpulse;
						pImpulse.impulse = m_pControlledActor->GetEntity()->GetWorldTM().GetColumn(1) * 300.f;

						IParticleEffect* pEffect = gEnv->p3DEngine->FindParticleEffect("alien_special.Trooper.doubleJumpAttack");

						if (PlayAnimAction("meleeJumpAttack", false))
						{
							...
							if (GetCurrentWeapon(m_pControlledActor))
								GetCurrentWeapon(m_pControlledActor)->MeleeAttack();
...
bool SControlClient::OnActionMoveForward(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	ApplyMovement(Vec3(0, value * 2.0f - 1.0f, 0));
	return false;
}

bool SControlClient::OnActionMoveBack(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	ApplyMovement(Vec3(0, -(value * 2.0f - 1.0f), 0));
	return false;
}

bool SControlClient::OnActionMoveLeft(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	ApplyMovement(Vec3(-(value * 2.0f - 1.0f), 0, 0));
	return false;
}

bool SControlClient::OnActionMoveRight(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	ApplyMovement(Vec3(value * 2.0f - 1.0f, 0, 0));
	return false;
}

void SControlClient::OnActionCrouch(string classname, const ActionId& action, int activationMode, float value)
{
	if (classname == "Scout" || classname == "Alien")
	{
	...
	else if (classname == "PlayerTrooper")
	{
		if (activationMode == eAAM_OnPress)
		{
			...
			PlayAnimAction("stayOnRock", true);
		}
		else if (activationMode == eAAM_OnRelease)
		{
...
void SControlClient::OnActionJump(string classname, const ActionId& action, int activationMode, float value)
{
	if (m_generic.cannotMove)
		return;
	...
	if (m_pControlledActor->IsAlien())
	{
		...
		{
			CTrooper* pTrooper = static_cast<CTrooper*>(m_pControlledActor);
			...
			if (activationMode == eAAM_OnPress)
			{
				...
				if (pTrooper->GetActorStats()->onGround > 0.25f)
				{
					...
					PlayAnimAction("CTRL_JumpStart", false);
				}
...
//shift key
void SControlClient::OnActionSprint(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		if (classname == "Scout")
		{
			...
			if (activationMode == eAAM_OnPress && m_scout.canDodge && m_pAbilitiesSystem->scout.isGyroEnabled && isMoving)
			{
				StoreCurrTime();
				m_scout.canDodge = false;
				...
				Ang3 angles(m_pControlledActor->GetAngles());
				Matrix33 baseMtx = Matrix33::CreateRotationXYZ(angles);

				Vec3 goal = baseMtx * m_generic.moveDir;
				...
				PlayAnimAction(dodgeAnim.c_str(), false);
				SpawnParticleEffect("alien_special.scout.dodge");
			}
		}
		else if (classname == "PlayerTrooper" || classname == "Trooper")
		{
...
void SControlClient::OnActionSpeedUp(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
...
void SControlClient::OnActionSpeedDown(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		m_scout.speedMult -= 0.2f;
...
void SControlClient::OnActionZoomIn(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
...
void SControlClient::OnActionZoomOut(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		switch (m_generic.zoomScale)
...
void SControlClient::OnActionNextItem(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		if (classname == "PlayerTrooper" && m_pAbilitiesSystem->trooper.doCeiling)
			return;
...
void SControlClient::OnActionPrevItem(string classname, const ActionId& action, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		if (classname == "PlayerTrooper" && m_pAbilitiesSystem->trooper.doCeiling)
...
bool SControlClient::OnActionViewLock(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
...
void SControlClient::OnActionUse(string classname, const ActionId& action, int activationMode, float value)
{
	if (IEntity* pTarget = GetMeleeTarget())
	{
		...
		if (IActor* pClient = g_pGame->GetIGameFramework()->GetClientActor())
		{
			if (!pVehicle && !pItem)
			{
				...
				if (pTable && pTargetTable)
				{
					...
					float fDist = (pTarget->GetWorldPos() - m_pControlledActor->GetEntity()->GetWorldPos()).GetLength();
					if (fDist <= 3.f && result)
					{
