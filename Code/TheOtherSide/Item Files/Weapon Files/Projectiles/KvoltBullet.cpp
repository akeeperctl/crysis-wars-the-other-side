...
int CKvoltBullet::m_waterMaterialId = -1;
IEntityClass* CKvoltBullet::EntityClass = 0;
...
void CKvoltBullet::HandleEvent(const SGameObjectEvent& event)
{
	...
	if (event.event == eGFE_OnCollision)
	{
		...
		if (pTarget)
		{
			...
			if (ok)
			{
				...
				if (m_weaponId)
				{
					...
					string targetClassName = pTarget->GetClass()->GetName();

					if (targetClassName == "Grunt" || targetClassName == "Player")
					{
						auto pTargetActor = dynamic_cast<CPlayer*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pTarget->GetId()));

						if (pTargetActor->GetHealth() <= 0)
							return;

						if (auto pSuit = pTargetActor->GetNanoSuit())
						{
							if (!pSuit->IsInvulnerable())
							{
								pSuit->Activate(false, NANODISABLE_GENERAL, 4.0f);
								pSuit->SetSuitEnergy(0.0f);

								pTargetActor->GetGameObject()->InvokeRMI(CPlayer::ClEMP(), CPlayer::EMPParams(4.0f), eRMI_ToClientChannel, pTargetActor->GetChannelId());
							}
						}

						auto classMultiplier = 1.0f;

						if (targetClassName == "Alien")
							classMultiplier = 1.5f;
						else if (targetClassName == "Player")
							classMultiplier = 0.5f;

						hitInfo.damage = m_damage * classMultiplier;
					}
		...
		if (pCollision->pEntity[0]->GetType() == PE_PARTICLE)
		{
			...
			if ((pCollision->idmat[1] == CKvoltBullet::m_waterMaterialId) &&
...
void CKvoltBullet::SetWaterMaterialId()
{
