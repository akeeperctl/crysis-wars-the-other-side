...
#define SCRIPT_REG_CLASSNAME &CScriptBind_Actor::
	...
	SCRIPT_REG_TEMPLFUNC(GetPlayerSlaveId, "");
...
int CScriptBind_Actor::GetSquadMembersCount(IFunctionHandler* pH)
{
	...
	if (g_pControlSystem)
	{
		if (SSquadSystem* pSS = g_pControlSystem->GetSquadSystem())
		{
			SSquad squad = pSS->GetSquadFromMember(pActor, 1);
			if (squad.GetLeader() != 0)
				return pH->EndFunction(squad.GetMembersCount());
		}
	}
...
int CScriptBind_Actor::GetAlienEnergy(IFunctionHandler* pH)
{
	...
	if (pActor && pActor->IsAlien())
	{
		...
		float energy = pAlien->m_energyParams.alienEnergy;
...
int CScriptBind_Actor::GetPlayerSlaveId(IFunctionHandler* pH)
{
	CActor* pActor = GetActor(pH);
	if (!pActor)
		return pH->EndFunction(0);
...
int CScriptBind_Actor::CanLaunchShockwave(IFunctionHandler* pH)
{
	...
	if (pActor && pActor->IsAlien())
	{
		...
		bool isEmpActive = pTrooper->m_empStats.isEmpState;
