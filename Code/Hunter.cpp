...
bool CHunter::NetSerialize(TSerialize ser, EEntityAspects aspect, uint8 profile, int flags)
{
...
void CHunter::ProcessRotation(float frameTime)
{
	...
	if (m_pInput->viewVector.len2() > 0.0f)
	{
		...
		m_eyeMtx.SetRotationVDir(m_pInput->viewVector.GetNormalizedSafe());
	}
...
void CHunter::ToggleShield()
{
	m_energyParams.isHunterShieldEnabled = !m_energyParams.isHunterShieldEnabled;
}

void CHunter::UpdateEnergyRecharge(float frametime)
{
	bool  isServer = gEnv->bServer;
	float rechargeTime = 300.0f;
