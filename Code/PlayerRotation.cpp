...
void CPlayerRotation::Process()
{
	...
	if (g_pControlSystem->GetLocalEnabled())
	{
		...
		string className = g_pControlSystem->GetLocalControlClient()->GetActorClassName();
		...
		if (!isSearch && !isGyro && className == "Scout" || className == "Alien")
		{
			ProcessAlienZerog();
		}
		else
...
void CPlayerRotation::ProcessAlienZerog()
{
	//float color[] = { 1,1,1,1 };
	//gEnv->pRenderer->Draw2dLabel(20, 20, 1.3f, color, false, "ProcessAlienZerog");
	...
	//view recoil in zero Gravity cause the player to rotate
