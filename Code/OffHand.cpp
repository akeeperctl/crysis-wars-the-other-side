...
constexpr auto MAX_CHOKE_SOUNDS = 5;

constexpr auto MAX_GRENADE_TYPES = 4;

constexpr auto INPUT_DEF = 0;
constexpr auto INPUT_USE = 1;
constexpr auto INPUT_LBM = 2;
constexpr auto INPUT_RBM = 3;

constexpr auto GRAB_TYPE_ONE_HANDED = 0;
constexpr auto GRAB_TYPE_TWO_HANDED = 1;
constexpr auto GRAB_TYPE_NPC = 2;

constexpr auto ITEM_NO_EXCHANGE = 0;
constexpr auto ITEM_CAN_PICKUP = 1;
constexpr auto ITEM_CAN_EXCHANGE = 2;

constexpr auto OFFHAND_RANGE = 2.5f;

//Sounds tables
namespace
{
	constexpr char G_CHOKE_SOUNDS_TABLE[MAX_CHOKE_SOUNDS][64] = 
	{
	...
	constexpr char G_DEATH_SOUNDS_TABLE[MAX_CHOKE_SOUNDS][64] = 
	{
...
namespace
{
	...
	{
	public:
		FinishOffHandAction(const EOffHandActions _eOHA, COffHand* _pOffHand)
		{
			eOHA = _eOHA;
			pOffHand = _pOffHand;
		}
...
COffHand::~COffHand()
{
	if (m_heldEntityId)
	{
		...
		if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(m_heldEntityId))
		{
			...
			if (const auto pProxy = dynamic_cast<IEntityRenderProxy*>(pEntity->GetProxy(ENTITY_PROXY_RENDER)))
...
void COffHand::Reset()
{
	...
	for (unsigned int& m_sound : m_sounds)
	{
...
bool COffHand::ReadItemParams(const IItemParamsNode* root)
{
	...
	{
		const int n = pickabletypes->GetChildCount();
		for (int i = 0; i < n; ++i)
		{
...
void COffHand::FullSerialize(TSerialize ser)
{
	...
	const EntityId oldHeldId = m_heldEntityId;
	...
	{
		const IActor* pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(oldHeldId);
		if (pActor)
		{
			...
			if (const auto       pProxy = dynamic_cast<IEntityRenderProxy*>(pActor->GetEntity()->GetProxy(ENTITY_PROXY_RENDER)))
				if (IRenderNode* pRenderNode = pProxy->GetRenderNode())
					pRenderNode->SetRndFlags(ERF_RENDER_ALWAYS, false);
		}
		else
...
void COffHand::PostPostSerialize()
{
	...
	else if ((m_heldEntityId || m_preHeldEntityId) && m_currentState != eOHS_TRANSITIONING)
	{
		...
		if (m_heldEntityId)
		{
			const IEntity* pEntity = gEnv->pEntitySystem->GetEntity(m_heldEntityId);
			if (!pEntity)
			{
			...
			else
			{
				...
				else if (m_currentState & (eOHS_HOLDING_NPC | eOHS_GRABBING_NPC | eOHS_THROWING_NPC))
				{
					//Do grabbing again
					m_currentState = eOHS_INIT_STATE;
					auto* pActor = dynamic_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_heldEntityId));
					bool  isDead = false;
	...
	if (needsReset)
	{
		m_mainHand = dynamic_cast<CItem*>(GetOwnerActor()->GetCurrentItem());
		m_mainHandWeapon = m_mainHand ? dynamic_cast<CWeapon*>(m_mainHand->GetIWeapon()) : nullptr;
...
//=============================================================
void COffHand::Select(const bool select)
{
	CWeapon::Select(select);
}

//=============================================================
void COffHand::Update(SEntityUpdateContext& ctx, const int slot)
{
...
//=============================================================
void COffHand::UpdateFPView(const float frameTime)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
	...
	else if (m_heldEntityId && m_currentState & (eOHS_HOLDING_OBJECT | eOHS_PICKING | eOHS_THROWING_OBJECT | eOHS_PICKING_ITEM | eOHS_MELEE))
	{
		...
		else if (const CActor* pActor = GetOwnerActor())
			if (auto*          pItem = dynamic_cast<CItem*>(pActor->GetCurrentItem()))
				if (auto*      pWeapon = dynamic_cast<CWeapon*>(pItem->GetIWeapon()))
...
void COffHand::UpdateCrosshairUsabilitySP()
{
	...
	if (pActor)
	{
		...
		const bool isLadder = pPlayer->IsLadderUsable();

		bool onLadder = false;
		if (const SPlayerStats* pStats = static_cast<SPlayerStats*>(pPlayer->GetActorStats()))
			onLadder = pStats->isOnLadder;

		if (CanPerformPickUp(pActor, nullptr) || (isLadder && !onLadder))
		{
			if (CHUD* pHUD = g_pGame->GetHUD())
			{
				IItem* pItem = m_pItemSystem->GetItem(m_crosshairId);
				if (g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_crosshairId))
				{
				...
				else if (pItem)
				{
					...
					if (pItem->GetIWeapon())
					{
						...
						const bool    isSocom = strcmp(pItemClass->GetName(), "SOCOM") != 0 && strcmp(pItemClass->GetName(), "AY69") != 0 ? false : true;
						const IItem*  pCurrentItem = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(pPlayer->GetInventory()->GetItemByClass(pItemClass));
						if ((!isSocom && pCurrentItem) || (isSocom && pCurrentItem && pCurrentItem->IsDualWield()))
						{
						...
						else
						{
							...
							{
								const IItem* pExchangedItem = GetExchangeItem(m_pItemSystem->GetItem(m_preHeldEntityId), pPlayer);
								if (pExchangedItem)
									pHUD->GetCrosshair()->SetUsability(true, "@game_exchange_weapon", pExchangedItem->GetEntity()->GetClass()->GetName(), itemName.c_str());
								else
...
void COffHand::UpdateCrosshairUsabilityMP()
{
	...
	if (pActor)
	{
		...
		bool onLadder = false;
		if (const auto* pStats = static_cast<SPlayerStats*>(pPlayer->GetActorStats()))
			onLadder = pStats->isOnLadder;
		...
		if (CheckItemsInProximity(info.eyePosition, info.eyeDirection, false) || (isLadder && !onLadder))
		{
			...
			if (CHUD* pHUD = g_pGame->GetHUD())
			{
				...
				if (auto* pItem = dynamic_cast<CItem*>(m_pItemSystem->GetItem(m_crosshairId)))
				{
					...
					if (pItem->GetIWeapon())
					{
						...
						const bool isSocom = strcmp(pItemClass->GetName(), "SOCOM") != 0 && 
							strcmp(pItemClass->GetName(), "AY69") != 0 ? false : true;

						const IItem* pCurrentItem = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(
							pPlayer->GetInventory()->GetItemByClass(pItemClass)
						);
...
void COffHand::UpdateHeldObject()
{
	...
	if (pEntity)
	{
		...
		if ((pPE && m_constraintId && m_checkForConstraintDelay < 0) || (pPE && !m_constraintId && pPE->GetType() == PE_ARTICULATED))
		{
...
void COffHand::UpdateGrabbedNPCState()
{
	...
	auto*        pActor = dynamic_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_heldEntityId));
	CActor*      pPlayer = GetOwnerActor();
	SActorStats* pStats = pPlayer ? pPlayer->GetActorStats() : nullptr;
...
//============================================================
void COffHand::PostFilterView(SViewParams& viewParams)
{
...
void COffHand::PostUpdate(float frameTime)
{
	...
	if (g_pGame->GetIGameFramework()->IsGamePaused() && m_currentState & (eOHS_GRABBING_NPC | eOHS_HOLDING_NPC | eOHS_THROWING_NPC))
	{
		...
		//UpdateGrabbedNPCState();
		UpdateGrabbedNPCWorldPos(pEntity, nullptr);
	}
}

//============================================================
void COffHand::UpdateGrabbedNPCWorldPos(IEntity* pEntity, const SViewParams* viewParams)
{
	if (pEntity)
	{
		...
		if (viewParams)
		{
			const SPlayerStats* stats = static_cast<SPlayerStats*>(GetOwnerActor()->GetActorStats());
			...
			const Vec3 neckLOffset(pSkeletonPose->GetAbsJointByID(neckId).t);
		...
		{
			const Vec3 itemAttachmentPos = GetSlotHelperPos(0, "item_attachment", true);
...
//=============================================================
void COffHand::OnAction(const EntityId actorId, const ActionId& actionId, const int activationMode, const float value)
{
	s_actionHandler.Dispatch(this, actorId, actionId, activationMode, value);
}

//-------------
bool COffHand::OnActionUse(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_USE, INPUT_USE, activationMode);
}

//------------
bool COffHand::OnActionAttack(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_USE, INPUT_LBM, activationMode);
}

//------------
bool COffHand::OnActionDrop(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_USE, INPUT_RBM, activationMode);
}

//------------
bool COffHand::OnActionThrowGrenade(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_THROW_GRENADE, INPUT_DEF, activationMode);
}

//------------
bool COffHand::OnActionXIThrowGrenade(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
...
//-----------
bool COffHand::OnActionSwitchGrenade(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_SWITCH_GRENADE, INPUT_DEF, activationMode);
}

//-----------
bool COffHand::OnActionXISwitchGrenade(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	if (activationMode == eAAM_OnPress)
	{
		m_fGrenadeToggleTimer = 0.0f;
...
//-----------
bool COffHand::OnActionSpecial(EntityId actorId, const ActionId& actionId, const int activationMode, float value)
{
	return ProcessOffHandActions(eOHA_MELEE_ATTACK, INPUT_DEF, activationMode);
}

//----------
bool COffHand::ProcessOffHandActions(const EOffHandActions eOHA, const int input, const int activationMode, const float value /*= 0.0f*/)
{
...
//==================================================================
bool COffHand::EvaluateStateTransition(const int requestedAction, const int activationMode, const int inputMethod)
{
	...
	case eOHA_THROW_GRENADE:
		if (activationMode == eAAM_OnPress && m_currentState == eOHS_INIT_STATE)
		{
			...
			if (auto*                   pPlayer = dynamic_cast<CPlayer*>(GetOwnerActor()))
				if (const SPlayerStats* pStats = static_cast<SPlayerStats*>(pPlayer->GetActorStats()))
	...
	case eOHA_USE:
	{
		...
		const CItem*     pMain = GetActorItem(GetOwnerActor());
		const bool invert = pMain ? pMain->IsDualWield() : false;
		...
		if (inputMethod == INPUT_LBM)
		{
			...
			if (activationMode == eAAM_OnRelease && m_currentState & eOHS_THROWING_OBJECT && (m_grabType == GRAB_TYPE_TWO_HANDED || invert))
				return true;
		}
		else if (inputMethod == INPUT_RBM)
		{
			...
			if (activationMode == eAAM_OnRelease && m_currentState & eOHS_THROWING_OBJECT && !invert)
				return true;
		}
...
//==================================================================
bool COffHand::PreExecuteAction(const int requestedAction, const int activationMode, const bool forceSelect)
{
	...
	auto*  pMainWeapon = dynamic_cast<CWeapon*>(pMain ? pMain->GetIWeapon() : nullptr);
	...
	if (exec)
	{
		...
		if (requestedAction == eOHA_THROW_GRENADE)
		{
			...
			{
				m_mainHand = dynamic_cast<CItem*>(m_mainHand->GetDualWieldSlave());
...
//=============================================================================
void COffHand::FinishAction(const EOffHandActions eOHA)
{
	switch (eOHA)
	{
	case eOHA_SWITCH_GRENADE:
	...
	case eOHA_THROW_OBJECT:
	{
		...
		if (pEntity)
		{
			...
			if (pPlayer && pPlayer->GetActorClass() == CPlayer::GetActorClassType())
				dynamic_cast<CPlayer*>(pPlayer)->NotifyObjectGrabbed(false, m_heldEntityId, false);
	...
	case eOHA_RESET: //Reset main weapon status and reset offhand
	{
		if (m_prevMainHandId && !GetOwnerActor()->IsSwimming())
		{
			...
			m_mainHand = dynamic_cast<CItem*>(GetOwnerActor()->GetCurrentItem());
			m_mainHandWeapon = dynamic_cast<CWeapon*>(m_mainHand ? m_mainHand->GetIWeapon() : nullptr);
		}
		SetCurrentFireMode(m_lastFireModeId);
		float timeDelay = 0.1f;
		if (!m_mainHand)
		{
			const SActorStats* pStats = GetOwnerActor()->GetActorStats();
	...
	case eOHA_FINISH_AI_THROW_GRENADE:
	{
		...
		if (pActor)
		{
			CItem* pMain = GetActorItem(pActor);
			if (pMain)
				pMain->PlayAction(g_pItemStrings->idle, 0, false, eIPAF_Default | eIPAF_NoBlend);
		}
	}
	break;
	}
...
void COffHand::Freeze(const bool freeze)
{
...
//==============================================================================
void COffHand::SetOffHandState(const EOffHandStates eOHS)
{
	m_currentState = eOHS;
...
//==============================================================================
void COffHand::StartSwitchGrenade(const bool xi_switch, const bool fakeSwitch)
{
	//Iterate different firemodes 
	const int firstMode = GetCurrentFireMode();
	...
	{
		if (m_mainHandWeapon && m_mainHandWeapon->GetEntity()->GetClass() == sFistsClass)
		{
			auto* pFists = dynamic_cast<CFists*>(m_mainHandWeapon);
		...
		GetScheduler()->TimerAction(m_mainHand->GetCurrentAnimationTime(eIGS_FirstPerson), CSchedulerAction<FinishOffHandAction>::Create(FinishOffHandAction(eOHA_SWITCH_GRENADE, this)), false);
	}
	else
	{
...
//==============================================================================
void COffHand::PerformThrow(const float speedScale)
{
	if (!m_fm)
		return;
	...
	auto* pThrow = dynamic_cast<CThrow*>(m_fm);
	...
	GetScheduler()->TimerAction(2000, CSchedulerAction<FinishOffHandAction>::Create(FinishOffHandAction(eOHA_FINISH_AI_THROW_GRENADE, this)), false);
}

//===============================================================================
void COffHand::PerformThrow(const int activationMode, const EntityId throwableId, int oldFMId /* = 0 */, const bool isLivingEnt /*=false*/)
{
	if (!m_fm)
		return;
...
int COffHand::CanPerformPickUp(CActor* pActor, IPhysicalEntity* pPhysicalEntity /*=NULL*/, bool getEntityInfo /*= false*/)
{
	...
	if (!getEntityInfo)
	{
		...
		CWeapon* pMainWeapon = pItem ? dynamic_cast<CWeapon*>(pItem->GetIWeapon()) : nullptr;
		if (pMainWeapon)
	...
	//if (pMC)
	{
		...
		if (lenSqr < m_range * m_range)
		{
			if (pEntity)
			{
				...
				if (auto* pActorAI = dynamic_cast<CActor*>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pEntity->GetId())))
				{
					...
					if (pActorAI->GetActorSpecies() == eGCT_HUMAN)
					{
						...
						if (static_cast<SPlayerStats*>(pActorAI->GetActorStats())->isStandingUp)
							return OH_NO_GRAB;
					}
				...
				if (auto* pItem = dynamic_cast<CItem*>(m_pItemSystem->GetItem(pEntity->GetId())))
				{
...
//========================================================================================
int COffHand::CheckItemsInProximity(const Vec3 pos, const Vec3 dir, const bool getEntityInfo)
{
	float sizeX = 1.2f;
	if (gEnv->bMultiplayer)
		sizeX = 1.75;
	const float sizeY = sizeX;
	const float sizeZup = 0.5f;
	const float sizeZdown = 1.75f;

	SEntityProximityQuery query;
	query.box = AABB(Vec3(pos.x - sizeX, pos.y - sizeY, pos.z - sizeZdown), Vec3(pos.x + sizeX, pos.y + sizeY, pos.z + sizeZup));
	...
	{
		const EntityId id = query.pEntities[i]->GetId();

		if (const CItem* pItem = dynamic_cast<CItem*>(m_pItemSystem->GetItem(id)))
			if (pItem->GetOwnerId() != GetOwnerId())
			{
				...
				{
					const float dstSqr = LinePointDistanceSqr(line, itemPos);
	...
	if (IEntity* pEntity = m_pEntitySystem->GetEntity(nearItemId))
	{
		...
		const Vec3 itemPos = bbox.GetCenter();
		const Vec3 dir = pos - itemPos;

		IPhysicalEntity* phys = pEntity->GetPhysics();

		ray_hit hit;
		if (!gEnv->pPhysicalWorld->RayWorldIntersection(itemPos, dir, ent_static | ent_rigid | ent_sleeping_rigid, rwi_stop_at_pierceable | rwi_ignore_back_faces, &hit, 1, phys ? &phys : nullptr, phys ? 1 : 0))
		{
...
//===========================================================================================
void COffHand::IgnoreCollisions(const bool ignore, const EntityId entityId /*=0*/)
{
	if (!m_heldEntityId && !entityId)
		return;

	const IEntity* pEntity = m_pEntitySystem->GetEntity(m_heldEntityId ? m_heldEntityId : entityId);
	if (!pEntity)
		return;
...
//==========================================================================================
void COffHand::DrawNear(const bool drawNear, const EntityId entityId /*=0*/)
{
	if (!m_heldEntityId && !entityId)
		return;
	...
	for (int i = 0; i < nslots; i++)
	{
		if (pEntity->GetSlotFlags(i) & ENTITY_SLOT_RENDER)
		{
			if (drawNear)
			{
				...
				if (auto* pPhysicsProxy = dynamic_cast<IEntityPhysicalProxy*>(pEntity->GetProxy(ENTITY_PROXY_PHYSICS)))
			...
			for (int n = 0; n < pEntity->GetSlotCount(); n++)
			{
				...
				if (ok && slotInfo.pStatObj)
				{
					...
					for (int j = 0; j < 2; j++)
					{
						...
						{
							const IStatObj::SSubObject* pSubObj = slotInfo.pStatObj->GetParentObject()->FindSubObject(helper.c_str());
							if (pSubObj)
							{
						...
						{
							const IStatObj::SSubObject* pSubObj = slotInfo.pStatObj->FindSubObject(helper.c_str());
							if (pSubObj)
							{
								m_holdOffset = pSubObj->tm;
								m_holdOffset.OrthonormalizeFast();
...
void COffHand::StartPickUpItem()
{
	...
	const IEntity* pPreHeldEntity = m_pEntitySystem->GetEntity(m_preHeldEntityId);
	...
	if (!pPlayer->CheckInventoryRestrictions(pPreHeldEntity->GetClass()->GetName()))
	{
		...
		const IItem* pItem = GetExchangeItem(pPickupItem, pPlayer);
		if (pItem)
		{
			const bool     dual_drop = false;
			...
			if (dual != 0)
			{
				const IItem* pDual = GetExchangeItem(pPickupItem, pPlayer);
...
{
	const IFireMode* pReloadFM = nullptr;
...
//=======================================================================================
void COffHand::PickUpObject(const bool isLivingEnt /* = false */)
{
	//Grab NPCs-----------------
	if (isLivingEnt)
		if (!GrabNPC())
	...
	const CPlayer* pPlayer = dynamic_cast<CPlayer*>(GetOwnerActor());
...
//=========================================================================================
void COffHand::ThrowObject(const int activationMode, const bool isLivingEnt /*= false*/)
{
	if (activationMode == eAAM_OnPress)
	{
...
bool COffHand::GrabNPC()
{
	...
	//Get actor
	auto* pActor = dynamic_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_preHeldEntityId));
	...
	bool  mounted = false;
	auto* currentItem = dynamic_cast<CItem*>(pActor->GetCurrentItem());
	if (currentItem)
	...
	{
		char value[256];
		const IAnimationGraphState::InputID actionInputID = pAGState->GetInputId("Action");
	...
	if (const IAISystem* pAISystem = gEnv->pAISystem)
	{
		...
		if (pAIActor)
		{
			...
			pAIActor->SetSignal(1, "OnFallAndPlay", nullptr, pEData);
		}
	}
	...
	//Disable IK
	auto* stats = static_cast<SPlayerStats*>(pActor->GetActorStats());
	...
	if (pPlayer->GetActorClass() == CPlayer::GetActorClassType())
		dynamic_cast<CPlayer*>(pPlayer)->NotifyObjectGrabbed(true, m_heldEntityId, true);
...
//=============================================================================================
void COffHand::ThrowNPC(const bool kill /*= true*/)
{
	//Get actor
	auto* pActor = dynamic_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_heldEntityId));
	...
	if (kill)
	{
		...
		const IItem* currentItem = pActor->GetCurrentItem();

		{
			const int prevHealth = pActor->GetHealth();
...
void COffHand::RunEffectOnGrabbedNPC(CActor* pNPC)
{
	...
	if (m_grabbedNPCSpecies == eGCT_TROOPER)
	{
		...
		{
			const SGameObjectEvent event(eCGE_InitiateAutoDestruction, eGOEF_ToScriptSystem);
...
//========================================================================================
void COffHand::PlaySound(const EOffHandSounds sound, const bool play)
{
	...
	case eOHSound_Choking_Human:
		...
		if (idx >= 0 && idx < MAX_CHOKE_SOUNDS)
			soundName = G_CHOKE_SOUNDS_TABLE[idx];
	//repeating = true;
		break;
	...
	case eOHSound_Kill_Human:
		...
		idx = CLAMP(idx, 0, MAX_CHOKE_SOUNDS-1);
		if (idx >= 0 && idx < MAX_CHOKE_SOUNDS)
			soundName = G_DEATH_SOUNDS_TABLE[idx];
		break;
...
void COffHand::MeleeAttack()
{
	if (m_melee)
	{
		if (CActor*                 pOwner = GetOwnerActor())
			if (const SPlayerStats* stats = static_cast<SPlayerStats*>(pOwner->GetActorStats()))
				if (stats->bLookingAtFriendlyAI)
					return;

		auto* melee = dynamic_cast<CMelee*>(m_melee);
...
float COffHand::GetObjectMassScale()
{
	...
	if (m_currentState & eOHS_HOLDING_OBJECT)
	{
		if (m_grabType == GRAB_TYPE_TWO_HANDED)
			return 0.65f;
		const float mass = CLAMP(m_heldEntityMass, 1.0f, 10.0f);
...
IItem* COffHand::GetExchangeItem(IItem* pPickupItem, CPlayer* pPlayer)
{
	...
	const char* itemCategory = nullptr;
	const char* pickupCategory = nullptr;
	if (pPickupItem && pItem)
	{
		...
		if (!strcmp(itemCategory, pickupCategory) && pItem->CanDrop())
			return pItem;
		const int i = pPlayer->GetInventory()->GetCount();
		for (int w = i - 1; w > -1; --w)
		{
			pItem = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(pPlayer->GetInventory()->GetItem(w));
			itemCategory = m_pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName());
...
//==============================================================
void COffHand::AttachGrenadeToHand(const int grenade, const bool fp /*=true*/, const bool attach /*=true*/)
{
	...
	{
		ICharacterInstance* pOwnerCharacter = GetOwnerActor() ? GetOwnerActor()->GetEntity()->GetCharacter(0) : nullptr;
		if (!pOwnerCharacter)
			return;

		IAttachmentManager* pAttachmentManager = pOwnerCharacter->GetIAttachmentManager();
		IAttachment*        pAttachment = pAttachmentManager ? pAttachmentManager->GetInterfaceByName(m_params.attachment[eIH_Left].c_str()) : nullptr;
		...
		if (pAttachment)
		{
			...
			else
			{
				...
				{
					auto* pCGFAttachment = new CCGFAttachment();
...
bool COffHand::CheckHeldItemForConflict(const EntityId target) const
{
