...
public:
	...
	void Update(SEntityUpdateContext& ctx, int slot) override;
	...
	ILINE int GetOffHandState() const
	{
	...
	bool ReadItemParams(const IItemParamsNode* root) override;
	...
	bool PerformPickUp();
	int  CanPerformPickUp(CActor* pActor, IPhysicalEntity* pPhysicalEntity = nullptr, bool getEntityInfo = false);
	...
	void UpdateGrabbedNPCWorldPos(IEntity* pEntity, const struct SViewParams* viewParams);
	...
	EntityId GetHeldEntityId() const override;
...
private:
	...
	void PostPostSerialize() override;
