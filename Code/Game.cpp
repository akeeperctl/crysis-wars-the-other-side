...
//string GetVersionInfo(LPCTSTR InfoItem)
//{
...
//	BOOL    bRetCode = FALSE;
//
//	LPCSTR lptstrFilename = "TheOtherSide";
//	DWORD   verHandle = 0;
...
//	if (verSize)
//	{
//		LPSTR   verData = new char[verSize];//(LPSTR)GlobalLock(hMem);NULL;
//		LPSTR   lpVersion = NULL;       // String pointer to Item text
//		UINT    uVersionLen = 0;
//
//		// Get the version info block
//		bRetCode = GetFileVersionInfo(szFullPath, verHandle, verSize, verData);
//		if (bRetCode && verSize && verData)
//		{
...
//			uVersionLen = 0;
//			lpVersion = NULL;
//			bRetCode = VerQueryValue(verData,
//				(LPSTR)(LPCTSTR)sName,
//				(void**)&lpVersion,
//				(UINT*)&uVersionLen);
//
...
string GetModVersion()
{
	...
	if (verSize != NULL)
	{
		...
		if (GetFileVersionInfo(lptstrFilename, verHandle, verSize, verData))
		{
			if (VerQueryValue(verData, "\\", (VOID FAR * FAR*) & lpBuffer, &size))
			{
				if (size)
				{
					...
					if (verInfo->dwSignature == 0xfeef04bd)
					{
						...
						//CryLogAlways("File Version: %d.%d.%d.%d",
...
//TheOtherSide
CControlSystem* g_pControlSystem = 0;
//~TheOtherSide
	...
	m_pDownloadTask(0)
{
	...
	//TheOtherSide
	g_pControlSystem = new CControlSystem();
	//~TheOtherSide
...
bool CGame::Init(IGameFramework* pFramework)
{
	...
	//TheOtherSide
	g_modVersion = GetModVersion();
	g_pControlSystem->Init(g_modVersion.c_str());
	//~TheOtherSide
	...
	HCURSOR g_Cursor = LoadCursor(static_cast<HINSTANCE>(g_hInst), MAKEINTRESOURCE(IDC_CURSOR1));
	SetCursor(g_Cursor);

	auto pLocalizationManager = gEnv->pSystem->GetLocalizationManager();
...
int CGame::Update(bool haveFocus, unsigned int updateFlags)
{
	...
	if (m_pFramework->IsGamePaused() == false)
	{
		...
		//TheOtherSide	
		g_pControlSystem->Update(frameTime);
		//~TheOtherSide
	}

	//TheOtherSide
	//g_pGameCVars->tos_show_version = gEnv->pSystem->IsDevMode() ? 1 : 0;

	if (g_pGameCVars->tos_show_version > 0)
	{
		...
		gEnv->pRenderer->Draw2dLabel(
			...
			height768 * 1.6f,
			color, false, "TOS Build: " + g_modVersion);

		//gEnv->pRenderer->Draw2dLabel(1260, 820, 1.5f, color, false, "TOS Build: " + g_modVersion);
	}
	//~TheOtherSide
...
void CGame::EditorResetGame(bool bStart)
{
	...
	if (bStart)
	{
		...
		//CryLogAlways("[CGame]::EditorResetGame(true)");
	}
	else
	{
		//CryLogAlways("[CGame]::EditorResetGame(false)");
...
void CGame::GameChannelDestroyed(bool isServer)
{
	if (!isServer)
	{
		...
		if (!gEnv->pSystem->IsSerializingFile())
		{
			...
			//TheOtherSide
			// buf = 0x1 memory error crash fix
			if (buf && g_pGameCVars->pAltitudeLimitCVar)
				g_pGameCVars->pAltitudeLimitCVar->ForceSet(buf.c_str());
			//~TheOtherSide
		}
...
void CGame::LoadActionMaps(const char* filename)
{
	...
	if (rootNode)
	{
		...
		// enable defaults
		pActionMapMan->EnableActionMap("theotherside", true);
...
void CGame::GetMemoryStatistics(ICrySizer* s)
{
	...
	*/
	...
	if (g_pControlSystem)
		g_pControlSystem->GetMemoryStatistics(s);
}
