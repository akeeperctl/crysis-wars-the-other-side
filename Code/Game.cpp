	...
	m_pDownloadTask(0)
{
	...
	//g_pControlSystem = new SControlSystem();
...
int CGame::Update(bool haveFocus, unsigned int updateFlags)
{
	...
	if (m_pFramework->IsGamePaused() == false)
	{
		...
		if (g_pControlSystem)
		{
			SControlClient* pControlClient = g_pControlSystem->GetControlClient();
			if (pControlClient)
			{
				//Update crosshair position and etc.
				if (GetIGameFramework()->GetClientActor())
					pControlClient->UpdateCrosshair();

				//Control client update
				if (g_pControlSystem->GetEnabled() && pControlClient->GetControlledActor())
					pControlClient->Update(frameTime);

				//Squad system update
				if (g_pControlSystem->GetSquadSystem())
					g_pControlSystem->GetSquadSystem()->Update();

				//Abilities System update
				if (g_pControlSystem->GetAbilitiesSystem())
					g_pControlSystem->GetAbilitiesSystem()->Update();
			}
		}
		//~TheOtherSide
	}
