...
CPlayer::CPlayer()
{
	...
	//TheOtherSide
	m_lastAutoPickUpScan = 0;
	m_pControlClient = 0;
	//m_pNetControlClient = new CNetControlClient(this);
	//~TheOtherSide
...
{
	//TheOtherSide
	//SAFE_DELETE(m_pNetControlClient);
	if (m_pControlClient)
	{
		if (m_pControlClient->GetControlledEntity())
			g_pControlSystem->StopLocal(false);

		g_pControlSystem->RegisterLocalControlClient(0);
...
bool CPlayer::Init(IGameObject* pGameObject)
{
	...
	//TheOtherSide
	//CryLogAlways("[C++][CPlayer id:%i::Init]", pGameObject->GetEntityId());

	if (IsPlayer())
	{
		m_customModelInfo.Reset();
		FillDefaultModelInfo();

		if (GetEntityId() == LOCAL_PLAYER_ENTITY_ID)
		{
			//g_pControlSystem->RegisterNetLocalControlClient(m_pNetControlClient);
		}
	}
	//~TheOtherSide
...
void CPlayer::PostInit(IGameObject* pGameObject)
{
	...
	m_entityName = GetEntity()->GetName();
}
...
void CPlayer::InitClient(int channelId)
{
	...
	//CryLogAlways("[C++][CPlayer][InitClient][channel <%i>]", channelId);
}
...
void CPlayer::InitLocalPlayer()
{
	...
	if (!m_pControlClient)
	{
		//gEnv->pLog->LogWarning("[C++][Player][InitLocalPlayer][Channel <% i>]", GetChannelId());
		//CryLogAlways("[C++][Player][InitLocalPlayer][Channel <%i>]",GetChannelId());

		m_pControlClient = new CControlClient(this);
		g_pControlSystem->RegisterLocalControlClient(m_pControlClient);

		/*	if (gEnv->bMultiplayer && gEnv->bClient)
			{
				int alienPlayerFlag = -1;
				SmartScriptTable controlTable;

				IScriptTable* pTable = GetEntity()->GetScriptTable();
				if (pTable)
				{
					if (pTable->GetValue("ControlParams", controlTable))
					{
						controlTable->GetValue("alienPlayerFlag", alienPlayerFlag);
						CryLogAlways("[C++][Player][InitLocalPlayer][alienPlayerFlag] %i", alienPlayerFlag);
					}
				}

				if (GetChannelId() < 3 && m_stats.spectatorMode == eASM_None)
				{
					EntityId slaveId = g_pControlSystem->GetChannelControlledId(GetChannelId());

					CryLogAlways("[C++][Player][InitLocalPlayer][spectatorMode == None][slaveId] %i", slaveId);

					g_pGame->GetGameRules()->StartControl(slaveId, GetEntityId());

					IEntity* pEntity = gEnv->pEntitySystem->FindEntityByName(slaveName);
					if ()
					{
						CryLogAlways("[C++][Player][InitLocalPlayer][pEntity] Finded");

						CActor* pActor = static_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pEntity->GetId()));
						if (pActor)
						{
							CryLogAlways("[C++][Player][InitLocalPlayer][StartControl]");

							g_pControlSystem->StartLocal(pActor);
						}
					}
				}
		...
		//{
		//	IScriptTable* pTable = g_pGame->GetGameRules()->GetEntity()->GetScriptTable();
		//	if (pTable)
		//	{
		//		ScriptHandle value;
		//		HSCRIPTFUNCTION GetDesiredActorId = 0;
		//		if (pTable->GetValue("GetDesiredActorId", GetDesiredActorId))
		//		{
		//			Script::CallReturn(gEnv->pScriptSystem, GetDesiredActorId, pTable, GetChannelId(), value);
		//			gEnv->pScriptSystem->ReleaseFunc(GetDesiredActorId);

		//			//CryLogAlways("[C++][Player][Revive][GetDesiredActorId] %i", value.n);
		//		}

		//		switch (m_stats.spectatorMode)
		//		{
		//		case CActor::eASM_None:
		//			CryLogAlways("[C++][Player][InitLocalPlayer][eASM_None]");
		//			break;
		//		case CActor::eASM_FirstMPMode:
		//			CryLogAlways("[C++][Player][InitLocalPlayer][eASM_FirstMPMode]");
		//			break;
		//		case CActor::eASM_Free:
		//			CryLogAlways("[C++][Player][InitLocalPlayer][eASM_Free]");
		//			break;
		//		case CActor::eASM_LastMPMode:
		//			CryLogAlways("[C++][Player][InitLocalPlayer][eASM_LastMPMode]");
		//			break;
		//		case CActor::eASM_Cutscene:
		//			CryLogAlways("[C++][Player][InitLocalPlayer][eASM_Cutscene]");
		//			break;
		//		}

		//		ScriptHandle desiredId;
		//		SmartScriptTable controlTable;
		...
		//			if (pTable->GetValue("ControlParams", controlTable))
		//			{
		//				controlTable->GetValue("desiredSlaveId", desiredId);
		//			}
		//		}

		//		auto* pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(desiredId.n);
		//		if (pActor)
		//		{
		//			CryLogAlways("[C++][Player][Revive][StartControl][desiredId %i]", desiredId.n);
...
void CPlayer::ProcessEvent(SEntityEvent& event)
{
	...
	if (event.event == ENTITY_EVENT_RESET)
	{
		...
		if (IsClient())
		{
			g_pControlSystem->GetConquerorSystem()->SetPlayerModel(this, m_defaultModelInfo);
			g_pControlSystem->GetConquerorSystem()->SetPlayerMaterial(this, m_defaultModelInfo);

			SetActorModelLua();
			InitActorAttachments();

			if (m_pNanoSuit && m_customModelInfo.m_character == m_defaultModelInfo.m_character || m_customModelInfo.m_character.empty())
...
void CPlayer::UpdateFirstPersonEffects(float frameTime)
{
	...
	//TheOtherSide
	auto pSlaveActor = static_cast<CActor*>(GetSlaveActor());
	bool isSquadCommand = g_pControlSystem->GetSquadSystem()->m_isCommandMode;
	//~TheOtherSide

	bool doInterference = g_pGameCVars->hud_enableAlienInterference && !m_interferenceParams.empty();

	//if (doInterference)
	if (doInterference && !(isSquadCommand || (pSlaveActor && pSlaveActor->IsAlien())))
	{
...
void CPlayer::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	if (m_stats.isRagDoll)
		adpm = eADPM_Never;
	else if (client || (gEnv->bMultiplayer && gEnv->bServer) || isConquest)
		adpm = eADPM_Never;
	else if (IsPlayer())
	...
	if (!gEnv->pSystem->IsEditorMode())
	{
		...
		if (now - m_lastAutoPickUpScan > 0.2f)
		{
			UpdateAutoPickUpItems(origin, radius);
			m_lastAutoPickUpScan = now;
		}
	}

	//if (IsClient())
		//GetNetControlClient()->UpdateLocal(frameTime);
	//~TheOtherSide
}
...
IEntity* CPlayer::LinkToVehicle(EntityId vehicleId)
{
	...
	if (pLinkedEntity)
	{
		...
		if (IVehicleSeat* pSeat = pVehicle->GetSeatForPassenger(GetEntity()->GetId()))
		{
			...
			//TheOtherSide
			g_pControlSystem->OnEnterVehicle(this, pVehicle, pSeat->GetSeatName(), m_stats.isThirdPerson);		
			m_vehicleStats.lastOperatedVehicleId = pVehicle->GetEntityId();
			m_vehicleStats.lastTimeEnter = gEnv->pTimer->GetFrameStartTime().GetSeconds();
	...
	else
	{
		...
		//TheOtherSide
		g_pControlSystem->OnExitVehicle(this);
		m_vehicleStats.lastOperatedVehicleId = 0;
		m_vehicleStats.lastTimeExit = gEnv->pTimer->GetFrameStartTime().GetSeconds();
		//~TheOtherSide
		...
		if ((GetHealth() > 0) && !isClient && !gEnv->bMultiplayer)
		{
			...
			if (pTable)
			{
				...
				if (pTable->GetValue("Properties", props))
					props->GetValue("bCannotSwim", cannotSwim);
			}
...
void CPlayer::Revive(bool fromInit)
{
	...
	if (!fromInit && IsClient())
	{
		...
		if (gEnv->bMultiplayer)
		{
			...
			{
				string gameState = g_pGame->GetGameRules()->GetCurrentStateName();

				CryLogAlways("[C++][Player][Revive][IsClient][GameRules][State] %s", gameState);
				...
				if (pTable)
				{
					if (pTable->GetValue("ControlParams", controlTable))
					{
						controlTable->GetValue("alienPlayerFlag", alienPlayer);
						controlTable->GetValue("desiredSlaveId", desiredId);
					}
				}

				auto* pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(desiredId.n);
				if (alienPlayer && pActor)
				{
					CryLogAlways("[C++][Player][Revive][StartControl][alienPlayer %i][controlledId %i]", alienPlayer, desiredId.n);
...
{
	//Responsible for synchronizing movement physics
	...
	if (aspect == IPlayerInput::INPUT_ASPECT)
	{
		...
		{
			//Responsible for synchronizing movement animated character
		...
		/*if (GetSlaveId() != 0)
		{
			CRY_ASSERT_MESSAGE(GetSlaveId(), "Player slave is not correct");
			CActor* pSlave = static_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(GetSlaveId()));
			if (pSlave)
			{
				pSlave->GetGameObject()->ChangedNetworkState(aspect);
			}
		}*/
...
void CPlayer::FullSerialize(TSerialize ser)
{
	...
	//TheOtherSide
	if (GetControlClient())
		GetControlClient()->FullSerialize(ser);
	//~TheOtherSide
...
void SPlayerStats::Serialize(TSerialize ser, unsigned aspects)
{
	...
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		//TheOtherSide
		ser.Value("isHumanMode", isHumanMode);
		//~TheOtherSide
	}
...
void CPlayer::HandleEvent(const SGameObjectEvent& event)
{
	...
	else
	{
		...
		if (event.event == eGFE_BecomeLocalPlayer)
		{
			...
			//TheOtherSide
			if (m_pNanoSuit && (!IsUsingCustomModel() || m_customModelInfo.m_character == m_defaultModelInfo.m_character))
				m_pNanoSuit->SelectSuitMaterial();
...
void CPlayer::SetSpectatorMode(uint8 mode, EntityId targetId)
{
	...
	if (mode && !m_stats.spectatorMode)
	{
		...
		//TheOtherSide
		//addition: in singleplayer spectator mode works if this line commented
		if (gEnv->bMultiplayer)
			m_health = -1;
		//~TheOtherSide
...
void CPlayer::FillDefaultModelInfo()
{
	if (!m_defaultModelInfo.IsEmpty())
		return;

	auto pEntity = GetEntity();
	if (pEntity)
	{
		auto pScriptProxy = static_cast<IEntityScriptProxy*>(pEntity->GetProxy(ENTITY_PROXY_SCRIPT));

		auto pEntityTable = pScriptProxy->GetScriptTable();
		if (pEntityTable)
		{
			SClassModel info;
			...
			if (pEntityTable->GetValue("Properties", pEntityProperties))
			{
				...
				pEntityProperties->GetValue("fileModel", character);
				pEntityProperties->GetValue("clientFileModel", fp3p);
				pEntityProperties->GetValue("fpItemHandsModel", arms);
			...
			m_defaultModelInfo = info;
		}
	}
}

//TheOtherSide
void CPlayer::TurnOnHumanMode()
{
	m_stats.isHumanMode = true;

	//g_pGameCVars->goc_enable = 1;

	//if (!m_stats.isThirdPerson)
	//	m_stats.isThirdPerson = true;
	CALL_PLAYER_EVENT_LISTENERS(OnToggleHumanMode(this, true));
}

void CPlayer::TurnOFFHumanMode()
{
	m_stats.isHumanMode = false;

	//g_pGameCVars->goc_enable = 0;

	//if (m_stats.isThirdPerson)
	//	m_stats.isThirdPerson = false;
	CALL_PLAYER_EVENT_LISTENERS(OnToggleHumanMode(this, false));
}

void CPlayer::ToggleHumanMode()
{
	m_stats.isHumanMode = !m_stats.isHumanMode;
	//g_pGameCVars->goc_enable = m_stats.isHumanMode;

	CALL_PLAYER_EVENT_LISTENERS(OnToggleHumanMode(this, m_stats.isHumanMode));
}

void CPlayer::TurnOnOnlyThirdPerson()
{
	m_stats.isOnlyThirdPerson = true;
	m_stats.movementControlMethod = 3;

	g_pGameCVars->goc_enable = 1;
	//gEnv->pSystem->GetIConsole()->GetCVar("ac_movementControlMethodHor")->Set(3);

	if (!m_stats.isThirdPerson)
		m_stats.isThirdPerson = true;
	//CALL_PLAYER_EVENT_LISTENERS(OnToggleOnlyThirdPerson(this, true));
}

void CPlayer::TurnOFFOnlyThirdPerson()
{
	m_stats.isOnlyThirdPerson = false;
	m_stats.movementControlMethod = 0;

	g_pGameCVars->goc_enable = 0;
	//gEnv->pSystem->GetIConsole()->GetCVar("ac_movementControlMethodHor")->Set(0);

	if (m_stats.isThirdPerson)
		m_stats.isThirdPerson = false;//Only third person view in Human Mode
	//CALL_PLAYER_EVENT_LISTENERS(OnToggleHumanMode(this, false));
}

void CPlayer::ToggleOnlyThirdPerson()
{
	m_stats.isOnlyThirdPerson = !m_stats.isOnlyThirdPerson;

	g_pGameCVars->goc_enable = m_stats.isOnlyThirdPerson;

	if (m_stats.isOnlyThirdPerson)
	{
		if (!m_stats.isThirdPerson)
			m_stats.isThirdPerson = true;//Only third person view in Human Mode
	}
	else
	{
		m_stats.isThirdPerson = false;
...
void CPlayer::UpdateAutoPickUpItems(Vec3& origin, float radius)
{
	if (!IsClient())
		return;
	...
	query.box = AABB(
		Vec3(origin.x - radius, origin.y - (radius * 1.33f), origin.z - radius),
		Vec3(origin.x + radius, origin.y + (radius * 1.33f), origin.z + radius));
	...
	for (int iEntity = 0; iEntity < query.nCount; iEntity++)
	{
		...
		if (pEntity && !pEntity->IsHidden() && pEntity != GetEntity())
		{
			EntityId id = pEntity->GetId();
			if (g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(id))
			{
				if (!GetLinkedVehicle())
					AutoPickUpItem(id); //when this is enabled the player looks for items to pick up automatically (ammo for current weapon)
			}
		}
	}
}

void CPlayer::ClearInterference()
{
	m_interferenceParams.clear();
}
...
//------------------------------------------------------------------------
IMPLEMENT_RMI(CPlayer, SvRequestDesiredActor)
{
	auto* pLocalActor = g_pGame->GetIGameFramework()->GetClientActor();
	if (pLocalActor)
	{
		GetGameObject()->InvokeRMI(ClSetDesiredActor(), params, eRMI_ToClientChannel, g_pGame->GetGameRules()->GetChannelId(params.ownerClientId));
		CryLogAlways("[C++][SERVER][RMI][CPlayer][SvRequestDesiredActor][Owner Channel <%i>]", g_pGame->GetGameRules()->GetChannelId(params.ownerClientId));
	}
	else
	{
		CryLogAlways("[C++][SERVER][RMI][CPlayer][SvRequestDesiredActor][LocalPlayer NULL][Owner Channel <%i>]", g_pGame->GetGameRules()->GetChannelId(params.ownerClientId));
	}

	return true;
}
//------------------------------------------------------------------------
IMPLEMENT_RMI(CPlayer, ClSetDesiredActor)
{
	//CNetControlClient* pNetControlCLient = GetNetControlClient();
	//if (!pNetControlCLient)
	//{
	//	CryLogAlways("[C++][Client][RMI][ClSetDesiredActor][This Player Channel <%i>] NET CONTROL CLIENT IS NULL", GetChannelId());
	//}

	////auto* pOwnerPlayer = static_cast<CPlayer*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(params.ownerId));
	//auto* pSlave = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(params.desiredActorId);
	//if (pSlave)
	//{
	//	CNetControlClient* pNetControl = GetNetControlClient();
	//	if (pNetControl)
	//		pNetControl->SetDesiredActor(pSlave, params.reset);
	//	else
	//	{
	//		CryLogAlways("[C++][Client][RMI][ClSetDesiredActor][Owner Channel <%i>] LOCAL CONTROL CLIENT IS NULL", GetChannelId());
	//	}
	//}
	//else
	//{
	//	CryLogAlways("[C++][Client][RMI][ClSetDesiredActor][Owner Channel <%i>] SLAVE IS NULL", GetChannelId());
	//}


	/*if (params.ownerId == g_pGame->GetIGameFramework()->GetClientActorId())
	{

	}
	else
	{
		CryLogAlways("[C++][Client][RMI][ClSetDesiredActor][Owner Channel <%i>] OWNER NOT LOCAL PLAYER", GetChannelId(params.ownerId));
	}*/
...
void CPlayer::AutoPickUpItem(EntityId itemId)
{
	...
	if (pItem->CheckAmmoRestrictions(GetEntityId()) && CheckInventoryRestrictions(pItem->GetEntity()->GetClass()->GetName()))
	{
		...
		{
			auto pItemClass = pItem->GetEntity()->GetClass();
