...
public:
	...
	struct SMovementParams
	{
		...
		SMovementParams() {};
		SMovementParams(Vec3 _look, Vec3 _delta, Vec3 _pos)
			: lookDirection(_look),
			deltaMovement(_delta),
			position(_pos)
		{
		}
		...
		{
			ser.Value("net_lookDir", lookDirection, 'dir0');
			ser.Value("net_deltaMov", deltaMovement, 'pMov');
			ser.Value("net_pos", position, 'wrld');
		}
	};

	DECLARE_CLIENT_RMI_NOATTACH_FAST(ClSetMove, SMovementParams, eNRT_ReliableUnordered);
	...
	//virtual void GetState(SSerializedAlienInput& input);
	virtual void SetState(const SSerializedAlienInput& input);
	...
	float GetAlienEnergy() { return m_energyParams.alienEnergy; };
	float GetMaxAlienEnergy() { return ALIEN_ENERGY; };
	void ResetAlienEnergy();

	ILINE virtual SAlienStats GetAlienStats() { return m_stats; };
	ILINE virtual SAlienInput* GetAlienInput() { if (m_pInput) return m_pInput; return 0; };
	...
	virtual void SetMovementTarget(const Vec3& position, const Vec3& looktarget, const Vec3& up, float speed) { m_pInput->posTarget = position; m_pInput->dirTarget = looktarget; m_pInput->upTarget = up; m_pInput->speedTarget = speed; }
...
protected:
	...
	//Members
public:
	SAlienEMPStats m_empStats;
	SAlienEnergyParams m_energyParams;
	SSerializedAlienInput m_serializedInput;

	SAlienInput* m_pInput;
	SAlienStats	m_stats;
