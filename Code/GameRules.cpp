...
bool CGameRules::Init(IGameObject* pGameObject)
{
	...
	//TheOtherSide ShotValidator can not be connected to non-player actors (controlled aliens in mp)
...
void CGameRules::ProcessEvent(SEntityEvent& event)
{
	...
	case ENTITY_EVENT_RESET:
		...
		//TheOtherSide
		m_resets.clear();
		g_pControlSystem->OnGameRulesReset();
		if (event.nParam[0] == 1)
		{
	...
	case ENTITY_EVENT_ENTER_SCRIPT_STATE:
		...
		//CryLogAlways("[GameRules][ProcessEvent][ENTITY_EVENT_ENTER_SCRIPT_STATE][m_currentStateId] %i", m_currentStateId);
...
void CGameRules::KillPlayer(CActor* pActor, bool dropItem, bool ragdoll, EntityId shooterId, EntityId weaponId, float damage, int material, int hit_type, const Vec3& impulse)
{
	...
	//TheOtherSide
	//In vehicle death animations bug fix
	auto pVehicle = pActor->GetLinkedVehicle();
	if (ragdoll && !pVehicle)
...
void CGameRules::ChangeSpectatorMode(CActor* pActor, uint8 mode, EntityId targetId, bool resetAll)
{
	...
	if (g_pControlSystem->GetLocalControlClient())
		g_pControlSystem->GetLocalControlClient()->OnChangedSpectatorMode(pActor, mode, targetId, resetAll);
	//~TheOtherSide
}
...
void CGameRules::SetTeam(int teamId, EntityId id)
{
	...
	{
		IActor* pClient = g_pControlSystem->GetLocalControlClient()->GetControlledActor();
		if (pClient)
		{
...
void CGameRules::ResetGameTime()
{
	...
	//TheOtherSide
	auto pConquest = g_pControlSystem->GetConquerorSystem();
	if (pConquest && pConquest->IsGamemode())
		timeLimit = g_pGameCVars->conq_limitTime;
	//~TheOtherSide
...
CActor* CGameRules::SpawnCustomActor(int channelId, const char* name, const char* _className, const Vec3& pos, const Ang3& angles)
{
	if (!gEnv->bServer)
		return 0;

	IEntityClass* pEntityClass = m_pEntitySystem->GetClassRegistry()->FindClass(_className);
	if (!pEntityClass)
		return 0;
	...
	{
		archetypeName = "ControlSystem.Multiplayer.Player_Trooper";
	}
	else if (className == "Scout")
	{
		archetypeName = "ControlSystem.Multiplayer.Player_Scout";
	}
	else if (className == "Hunter")
	{
		archetypeName = "ControlSystem.Multiplayer.Player_Hunter";
	}

	//CActor* pActor = static_cast<CActor*>(m_pActorSystem->CreateActor(0, name, className, pos, Quat(angles), Vec3(1, 1, 1)));
	//return pActor;

	IEntityArchetype* pArchetype = gEnv->pEntitySystem->LoadEntityArchetype(archetypeName.c_str());
	if (pArchetype)
	{
		...
		params.qRotation = Quat(angles);
		params.vScale = Vec3(1, 1, 1);
		params.nFlags = ENTITY_FLAG_TRIGGER_AREAS | ENTITY_FLAG_CASTSHADOW;
		params.bStaticEntityId = true;

		IEntity* pEntity = m_pEntitySystem->SpawnEntity(params);
		if (pEntity)
		{
			return GetActorByEntityId(pEntity->GetId());
		}
	}
...
const char* CGameRules::GetCurrentStateName()
{
...
void CGameRules::CreateEntityResetData(EntityId entityId)
{
	if (!gEnv->bServer || m_pGameFramework->IsEditing())
		return;

	auto pEntity = m_pEntitySystem->GetEntity(entityId);
	if (!pEntity)
		return;

	SEntityRespawnData resetData;
	resetData.position = pEntity->GetWorldPos();
	resetData.rotation = pEntity->GetWorldRotation();
	resetData.scale = pEntity->GetScale();
	resetData.flags = pEntity->GetFlags();
	resetData.pClass = pEntity->GetClass();

	AABB aabb;
	pEntity->GetLocalBounds(aabb);

	resetData.obb.Basis = Matrix33(pEntity->GetWorldTM());
	resetData.obb.center = pEntity->GetWorldTM().GetTranslation() + aabb.GetCenter();
	resetData.obb.size = aabb.GetSize();
	resetData.obb.bOriented = true;

	auto pScriptTable = pEntity->GetScriptTable();

	if (pScriptTable)
		pScriptTable->GetValue("Properties", resetData.properties);

	m_respawndata.insert(TEntityRespawnDataMap::value_type(entityId, resetData));
}

bool CGameRules::HasEntityResetData(EntityId entityId) const
{
	return m_resets.find(entityId) != m_resets.end();
}

float CGameRules::GetEntityResetTimer(EntityId entityId) const
{
	auto it = m_resets.find(entityId);
	if (it != m_resets.end())
...
void CGameRules::ScheduleEntityReset(EntityId entityId, bool unique, bool spatialcheck, float timer)
{
	if (!gEnv->bServer || m_pGameFramework->IsEditing())
		return;
	...
	SEntityRespawn reset;
	reset.timer = timer;
	reset.sctimer = timer;
	reset.unique = unique;
	reset.spatialcheck = spatialcheck;

	m_resets.insert(TEntityRespawnMap::value_type(entityId, reset));

}

bool CGameRules::TestEntitySpawnPosition2(primitives::box& obb)
{
	auto entTypes = ent_all;
	auto rwiflags = rwi_colltype_any | rwi_stop_at_pierceable;

	geom_contact* pContact = 0;
	float dist = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(primitives::box::type, &obb, ZERO, entTypes,
		&pContact, 0, rwiflags);
...
void CGameRules::UpdateEntitySchedules(float frameTime)
{
	...
	//TheOtherSide
	auto iter = m_resets.begin();
	auto end = m_resets.end();

	for (; iter != end; iter++)
	{
		EntityId id = iter->first;
		SEntityRespawn& respawn = iter->second;

		auto pEntity = m_pEntitySystem->GetEntity(id);
		if (!pEntity)
			continue;
		...
		if (respawn.timer <= 0.0f)
		{
			auto dit = m_respawndata.find(id);
			...
			//auto pTable = pEntity->GetScriptTable();
			//Script::CallMethod(pTable, "OnReset");

			auto pVehicle = g_pGame->GetIGameFramework()->GetIVehicleSystem()->GetVehicle(id);
			...
			pEntity->SetWorldTM(Matrix34::Create(data.scale, data.rotation, data.position));
			pEntity->SetFlags(data.flags);

			//m_pEntitySystem->InitEntity(pEntity, params);

			//stl::find_and_erase(m_resets, id);
			//stl::find_and_erase(m_respawndata, id);
...
void CGameRules::ChangeDesiredActor(EntityId ownerId, EntityId desiredActorId, bool reset)
{
	CPlayer::DesiredActorParams params;
	params.ownerClientId = ownerId;
	params.desiredActorId = desiredActorId;
	params.reset = reset;
	...
	if (pPlayer)
	{
		if (gEnv->bServer)
		{
			...
			//{
			pPlayer->GetGameObject()->InvokeRMI(CPlayer::ClSetDesiredActor(), params, eRMI_ToClientChannel, pPlayer->GetChannelId());
			//CryLogAlways("[C++][SERVER][GameRules][ChangeDesiredActor][Channel <%i> ][ownerid <%i> ]", GetChannelId(ownerId), ownerId);
			//}
		}
		else if (gEnv->bClient)
		{
			pPlayer->GetGameObject()->InvokeRMI(CPlayer::SvRequestDesiredActor(), params, eRMI_ToServer);
			//CryLogAlways("[C++][CLIENT][GameRules][ChangeDesiredActor][Channel <%i> ][ownerid <%i> ]", pPlayer->GetChannelId(), ownerId);
		}
	}
	else
	{
		//CryLogAlways("[C++][GameRules][ChangeDesiredActor][OWNER IS NULL]");
	}
}

void CGameRules::StartControl(EntityId slaveId, EntityId ownerId, bool isReset)
{
	if (!gEnv->bMultiplayer)
	{
		auto* pLocalActor = (g_pGame->GetIGameFramework()->GetClientActor());
		auto* pSlaveActor = (g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(slaveId));

		if (pLocalActor)
		{
			if (pSlaveActor && ownerId == pLocalActor->GetEntityId())
			{
				g_pControlSystem->StartLocal(pSlaveActor);
				//CryLogAlways("[C++][LocalClient][GameRules][StartControl][Owner: LOCALPLAYER][SLAVE: %s]", pSlaveActor->GetEntity()->GetName());
			}
		}
		else
		{
			if (pSlaveActor && g_pGame->GetIGameFramework()->GetClientChannel()->IsLocal())
			{
				//g_pControlSystem->StartLocalDesired(pSlaveActor);
				//CryLogAlways("[C++][LocalClient][GameRules][StartLocalDesired][ERORORROROR]");
			}
		}
	}
	else
	{
		if (gEnv->bClient)
		{
			...
			GetGameObject()->InvokeRMI(SvRequestTakeControl(), params, eRMI_ToServer);
			//CryLogAlways("[C++][Client][CGameRules][StartControl][Owner: %i][Slave: %i]", ownerId, slaveId);
		}
		if (gEnv->bServer)
		{
			ControlParams params;
			...
			if (ownerId != g_pGame->GetIGameFramework()->GetClientActorId())
			{
				GetGameObject()->InvokeRMI(ClTakeControl(), params, eRMI_ToClientChannel, GetChannelId(ownerId));
			}
		
			//CryLogAlways("[C++][Server][CGameRules][StartControl][Owner: %i][Slave: %i]", ownerId, slaveId);
		}
	}
}

void CGameRules::StopControl(EntityId ownerId)
{
	ControlParams params;
	params.beamPlayer = false;
	params.hidePlayer = false;
	params.ownerId = ownerId;
	params.slaveId = 0;
