...
void CSingle::Activate(bool activate)
{
	...
	//TheOtherSide
	m_IsAllowHeating = true;
	m_IsMachineGun = false;

	auto* pActor = m_pWeapon->GetOwnerActor();
	auto* pEntity = m_pWeapon->GetEntity();

	if (pEntity && m_pWeapon->IsSelected() && pActor && pActor->IsClient())
	{
		string weaponClassName = pEntity->GetClass()->GetName();
		if (weaponClassName == "C3_MK60" && GetClipSize() == 0 && GetAmmoCount() == 0)
		{
			UpdateMachineGunBullets(GetAmmoCount(), 2);
		}
	}
	//~TheOtherSide
...
void CSingle::StartReload(int zoomed)
{
	...
	if (pEntity && m_pWeapon->IsSelected() && pActor && pActor->IsClient())
	{
		//CryLogAlways("[WEAPON CLASS NAME] %s", pEntity->GetClass()->GetName());
		string weaponClassName = pEntity->GetClass()->GetName();
		if (weaponClassName == "C3_MK60")
		{
			UpdateMachineGunBullets(GetAmmoCount(), 1);
		}
	}
	//~TheOtherSide
...
bool CSingle::InternalShoot(IEntityClass* spawn_ammo, bool resetAnimation, bool autoreload/* =true */, bool noSound /* =false */)
{
	...
	if (pAmmo)
	{
		...
		pAmmo->Launch(pos, dir, vel, m_speed_scale);//throw grenade true
	...
	if (playerIsShooter && pActor->IsClient())
	{
		...
		if (pEntity && m_pWeapon->IsSelected() && pActor && pActor->IsClient())
		{
			...
			if (weaponClassName == "C3_MK60")
			{
				UpdateMachineGunBullets(GetAmmoCount(), 0);
			}
		}
		//~TheOtherSide
	}
...
//TheOtherSide
void CSingle::ToggleHeating(const bool toggle)
{
	//m_heat = !m_IsAllowHeating && toggle ? 1.0f : 0.0f;
	//m_overheat = !m_IsAllowHeating && toggle ? 0.0f : m_heatingparams.duration;

	if (!m_IsAllowHeating && toggle)
	{
		//m_firing = false;
		PerformOverheating();
	}
	...
	m_IsAllowHeating = toggle;
}

void CSingle::PerformOverheating()
{
	//Start and stop firing is needed for normal functioning the overheating after use the rage ability 
	StartFire();
	StopFire();
...
void CSingle::UpdateMachineGunBullets(int ammoCount, int onReason) noexcept
{
	const int neededCount = 9;

	auto pCharacter = m_pWeapon ? m_pWeapon->GetEntity()->GetCharacter(0) : NULL;
	if (pCharacter)
	{
		auto pAttachMan = pCharacter->GetIAttachmentManager();
		if (pAttachMan)
		{			
			if (onReason == 0)
			{
				...
				char attachmentNameBuffer[256];
				sprintf(attachmentNameBuffer, "bullet_%d", ammoCount);

				if (ammoCount <= neededCount)
				{
					if (auto pAttachment = pAttachMan->GetInterfaceByName(attachmentNameBuffer))
						pAttachment->HideAttachment(1);
				}
			}	
			else
			{
				for (int i = 0; i != neededCount+1; i++)
				{
					char attachmentNameBuffer[256];
					sprintf(attachmentNameBuffer, "bullet_%d", i);

					if (auto pAttachment = pAttachMan->GetInterfaceByName(attachmentNameBuffer))
					{
						pAttachment->HideAttachment(onReason == 1 ? 0 : 1);
					}					
				}
			}
		}
...
Vec3 CSingle::GetFiringPos(const Vec3& probableHit) const
{
	...
	if (pMC)
	{
		...
		if (/*pActor->IsPlayer() && */(HasFireHelper() && ShootFromHelper(pos, probableHit)))
		{
			...
			//TheOtherSide 3rd person grenade fix
...
float CSingle::GetSpread() const
{
	...
	//TheOtherSide
	//No spread for controlled alien scout
	if (g_pControlSystem->GetLocalEnabled())
	{
		if (g_pControlSystem->GetLocalControlClient())
		{
			if (CActor* pActor = static_cast<CActor*>(g_pControlSystem->GetLocalControlClient()->GetControlledActor()))
			{
				IEntity* pControlEntity = pActor->GetEntity();
				IEntity* pWeaponOwner = m_pWeapon->GetOwner();

				if (pWeaponOwner && pControlEntity)
				{
					if (g_pControlSystem->GetLocalControlClient()->GetActorClassName() == "Scout")
					{
						if (pWeaponOwner == pControlEntity)
							return m_spread;
					}
...
void CSingle::UpdateHeat(float frameTime)
{
	...
	//TheOtherSide
	if (!m_IsAllowHeating || isOwnerAIControlled)
		return;
	//~TheOtherSide
