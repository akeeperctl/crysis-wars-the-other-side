...
void CGameRules::ClientHit(const HitInfo& hitInfo)
{
	...
	//TheOtherSide
	if (g_pControlSystem->GetEnabled() && g_pControlSystem->GetControlClient())
	{
		CActor* pControlActor = g_pControlSystem->GetControlClient()->GetControlledActor();
		if ((pControlActor && pControlActor->GetEntity() == pShooter) && pTarget && (pVehicle || pActor) && !dead)
		{
		...
		if (pTarget == pControlActor->GetEntity())
		{
			g_pControlSystem->GetControlClient()->ShowClientHit(hitInfo);
...
void CGameRules::ServerHit(const HitInfo& hitInfo)
{
	...
	if (g_pControlSystem->GetEnabled())
	{
		...
		{
			g_pControlSystem->GetControlClient()->ShowHitIndicator(hitInfo);
		}
	}
...
void CGameRules::ProcessClientExplosionScreenFX(const ExplosionInfo& explosionInfo)
{
	...
	if (pClientActor)
	{
		...
		SControlClient* pCC = pPlayer->m_pControlClient;
		if (!pCC)
			return;
		//~TheOtherSide
...
{
	if (CActor* pSlave = static_cast<CActor*>(GetActorByEntityId(params.slaveId)))
	{
...
IMPLEMENT_RMI(CGameRules, SvRequestTakeControl)
{
	...
	if (gEnv->bServer)
	{
		...
		//CryLogAlways("Server accept request to take control of %s from ownerId %s", gEnv->pEntitySystem->GetEntity(params.slaveId) ? (string)gEnv->pEntitySystem->GetEntity(params.slaveId)->GetName() : string("Null"), gEnv->pEntitySystem->GetEntity(params.ownerId) ? (string)gEnv->pEntitySystem->GetEntity(params.ownerId)->GetName() : string("Null"));
	}
	else if (gEnv->bClient)
	{
		//Not called at 04.11.2020 12:01
		//This we can remove
		GetGameObject()->InvokeRMI(SvRequestTakeControl(), params, eRMI_ToServer); //eRMI_ToServer //eRMI_ToOwnClient
		if (IEntity* pClient = gEnv->pEntitySystem->GetEntity(params.ownerId))
		{
			CryLogAlways("Client %s send request to take control to server", pClient->GetName());
		}
	}
	return true;
}
...
{
	//SendChatMessage((EChatMessageType)params.type, params.sourceId, params.targetId, params.msg.c_str());
	//TakeControl(params.ownerId, params.slaveId, params.hidePlayer, params.beamPlayer);
	if (g_pControlSystem)
	{
		//CryLogAlways("OnServerRMI ClTakeControl ownerId %s", gEnv->pEntitySystem->GetEntity(params.ownerId) ? (string)gEnv->pEntitySystem->GetEntity(params.ownerId)->GetName() : string("Null"));

		bool hide = params.hidePlayer;

		CPlayer* pPlayer = static_cast<CPlayer*>(g_pGame->GetIGameFramework()->GetClientActor());

		if (pPlayer->m_pControlClient && g_pControlSystem->GetControlClient() &&
			pPlayer->m_pControlClient == g_pControlSystem->GetControlClient())
		{
			SControlClient* pCC = pPlayer->m_pControlClient;
			if (params.slaveId == 0)
			{
			...
			if (pOwner && pSlave)
			{
				if (pOwner == pPlayer && pPlayer->IsPlayer())
				{
					...
					pCC->SetActor(pSlave);
					pCC->ToggleDudeHide(hide);
					pCC->ToggleDudeBeam(params.beamPlayer);

					if (!pPlayer->IsThirdPerson() && !hide)
						pPlayer->ToggleThirdPerson();
				}
			}
		}
