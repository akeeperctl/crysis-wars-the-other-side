...
void CGameRules::ClientHit(const HitInfo& hitInfo)
{
	...
	IEntity* pClientEntity = g_pControlSystem->GetClientActor()->GetEntity();

	IEntity* pTarget = m_pEntitySystem->GetEntity(hitInfo.targetId);
	IEntity* pShooter = m_pEntitySystem->GetEntity(hitInfo.shooterId);

	IVehicle* pTargetVehicle = g_pGame->GetIGameFramework()->GetIVehicleSystem()->GetVehicle(hitInfo.targetId);
	IActor* pTargetActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(hitInfo.targetId);

	bool dead = pTargetActor ? (pTargetActor->GetHealth() <= 0) : false;

	//TheOtherSide
	if ((pClientEntity == pShooter) && pTarget && ((pTargetVehicle && !pTargetVehicle->IsDestroyed()) || pTargetActor) && !dead)
	{
		//not sure if it works
		SAFE_HUD_FUNC(GetCrosshair()->CrosshairHit());

		SAFE_HUD_FUNC(IndicateHit(false,pTarget));
		SAFE_HUD_FUNC(GetTagNames()->AddEnemyTagName(pTargetActor ? pTargetActor->GetEntityId() : pTargetVehicle->GetEntityId()));
	}

	//Play hit sound from COD
	if ((pClientEntity == pShooter) && pTarget && pTargetActor && !dead)
	{
		auto pWeaponEntity = GET_ENTITY(hitInfo.weaponId);
		if (pWeaponEntity && strcmp(pWeaponEntity->GetClass()->GetName(), "Fists") != 0)
			TOS_Sounds::PlayEx(pClientEntity, "sounds/hitsounds:hit:hit", 0.0f, 0.0f, 0.5f, FLAG_SOUND_2D, eSoundSemantic_HUD);
	}

	if (pTarget == pClientEntity)
	{
		if (g_pControlSystem->GetLocalEnabled())
			g_pControlSystem->GetLocalControlClient()->OnClientHit(hitInfo);
		...
		if (pShooter)
		{
			Vec3 dir = (pClientEntity->GetWorldPos() - pShooter->GetWorldPos());
			dir.NormalizeSafe();
...
void CGameRules::ProcessClientExplosionScreenFX(const ExplosionInfo& explosionInfo)
{
	...
	{
		//TheOtherSide 
		// Scout and hunter player must not have blur when near explosion be;
		CPlayer* pPlayer = static_cast<CPlayer*>(pClientActor);
		CControlClient* pCC = pPlayer->m_pControlClient;
		if (!pCC)
			return;
		//~TheOtherSide
		...
		{
			bool playerIsBigAlien = pCC->GetActorClassName() == "Hunter" || pCC->GetActorClassName() == "Scout";	

			if (inFOV && dist < maxBlurDistance && !playerIsBigAlien)
			{
				ray_hit hit;
		...
		//TheOtherSide
		//Any alien have immune to flashbang effect
		if (pCC->GetControlledActor() && pCC->GetControlledActor()->IsAlien())
			return;
		//~TheOtherSide
...
//TheOtherSide
//------------------------------------------------------------------------
IMPLEMENT_RMI(CGameRules, ClSetOwner)
{
	CActor* pSlave = static_cast<CActor*>(GetActorByEntityId(params.slaveId));
	CActor* pOwner = static_cast<CActor*>(GetActorByEntityId(params.ownerId));
	if (pSlave && pOwner)
	{
		pSlave->SetOwnerId(params.ownerId);
		pOwner->SetSlaveId(params.slaveId);
	}
...
//------------------------------------------------------------------------
IMPLEMENT_RMI(CGameRules, SvRequestSetOwner)
{
	if (gEnv->pSystem->IsDedicated())
		return true;

	auto* pLocalClient = g_pGame->GetIGameFramework()->GetClientActor();
	if (!pLocalClient)
	{
		CryLogAlways("[C++][RMI][GameRules][SvRequestSetOwner][Local Player NULL]");
		//return true;
	}
		
	GetGameObject()->InvokeRMI(ClSetOwner(), params, eRMI_ToAllClients);
	return true;
}
//~TheOtherSide

//------------------------------------------------------------------------
IMPLEMENT_RMI(CGameRules, SvRequestTakeControl)
{
	const int serverChannelId = 1;

	auto* pLocalActor = g_pGame->GetIGameFramework()->GetClientActor();
	if (pLocalActor)
		GetGameObject()->InvokeRMI(ClTakeControl(), params, eRMI_ToClientChannel, GetChannelId(params.ownerId));
	else
	{
		CryLogAlways("[C++][SERVER][RMI][GameRules][SvRequestTakeControl][LocalPlayer NULL][Owner Channel <%i>]", GetChannelId(params.ownerId));
	}
	...
	//if (pGF)
	//{
	//	INetContext* pContext = pGF->GetNetContext();
	//	if (pContext)
	//	{
	//		IActor* pActor = pGF->GetIActorSystem()->GetActor(params.ownerId);
	//		IActor* pSlave = pGF->GetIActorSystem()->GetActor(params.slaveId);

	//		if (pActor && pActor->IsPlayer() && pActor->GetHealth() > 0)
	//			pChannel = pActor->GetGameObject()->GetNetChannel();
	//		else
	//			pChannel = pGF->GetNetChannel(serverChannelId);

	//		if (pChannel && pSlave && pSlave->GetHealth() > 0)
	//			pContext->DelegateAuthority(pSlave->GetEntityId(), pChannel);

	//		//CryLogAlways("[GameRules][SvRequestSetOwner][Channel %i][Slave %s]", pChannel->GetLocalChannelID(), pSlaveEntity->GetName());
	//	}
	//}
...
//------------------------------------------------------------------------
IMPLEMENT_RMI(CGameRules, ClTakeControl)
{
	CActor* pOwner = GetActorByEntityId(params.ownerId);
	CActor* pSlave = GetActorByEntityId(params.slaveId);
	...
	if (!pPlayer)
	{
		if (pSlave)
		{
			if (g_pGame->GetIGameFramework()->GetClientChannel())
			{
				//g_pControlSystem->StartLocalDesired(pSlave, params.hidePlayer, params.beamPlayer);
				CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl][LocalPlayer NULL][StartLocalDesired][EROROROR]");
	...
	if (pPlayer)
	{
		CControlClient* pCC = pPlayer->GetControlClient();
		if (!pCC)
		{
			CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl][ControlClient] NULL");
		}

		if (pCC && params.slaveId == 0)
		{
			CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl] params.slaveId equal to 0");

			if (g_pControlSystem->GetLocalEnabled())
			{
				g_pControlSystem->StopLocal(false);

				pCC->ToggleDudeHide(false);
				pCC->ToggleDudeBeam(false);

				if (pPlayer->IsThirdPerson() && !pPlayer)
					pPlayer->ToggleThirdPerson();

				CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl][LocalEnabled][params.slaveId 0][StopLocal]");
			}
		}

		if (!pOwner)
		{
			CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl][pOwner] NULL");
		}

		if (!pSlave)
		{
			CryLogAlways("[C++][CLIENT][RMI][GameRules][ClTakeControl][pSlave] NULL");
		}

		if (pOwner && pSlave)
		{
			if (pOwner == pPlayer && pPlayer->IsPlayer())
			{
				g_pControlSystem->StartLocal(pSlave, params.hidePlayer, params.beamPlayer);

				//CryLogAlways("[ClTakeControl][hidePlayer %i][beamPlayer %i]", params.hidePlayer, params.beamPlayer);

				if (!pPlayer->IsThirdPerson() && !params.hidePlayer)
					pPlayer->ToggleThirdPerson();
...
IMPLEMENT_RMI(CGameRules, ClDamageIndicator)
{
	...
	{
		IActor* pLocal = m_pGameFramework->GetClientActor();
