...
// FlowIterator1.cpp
//
// Purpose: Flowgraph nodes for iteration loops dealing with entities
//
// History:
//	- 7/20/08 : File created - KAK
/////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////
class CFlowBaseIteratorNode : public CFlowBaseNode
{
protected:
	enum EInputPorts
	{
...
public:
	...
	////////////////////////////////////////////////////
	virtual ~CFlowBaseIteratorNode(void)
	{
	}
...
public:
	...
	////////////////////////////////////////////////////
	virtual ~CFlowNode_GetEntitiesInSphere(void)
	{
	}
	...
	virtual bool GetCustomConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<string>("Name", "", _HELP("Name of entity"),NULL,NULL),
			{0}
		};
	...
	virtual void OnIterStart(SActivationInfo* pActInfo)
	{
		...
		for (int i = 0; i < numEnts; ++i)
		{
			...
			if (IsValidType(type, entityType)/* && name == ""*/)
			{
				AddEntity(id);
			}
...
public:
	...
	////////////////////////////////////////////////////
	virtual ~CFlowNode_GetEntitiesInBox(void)
	{
	}
...
public:
	...
	////////////////////////////////////////////////////
	virtual ~CFlowNode_GetEntitiesInArea(void)
	{
	}
	...
	virtual bool GetCustomConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			ADD_BASE_INPUTS(),
			InputPortConfig<int>("Type", 0, _HELP("Type of entity to iterate"), 0, _UICONFIG(ENTITY_TYPE_ENUM)),
			InputPortConfig<string>("Area", 0, _HELP("Name of area shape"), 0, 0),
			InputPortConfig<int>("Species", 0, _HELP("Species filter to find entities (-1 - disable)"), 0, 0),
	...
	virtual void OnIterStart(SActivationInfo* pActInfo)
	{
		...
		// Find the entity
		IEntitySystem* pEntitySystem = gEnv->pEntitySystem;
		int InputSpecies = GetPortInt(pActInfo, EIP_FilterSpecies);
		if (pEntitySystem)
		{
				...
				if (pAreaProxy)
				{
					...
					for (int i = 0; i < numEnts; ++i)
					{
						...
						if (IsValidType(type, entityType))
						{
							...
							if (pAI && InputSpecies != -1)
							{
								IAIActor* pAIActor = pAI->CastToIAIActor(); // Get interface of ai actor
								if (pAIActor)
								{
									int ActorSpecies = pAIActor->GetParameters().m_nSpecies;
									if (pAreaProxy->IsPointWithin(pEntity->GetWorldPos(), pAreaProxy->GetHeight() == 0))
									{
										if (InputSpecies == ActorSpecies)
										{
											AddEntity(id);
										}
									}
...
public:
	...
	////////////////////////////////////////////////////
	virtual ~CFlowNode_GetEntities(void)
	{
	}
