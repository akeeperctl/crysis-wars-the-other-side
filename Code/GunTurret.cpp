...
namespace
{
	...
	void Interp(float& actual, float goal, float speed, float frameTime, float minSpeed = 0.f)
	{
...
bool CGunTurret::NetSerialize(TSerialize ser, EEntityAspects aspect, uint8 profile, int flags)
{
	...
	if (aspect == ASPECT_GOALORIENTATION)
	{
		...
		if (old_id != m_targetId)
			OnTargetLocked(m_targetId ? gEnv->pEntitySystem->GetEntity(m_targetId) : nullptr);
...
Vec3 CGunTurret::GetSweepPos(IEntity* pTarget, const Vec3& shootPos)
{
	...
	float sweepTime = m_turretparams.sweep_time / static_cast<float>(nhints);
...
bool CGunTurret::IsTargetAimable(float angleYaw, float anglePitch) const
{
	...
	ok &= (m_turretparams.max_pitch == 0.f && m_turretparams.min_pitch == 0.f) || (anglePitch < DEG2RAD(m_turretparams.max_pitch) && anglePitch > DEG2RAD(m_turretparams.min_pitch));
...
bool CGunTurret::RayCheck(IEntity* pTarget, const Vec3& pos, const Vec3& dir) const
{
	...
	if (hits = gEnv->pPhysicalWorld->RayWorldIntersection(newPos, dir, ent_all, rwi_stop_at_pierceable | rwi_colltype_any, &rayhit, 1, pSkipEnts, nSkip))
	{
		if (rayhit.pCollider)
		{
			IEntity* pEntity = static_cast<IEntity*>(rayhit.pCollider->GetForeignData(PHYS_FOREIGN_ID_ENTITY));

			if (pEntity == pTarget)
			{
				return true;
			}
			if (CActor* pActor = GetActor(pTarget->GetId()))
			{
...
bool CGunTurret::IsTargetCloaked(IActor* pActor) const
{
	...
	if (static_cast<CActor*>(pActor)->GetActorClass() == CPlayer::GetActorClassType())
		if (CNanoSuit*            pSuit = static_cast<CPlayer*>(pActor)->GetNanoSuit())
...
void CGunTurret::HandleEvent(const SGameObjectEvent& event)
{
	...
	{
		if (event.ptr)
			m_destinationId = static_cast<IEntity*>(event.ptr)->GetId();
	}
	break;
