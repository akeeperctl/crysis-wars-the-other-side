...
void CLam::AttachLAMLight(bool attach, CItem* pLightAttach, eGeometrySlot slot)
{
	...
	if (attach)
	{
		...
		//TheOtherSide
		CActor* pOwner = pLightAttach->GetOwnerActor();
		if (pOwner && pOwner->IsLocalOwner())
		{
			const char* material = 0;
			string name = g_pControlSystem->GetControlClient()->GetActorClassName();
			if (name == "Scout")
				m_lightID[id] = pLightAttach->AttachLightEx(slot, 0, true, fakeLight, castShadows, pCasterException, m_lamparams.light_range[id] * 2, color, specular, m_lamparams.light_texture[id], m_lamparams.light_fov[id], helper.c_str(), localOffset, dir, material, m_lamparams.light_hdr_dyn[id]);
			else
				m_lightID[id] = pLightAttach->AttachLightEx(slot, 0, true, fakeLight, castShadows, pCasterException, m_lamparams.light_range[id], color, specular, m_lamparams.light_texture[id], m_lamparams.light_fov[id], helper.c_str(), localOffset, dir, material, m_lamparams.light_hdr_dyn[id]);
		}
		else
...
{
	/*float color[] = { 1,1,1,1 };
	gEnv->pRenderer->Draw2dLabel(240, 360, 1.f, color, 0, "pOwnerItem");*/

	if (CActor* pActor = GetOwnerActor())
	{
		string className = pActor->GetEntity()->GetClass()->GetName();
		if (className == "PlayerTrooper" || className == "Trooper")
			UpdateTrooperLaser(frameTime, pActor);
	}
...
void CLam::UpdateTPLaser(float frameTime, CItem* parent)
{
	...
	Vec3   lamPos = pRootEnt->GetWorldPos();
	Vec3   dir = pRootEnt->GetWorldRotation().GetColumn1();
...
void CLam::UpdateTrooperLaser(float frameTime, IActor* trooper)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
	...
	if (trooper)
	{
		if (CActor* pOwner = static_cast<CActor*>(trooper))
		{
			ICharacterInstance* pCharacter = pOwner->GetEntity()->GetCharacter(0);
			if (pCharacter && !pCharacter->IsCharacterVisible())
				charNotVisible = true;
	...
	// workaround??: Use player movement controller locations, or else the laser
	// pops all over the place when character out of the screen.
	CActor* pActor = static_cast<CActor*>(trooper);
	if (pActor && (!pActor->IsPlayer() || charNotVisible))
	{
		if (IMovementController* pMC = pActor->GetMovementController())
		{
