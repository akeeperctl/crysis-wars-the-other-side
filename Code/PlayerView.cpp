...
void CPlayerView::ViewPreProcess(const CPlayer& rPlayer, SViewParams& viewParams, SViewStateIn& m_in)
{
	// SViewStateIn --- the constants
	{
		...
		//TheOtherSide
		if (rPlayer.m_pControlClient)
			m_in.pControlledActor = rPlayer.m_pControlClient->GetControlledActor();
		m_in.bForceLowerWeapon = rPlayer.m_stats.bForceLowerWeapon;
...
void CPlayerView::ViewProcess(SViewParams& viewParams)
{
	...
	//TheOtherSide
	else if (m_in.pControlledActor)
	{
		ViewControlledEntity(viewParams);
	}
	//~TheOtherSide
	...
	else if (m_in.pVehicle && !m_in.pControlledActor)
	{
...
void CPlayerView::ViewFirstThirdSharedPre(SViewParams& viewParams)
{
	...
	//Для позиции по умолчанию используется GetWorldPos;
...
void CPlayerView::ViewFirstPerson(SViewParams& viewParams)
{
	...
	else if ((m_in.bForceLowerWeapon || m_in.bLookingAtFriendlyAI) && !weaponZomming && !weaponZoomed)
	{
...
//TheOtherSide
void CPlayerView::ViewControlledEntity(SViewParams& viewParams)
{
	CPlayer* pThisPlayer = static_cast<CPlayer*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(m_in.entityId));
	if (pThisPlayer && pThisPlayer->IsPlayer())
	{
		if (CControlClient* pCC = pThisPlayer->GetControlClient())
		{
			if (pCC->GetControlledActor())
				pCC->UpdateView(viewParams);
		}
	}
