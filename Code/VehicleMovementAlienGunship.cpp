...
#include "VehicleMovementAlienGunship.h"
...
//------------------------------------------------------------------------
CVehicleMovementAlienGunship::CVehicleMovementAlienGunship()
	: m_horizontal(0.0f),
	m_isVTOLMovement(true),
	m_forwardInverseMult(0.2f),
	m_engineForwardAnimTime(1.0f),
	m_engineForwardAnimTimeInterp(1.0f),
	m_strafeAction(0.0f),
	m_strafeForce(1.0f),
	m_relaxTimer(0.0f),
	...
	m_enginesRotAnimTime(0.5f)
{
}

//------------------------------------------------------------------------
bool CVehicleMovementAlienGunship::Init(IVehicle* pVehicle, const SmartScriptTable& table)
{
	...
	m_pEngineForwardAnimation = NULL;
	m_wingHorizontalStateId = InvalidVehicleAnimStateId;
	m_wingVerticalStateId = InvalidVehicleAnimStateId;

	m_pEnginesRotAnimation = nullptr;

	if (!table->GetValue("timeUntilWingsRotate", m_timeUntilWingsRotate))//нерабочее гавно
		m_timeUntilWingsRotate = 0.65f;//берется отсюда
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::PostInit()
{
	CVehicleMovementHelicopter::PostInit();

	m_pEngineForwardAnimation = m_pVehicle->GetAnimation("engineForward");

	if (m_pEngineForwardAnimation)
	{
		m_wingHorizontalStateId = m_pEngineForwardAnimation->GetStateId("tohorizontal");
		m_wingVerticalStateId = m_pEngineForwardAnimation->GetStateId("tovertical");

		m_pEngineForwardAnimation->ToggleManualUpdate(true);
	}

	m_pEnginesRotAnimation = m_pVehicle->GetAnimation("enginesRotation");

	if (m_pEnginesRotAnimation)
	{
		m_pEnginesRotAnimation->ToggleManualUpdate(true);
	}
}

//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::Reset()
{
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::SetHorizontalMode(float horizontal)
{
	if (horizontal > 0.0f)
	{
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::PreProcessMovement(const float deltaTime)
{
	...
	Interpolate(m_engineForwardAnimTime, 1.0f - (m_wingsTimer / m_timeUntilWingsRotate), m_wingsSpeed, deltaTime);
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::ResetActions()
{
...
//------------------------------------------------------------------------
bool CVehicleMovementAlienGunship::StartEngine(EntityId driverId)
{
	...
	if (m_pEngineForwardAnimation)
	{
		m_pEngineForwardAnimation->StartAnimation();
		m_pEngineForwardAnimation->ToggleManualUpdate(true);
	}

	if (m_pEnginesRotAnimation)
	{
		m_pEnginesRotAnimation->StartAnimation();
		m_pEnginesRotAnimation->ToggleManualUpdate(true);
	}
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::StopEngine()
{
	CVehicleMovementHelicopter::StopEngine();

	if (m_pEngineForwardAnimation)
	{
		m_pEngineForwardAnimation->StopAnimation();
	}

	if (m_pEnginesRotAnimation)
	{
		m_pEnginesRotAnimation->StopAnimation();
	}
}

//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::ProcessActions(const float deltaTime)
{
	...
	pRenderer->Draw2dLabel(5.0f, 320.0f, 1.5f, color, false, "AlienGunship m_wingsAnimTime:  %f", m_engineForwardAnimTime);
	pRenderer->Draw2dLabel(5.0f, 340.0f, 1.5f, color, false, "AlienGunship m_wingsSpeed:  %f", m_wingsSpeed);
	pRenderer->Draw2dLabel(5.0f, 360.0f, 1.5f, color, false, "AlienGunship m_wingsTimer:  %f", m_wingsTimer);
	pRenderer->Draw2dLabel(5.0f, 380.0f, 1.5f, color, false, "AlienGunship m_wingsAnimTimeInterp:  %f", m_engineForwardAnimTimeInterp);
	pRenderer->Draw2dLabel(5.0f, 400.0f, 1.5f, color, false, "AlienGunship m_pWingsAnimation->GetState():  %f", (int)m_pEngineForwardAnimation->GetState());
	gEnv->pRenderer->Draw2dLabel(5.0f, 420.0f, 1.5f, color, false, "AlienGunship abs(m_turnAction) %f", abs(m_turnAction));
	gEnv->pRenderer->Draw2dLabel(5.0f, 440.0f, 1.5f, color, false, "AlienGunship m_soundParamTurn %f", m_soundParamTurn);
	gEnv->pRenderer->Draw2dLabel(5.0f, 460.0f, 1.5f, color, false, "AlienGunship m_desiredRoll %f", m_desiredRoll);
	gEnv->pRenderer->Draw2dLabel(5.0f, 480.0f, 1.5f, color, false, "AlienGunship m_actionRoll %f", m_actionRoll);
	gEnv->pRenderer->Draw2dLabel(5.0f, 500.0f, 1.5f, color, false, "AlienGunship m_actionYaw %f", m_actionYaw);*/
...
void CVehicleMovementAlienGunship::ProcessAI(const float deltaTime)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::OnEvent(EVehicleMovementEvent event, const SVehicleMovementEventParams& params)
{
	...
	if (event == eVME_SetMode)
	{
		...
		if (requestedMovement != m_isVTOLMovement)
		{
			...
			{
				//CryLogAlways("m_pWingsAnimation->ChangeState() to m_wingHorizontalStateId");
				m_pEngineForwardAnimation->ChangeState(m_wingHorizontalStateId);
			}
			else
			{
				//CryLogAlways("m_pWingsAnimation->ChangeState() to m_wingVerticalStateId");
				m_pEngineForwardAnimation->ChangeState(m_wingVerticalStateId);
			}
		}
	}
}
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::Update(const float deltaTime)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
	...
	if (m_pEngineForwardAnimation)
	{
		if (pActor && pActor->IsClient())
			m_engineForwardAnimTimeInterp = m_engineForwardAnimTime;
		else
			Interpolate(m_engineForwardAnimTimeInterp, m_engineForwardAnimTime, 2.0f, deltaTime);

		m_pEngineForwardAnimation->SetTime(m_engineForwardAnimTimeInterp);
	}
	...
	if (m_pEnginesRotAnimation)
	{
		...
		float yawTrigger = m_turnAction;

		//if ai then use other condition to trigger when vehicle is rotating on yaw
		if (m_movementAction.isAI)
			yawTrigger = m_actionYaw;

		if (abs(yawTrigger) != 0)
		{
			if (m_actionYaw > 0.f)
				Interpolate(m_enginesRotAnimTime, right, abs(m_actionYaw), deltaTime);
			else if (m_actionYaw < 0.f)
				Interpolate(m_enginesRotAnimTime, left, abs(m_actionYaw), deltaTime);
		}
		else
			Interpolate(m_enginesRotAnimTime, 0.5f, 1.0f, deltaTime);

		//gEnv->pRenderer->Draw2dLabel(5.0f, 520.0f, 1.5f, color, false, "AlienGunship center %f", center);

		m_pEnginesRotAnimation->SetTime(m_enginesRotAnimTime);
	}
	...
	if (pActor && pActor->IsClient())
	{
		...
		//pRenderer->Draw2dLabel(5.0f, 420.0f, 1.5f, color, false, "m_pWingsAnimation->GetState():  %f", (int)m_pWingsAnimation->GetState());
		//pRenderer->Draw2dLabel(5.0f, 440.0f, 1.5f, color, false, "m_pWingsAnimation->GetState():  %f", (int)m_pWingsAnimation->GetState());
		//pRenderer->Draw2dLabel(5.0f, 460.0f, 1.5f, color, false, "m_pWingsAnimation->GetState():  %f", (int)m_pWingsAnimation->GetState());
	}
}

//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::UpdateEngine(float deltaTime)
{
...
//------------------------------------------------------------------------
float CVehicleMovementAlienGunship::GetDamageMult()
{
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::GetMemoryStatistics(ICrySizer* s)
{
	s->Add(*this);
	CVehicleMovementHelicopter::GetMemoryStatistics(s);
}

void CVehicleMovementAlienGunship::InitSoundNames(const char* eventGroupName)
{
	if (eventGroupName[0])
	{
		string prefix("sounds/vehicles/gunship:gunship");
...
//------------------------------------------------------------------------
void CVehicleMovementAlienGunship::Serialize(TSerialize ser, unsigned aspects)
{
	CVehicleMovementHelicopter::Serialize(ser, aspects);

	if ((ser.GetSerializationTarget() == eST_Network) && (aspects & eEA_GameClientDynamic))
	{
		ser.Value("wings", m_engineForwardAnimTime, 'unit');
	}
	else
	{
		ser.Value("horizontal", m_horizontal);
		ser.Value("wingsTime", m_engineForwardAnimTime);
