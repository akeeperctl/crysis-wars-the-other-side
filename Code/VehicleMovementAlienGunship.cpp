...
bool CVehicleMovementAlienGunship::Init(IVehicle* pVehicle, const SmartScriptTable& table)
{
	...
	m_pEngineForwardAnimation = nullptr;
...
void CVehicleMovementAlienGunship::PostInit()
{
	...
	//TODO Только одна анимация работает одновременно

	m_pEngineForwardAnimation = m_pVehicle->GetAnimation("enginesForward");
...
void CVehicleMovementAlienGunship::Update(const float deltaTime)
{
	...
	if (m_pEnginesRotAnimation)
	{
		...
		//float color[] = { 1,1,1,1 };
		//gEnv->pRenderer->Draw2dLabel(5.0f, 520.0f, 1.5f, color, false, "AlienGunship m_enginesRotAnimTime %f", m_enginesRotAnimTime);
