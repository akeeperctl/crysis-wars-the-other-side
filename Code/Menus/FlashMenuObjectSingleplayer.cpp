...
void CFlashMenuObject::UpdateConquerorLevels()
{
	m_pCurrentFlashMenuScreen->Invoke("Root.MainMenu.SinglePlayer.resetConquerorLevels");
	...
	if (pLevelSystem)
	{
		...
		for (int l = 0; l < pLevelSystem->GetLevelCount(); ++l)
		{
			...
			if (pLevelInfo)
			{
				...
				XmlNodeRef xmlInfo;
				GetLevelXmlInfo(pLevelInfo->GetName(), xmlInfo);

				if (file.find("Conquest") == 0)
				{
					SFlashVarValue args[2] = 
					{ 
						file.c_str(),
						display.empty() ? file.c_str() : display.c_str() 
					};

					if (m_pCurrentFlashMenuScreen->Invoke("_root.Root.MainMenu.SinglePlayer.addConquerorLevel", args, 2))
					{
						//CryLogAlways("addConquerorLevel %s", file);
					}
				}
			}
		}
