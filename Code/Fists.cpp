...
-------------------------------------------------------------------------
History:
- 26:04:2006   18:38 : Created by Mбrcio Martins
...
//------------------------------------------------------------------------
void CFists::Update(SEntityUpdateContext& ctx, const int slot)
{
...
//------------------------------------------------------------------------
void CFists::UpdateFPView(const float frameTime)
{
	CWeapon::UpdateFPView(frameTime);
}

//------------------------------------------------------------------------
void CFists::OnAction(const EntityId actorId, const ActionId& actionId, const int activationMode, const float value)
{
...
//------------------------------------------------------------------------
void CFists::Select(const bool select)
{
...
//------------------------------------------------------------------------
void CFists::EnterWater(const bool enter)
{
	m_underWater = enter;
}
...
void CFists::UpdateAnimState(const float frameTime)
{
	//Only if selected
...
void CFists::RequestAnimState(const EFistAnimState eFAS, bool force /*=false*/)
{
	//Only if selected
...
{
	EndRaiseWeaponAction(CFists* _fists)
		: fists(_fists) {};
	CFists* fists;

	void execute(CItem* _this) const
	{
...
void CFists::RaiseWeapon(bool raise, bool faster /*= false*/)
{
	...
	if (raise && (GetCurrentAnimState() == eFAS_RUNNING || GetCurrentAnimState() == eFAS_JUMPING) && !IsWeaponRaised())
	{
		...
		if (speedOverride > 0.0f)
			PlayAction(g_pItemStrings->raise, 0, false, eIPAF_Default, speedOverride);
		else
...
void CFists::CollisionFeeback(const Vec3& pos, const int eFAS) const
{
	...
	if (pEffect)
	{
		const Matrix34 tm = IParticleEffect::ParticleLoc(pos);
		pEffect->Spawn(true, tm);
	}
}

//---------------------------------------------------------------
bool CFists::OnActionAttack(const EntityId actorId, const ActionId& actionId, const int activationMode, const float value)
{
...
bool CFists::OnActionSpecial(const EntityId actorId, const ActionId& actionId, const int activationMode, const float value)
{
	CWeapon::OnAction(actorId, actionId, activationMode, value);
...
//------------------------------------------------------------------------
void CFists::NetStartMeleeAttack(const bool weaponMelee)
{
	RequestAnimState(eFAS_FIGHT);
...
//-----------------------------------------------------
tSoundID CFists::PlayAction(const ItemString& action, const int layer /* =0  */, const bool loop /* =false  */, const uint flags /* = eIPAF_Default  */, const float speedOverride /* = -1.0f  */)
{
	...
	if (action == g_pItemStrings->offhand_off && m_currentAnimState == eFAS_RELAXED)
