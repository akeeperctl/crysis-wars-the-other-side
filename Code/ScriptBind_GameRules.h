...
public:
	...
	// EMP Grenade
	int ProcessEMPEffect(IFunctionHandler* pH, ScriptHandle targetId, ScriptHandle shooterId, float timeScale);
	int PerformDeadHit(IFunctionHandler* pH);

	int IsItemAllowed(IFunctionHandler* pH, const char* itemName);

	//TheOtherSide
	int SetBeamControlPlayer(IFunctionHandler* pH, int value);
	int GetControlSystemEnabled(IFunctionHandler* pH);
	int GetControlActorId(IFunctionHandler* pH);

	int ChangeDesiredActor(IFunctionHandler* pH, ScriptHandle ownerId, ScriptHandle desiredActorId, bool reset);
	int StartControl(IFunctionHandler* pH, ScriptHandle ownerId, ScriptHandle slaveId, bool reset);
	int StopControl(IFunctionHandler* pH, ScriptHandle ownerId);

	int IsConquestGamemode(IFunctionHandler* pH);
	int GetConquestActors(IFunctionHandler* pH);
	int ConquestAddPoints(IFunctionHandler* pH, ScriptHandle playerid, int value);
	int GetConquestSpecies(IFunctionHandler* pH, ScriptHandle playerid);
	int SetEntityMaterial(IFunctionHandler* pH, int slot, ScriptHandle entityId, const char* materialName);
	//~TheOtherSide
...
private:
	...
	SmartScriptTable	m_conquerorplayers;
