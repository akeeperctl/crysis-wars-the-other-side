...
public:
	...
	//TheOtherSide
	virtual int CanLaunchShockwave(IFunctionHandler* pH);

	virtual int SetAlienEnergy(IFunctionHandler* pH, float energy);
	virtual int GetAlienEnergy(IFunctionHandler* pH);

	virtual int SetShieldEnergy(IFunctionHandler* pH, float energy);
	virtual int GetShieldEnergy(IFunctionHandler* pH);
	virtual int IsShieldProjected(IFunctionHandler* pH);

	virtual int IsHaveOwner(IFunctionHandler* pH);
	virtual int IsHaveSlave(IFunctionHandler* pH);
	virtual int IsLocalOwner(IFunctionHandler* pH);

	virtual int GetOwnerId(IFunctionHandler* pH);
	virtual int GetSlaveId(IFunctionHandler* pH);

	virtual int IsRageMode(IFunctionHandler* pH);

	virtual int IsHumanMode(IFunctionHandler* pH);
	virtual int IsEMP(IFunctionHandler* pH);
	virtual int GetProbablySearchLocation(IFunctionHandler* pH);

	//Squad System
	int GetSquadMembersCount(IFunctionHandler* pH);
	//int GetSquadEnabled(IFunctionHandler* pH);
	//~TheOtherSide
