...
int CScriptBind_GameRules::SetBeamControlPlayer(IFunctionHandler* pH, int value)
{
	...
	if (g_pControlSystem && g_pControlSystem->GetEnabled())
		g_pControlSystem->GetControlClient()->ToggleDudeBeam(value);
...
//int CScriptBind_GameRules::GetSquadEnabled(IFunctionHandler* pH)
//{
//	CGameRules* pGameRules = GetGameRules(pH);
//	if (!pGameRules)
//		return pH->EndFunction();
//
//	if (g_pControlSystem &&
//		g_pControlSystem->GetControlClient()->GetSquad() &&
//		g_pControlSystem->GetControlClient()->GetSquad()->IsEnabled())
//		return pH->EndFunction(1);
//	else
//		return pH->EndFunction();
//}
...
{
	//CryLogAlways("ProcessEMPEffect timescale %f", timeScale);
	...
	if (timeScale > 0.0f)
	{
		...
		if (pActor && (pActor->GetSpectatorMode() == 0) && (pActor->GetActorClass() == CPlayer::GetActorClassType()))
		{
			...
			{
				if (!pSuit->IsInvulnerable() && pPlayer->GetSlaveId() == 0)
				{
		...
		else if (pActor && pActor->IsAlien() && (pActor->GetSpectatorMode() == 0))
		{
			const float baseTime = 15.0f;
			float time = max(3.0f, baseTime * timeScale);
			...
			if (CPlayer* pPlayer = static_cast<CPlayer*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pAlien->GetOwnerId())))
			{
				pPlayer->GetGameObject()->InvokeRMI(CPlayer::ClEMP(), CPlayer::EMPParams(time / 2), eRMI_ToClientChannel, pPlayer->GetChannelId());
			}
		}
	}
