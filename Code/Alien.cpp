...
bool CAlien::Init(IGameObject* pGameObject)
{
	...
	CRY_ASSERT(pNetContext);

	if (pNetContext)
	{
		pNetContext->EnableAspects(GetEntityId(), eEA_GameClientDynamic | eEA_GameClientStatic | eEA_GameServerDynamic | eEA_GameServerStatic, true);
	}*/
...
void CAlien::PrePhysicsUpdate()
{
	...
	if (!m_stats.isRagDoll && GetHealth() > 0)
	{
		...
		// marcok: moved out
		Vec3 desiredMovement(m_pInput->posTarget - pEnt->GetWorldPos());
		...
		if (m_pInput->posTarget.len2() > 0.0f)
		{
			Vec3 desiredMovement(m_pInput->posTarget - pEnt->GetWorldPos());
		...
		if (m_pInput->dirTarget.len2() > 0.0f)
		{
			...
			Vec3 desiredDir = m_pInput->viewDir.IsZero() ?
				Vec3::CreateSlerp(m_pInput->movementVector.GetNormalizedSafe(m_pInput->dirTarget), m_pInput->dirTarget, t) :
				Vec3::CreateSlerp(m_pInput->viewDir, m_pInput->dirTarget, t);
		...
		if (m_linkStats.CanMove())
		{
			...
			else if (m_pInput->actions & ACTION_CROUCH)
				SetStance(STANCE_CROUCH);
			else if (m_pInput->actions & ACTION_PRONE)
				SetStance(STANCE_PRONE);
			else if (m_pInput->actions & ACTION_STEALTH)
...
void CAlien::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	if (m_empStats.isEmpState)
	{
		if (m_empStats.empTimer >= 0.01f)
		{
			Interpolate(m_empStats.empTimer, 0, 1.f, frameTime);
...
void CAlien::ProcessRotation(float frameTime)
{
	...
	//	if (m_stats.inAir && IsZeroG())
	{
		...
		if (m_pInput->viewVector.len2() > 0.0f)
		{
			//			m_eyeMtx.SetRotationVDir(m_input.viewVector.GetNormalizedSafe());
			Matrix33	eyeTarget;
			eyeTarget.SetRotationVDir(m_pInput->viewVector.GetNormalizedSafe());
		...
		if (m_pInput->viewVector.len2() > 0.0f)
		{
			...
			if (lookat.Dot(orient) < cosf(DEG2RAD(25.0f)) || m_forceOrient)
				m_viewMtx.SetRotationVDir(m_pInput->viewVector.GetNormalizedSafe());
		}
		else //if (m_input.deltaRotation.len2()>0.001f)
		{
			Ang3 desiredAngVel(m_pInput->deltaRotation.x * rotSpeed, 0, m_pInput->deltaRotation.z * rotSpeed);

			//rollage
			if (m_pInput->actions & ACTION_LEANLEFT)
				desiredAngVel.y -= 10.0f * rotSpeed;
			if (m_pInput->actions & ACTION_LEANRIGHT)
				desiredAngVel.y += 10.0f * rotSpeed;
...
void CAlien::GetMovementVector(Vec3& move, float& speed, float& maxSpeed)
{
	...
	move += column0 * m_pInput->deltaMovement.x * maxSpeed;
	move += column1 * m_pInput->deltaMovement.y * maxSpeed;
	move += column2 * m_pInput->deltaMovement.z * maxSpeed;
...
void CAlien::ProcessMovement(float frameTime)
{
	...
	//TheOtherSide
	if (IsHaveOwner())
		m_moveRequest.rotation = g_pControlSystem->GetControlClient()->GetNakedParams().modelQuat;
	else
...
void CAlien::ProcessMovement2(float frameTime)
{
	...
	//movement
	Vec3 move(m_pInput->movementVector);

	move += m_viewMtx.GetColumn(0) * m_pInput->deltaMovement.x;
	move += m_viewMtx.GetColumn(1) * m_pInput->deltaMovement.y;
	move += m_viewMtx.GetColumn(2) * m_pInput->deltaMovement.z;
...
void CAlien::Reset(bool toGame)
{
	...
	m_energyParams.alienEnergy = 0.0f;
	ResetAlienEnergy();
}
...
void CAlien::Kill()
{
	...
	//TheOtherSide
	IParticleEmitter* pEMPEmitter = GetEntity()->GetParticleEmitter(m_empStats.effectSlot);
	if (pEMPEmitter)
...
void CAlien::Revive(bool fromInit)
{
	...
	m_pInput->posTarget.zero();
	m_pInput->dirTarget.zero();
	m_pInput->deltaMovement.zero();
	...
	//emp effect
	m_empStats.isEmpState = 0;
	m_empStats.empTimer = 0;
	IParticleEmitter* pEmitter = GetEntity()->GetParticleEmitter(m_empStats.effectSlot);
	if (pEmitter)
	{
		pEmitter->Activate(false);
		gEnv->p3DEngine->DeleteParticleEmitter(pEmitter);
		GetEntity()->FreeSlot(m_empStats.effectSlot);
	}
	//~TheOtherSide
}
...
void CAlien::OnAction(const ActionId& actionId, int activationMode, float value)
{
	...
	{
		m_pInput->deltaRotation.z -= value;
		filterOut = false;
	}
	else if (actions.rotatepitch == actionId)
	{
		m_pInput->deltaRotation.x -= value;
		filterOut = false;
	}
	else if (actions.moveright == actionId)
	{
		m_pInput->deltaMovement.x += value;
		filterOut = false;
	}
	...
	{
		m_pInput->deltaMovement.y += value;
		filterOut = false;
	}
	...
	{
		m_pInput->actions |= ACTION_JUMP;
	}
	else if (actions.crouch == actionId)
	{
		m_pInput->actions |= ACTION_CROUCH;
	}
	else if (actions.prone == actionId)
	{
		if (!(m_pInput->actions & ACTION_PRONE))
			m_pInput->actions |= ACTION_PRONE;
		else
			m_pInput->actions &= ~ACTION_PRONE;
	}
	else if (actions.sprint == actionId)
	{
		m_pInput->actions |= ACTION_SPRINT;
	}
	else if (actions.leanleft == actionId)
	{
		m_pInput->actions |= ACTION_LEANLEFT;
	}
	else if (actions.leanright == actionId)
	{
		m_pInput->actions |= ACTION_LEANRIGHT;
	}
...
{
	m_pInput->movementVector = desiredSpeed;
}
...
void CAlien::SetActorMovementCommon(SMovementRequestParams& control)
{
	...
	if (!control.vAimTargetPos.IsZero())
		m_pInput->viewDir = (control.vAimTargetPos - state.weaponPosition).GetNormalizedSafe();
	else if (!control.vLookTargetPos.IsZero())
		m_pInput->viewDir = (control.vLookTargetPos - state.eyePosition).GetNormalizedSafe();
	else
		m_pInput->viewDir = GetEntity()->GetWorldRotation() * FORWARD_DIRECTION;
	m_pInput->pathLength = control.fDistanceToPathEnd;
	...
	if (control.bExactPositioning || bExactPos)
	{
		...
		if (nPoints && (nPoints == 1 || GetEntity()->GetWorldPos().GetSquaredDistance(control.remainingPath[0].vPos) < 2.5f))
		{
			...
			m_pInput->posTarget = control.remainingPath[0].vPos;
			m_pInput->dirTarget = control.remainingPath[0].vDir;
			// this used to be control.
			m_pInput->speedTarget = 10.0f;
			exactActive = true;
		}
	}
...
{
	m_pInput->deltaMovement = movement;
}
...
{
	if (gEnv->bServer && (g_pGameCVars->sv_input_timeout > 0) && ((gEnv->pTimer->GetFrameStartTime() - m_pInput->m_lastUpdate).GetMilliSeconds() >= g_pGameCVars->sv_input_timeout))
	{
		m_pInput->ResetDeltas();
...
void CAlien::UpdateEnergyRecharge(float frametime)
{
	...
	NETINPUT_TRACE(GetEntityId(), m_energyParams.alienEnergy);
	NETINPUT_TRACE(GetEntityId(), recharge);

	if (gEnv->bServer)
	{
		if (GetHealth() != 0 && !m_energyParams.cannotRegen)
		{
			if (recharge < 0.0f || m_energyParams.alienEnergyRechargeDelay <= 0.0f)
			{
				SetAlienEnergy(clamp(m_energyParams.alienEnergy + recharge * frametime, 0.0f, ALIEN_ENERGY));
			}
		}
	}
...
void CAlien::DoSetState(const SSerializedAlienInput& input)
{
	//GetGameObject()->ChangedNetworkState(SControlClient::INPUT_ASPECT);
	...
	m_pInput->actions = input.actions;
	...
	m_pMovementController->RequestMovement(moveRequest);

	//GetGameObject()->ChangedNetworkState(SControlClient::INPUT_ASPECT);
}

void CAlien::SetState(const SSerializedAlienInput& input)
{
	DoSetState(input);

	//It doesn't matter, you can exclude this line
	m_pInput->m_lastUpdate = gEnv->pTimer->GetCurrTime();
}

//void CAlien::GetState(SSerializedAlienInput& input)
//{
//	SMovementState state;
//	GetMovementController()->GetMovementState(state);
//
//	//Get look dir from view matrix
//	input.actions =	m_pInput->actions;
//	input.deltaMovement = m_vNetDeltaMovement;
//	input.lookDirection = m_viewMtx.GetColumn1().GetNormalized();
...
IMPLEMENT_RMI(CAlien, SvRequestSetMove)
{
	if (gEnv->bServer)
	{
		//CRY_ASSERT(input.deltaMovement.IsValid());
		//CRY_ASSERT(input.lookDirection.IsValid());
		//CRY_ASSERT(input.position.IsValid());

		GetGameObject()->InvokeRMI(ClSetMove(), params, eRMI_ToAllClients);
	}
	return true;
}

IMPLEMENT_RMI(CAlien, ClSetMove)
{
	SSerializedAlienInput input;
	input.deltaMovement = params.deltaMovement;
	input.lookDirection = params.lookDirection;
...
void CAlien::EnableEmpState(bool emp, float duration, CAlien* pAlien)
{
	...
	if (emp && duration > 0.01f)// Enable EMP state
	{
		...
		const bool isTrooper = className == "PlayerTrooper" || className == "Trooper";
		...
		m_empStats.oldRot = pAlien->GetEntity()->GetRotation();
		...
		if (pEffect)
		{
			...
			if (!pAlien->GetEntity()->GetParticleEmitter(m_empStats.effectSlot))
				m_empStats.effectSlot = pAlien->GetEntity()->LoadParticleEmitter(-1, pEffect, &sp);

			IParticleEmitter* pEmitter = pAlien->GetEntity()->GetParticleEmitter(m_empStats.effectSlot);
			if (pEmitter)
			{
		...
		m_empStats.isEmpState = emp;
		m_empStats.empTimer = duration;
	}
	else if (!emp || duration == 0) //Disable EMP state
	{
		if (m_empStats.isEmpState)
		{
			m_empStats.isEmpState = false;
			...
			if (pEmitter)
			{
				pEmitter->Activate(false);
				gEnv->p3DEngine->DeleteParticleEmitter(pEmitter);
				pAlien->GetEntity()->FreeSlot(m_empStats.effectSlot);
			}
			...
			if (isTrooper && pAlien->GetHealth() > 0.1f)
			{
				...
				if (pTrooper->m_shieldParams.shieldType != CTrooper::eShieldType_Guardian)
				{
					...
					pAlien->GetEntity()->SetRotation(m_empStats.oldRot);
				}
			}
...
{
	energyValue = clamp(energyValue, 0.0f, ALIEN_ENERGY);
	if (energyValue != m_energyParams.alienEnergy && gEnv->bServer)
		GetGameObject()->ChangedNetworkState(ASPECT_ALIEN_ENERGY); //CPlayer::ASPECT_NANO_SUIT_ENERGY

	if (!gEnv->bMultiplayer)
	{
		if (energyValue < m_energyParams.alienEnergy)
		{
	...
	if (energyValue < m_energyParams.alienEnergy)
	{
		if (gEnv->bMultiplayer && ((energyValue / ALIEN_ENERGY) <= 0.2f) && (m_energyParams.alienEnergy > energyValue) && g_pGameCVars->g_mpSpeedRechargeDelay) // if we cross the 20% boundary we don't regenerate for 3secs
			m_energyParams.alienEnergyRechargeDelay = 3.0f;
	}

	m_energyParams.alienEnergy = energyValue;
}

void CAlien::ResetAlienEnergy()
{
	SetAlienEnergy(ALIEN_ENERGY);
	m_energyParams.alienEnergy = ALIEN_ENERGY;

	if (gEnv->bServer)
		GetGameObject()->ChangedNetworkState(ASPECT_ALIEN_ENERGY);
}