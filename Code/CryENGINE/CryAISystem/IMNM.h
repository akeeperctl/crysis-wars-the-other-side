...
namespace MNM
{
	...
	{
		GlobalIslandID(const uint64 defaultValue = Constants::eGlobalIsland_InvalidIslandID)
			: id(defaultValue)
		{
			STATIC_ASSERT(sizeof(StaticIslandID) <= 4, "The maximum supported size for StaticIslandIDs is 4 bytes.");
		}

		GlobalIslandID(const uint32 navigationMeshID, const StaticIslandID islandID)
		{
			id = (static_cast<uint64>(navigationMeshID) << 32) | islandID;
		}
		...
		bool operator==(const GlobalIslandID& rhs) const
		{
			return id == rhs.id;
		}
		...
		StaticIslandID GetStaticIslandID() const
		{
			return static_cast<StaticIslandID>(id & (static_cast<uint64>(1) << 32) - 1);
		}
	...
	public:
		...
		{
			return (m_linkType == LinkDataClass::GetType()) ? static_cast<LinkDataClass*>(this) : nullptr;
		}

		template<class LinkDataClass>
		const LinkDataClass* CastTo() const
		{
			return (m_linkType == LinkDataClass::GetType()) ? static_cast<const LinkDataClass*>(this) : nullptr;
		}
