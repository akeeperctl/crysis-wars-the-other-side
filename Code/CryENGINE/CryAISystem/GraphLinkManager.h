...
private:
	...
	void MarkBucketForDeletion(unsigned bucketIndex) const;
...
inline void CGraphLinkManager::MarkBucketForDeletion(unsigned bucketIndex) const
{
	GraphLinkBidirectionalData* slotData = m_buckets[bucketIndex];
	for (unsigned slot = 0; slot < BUCKET_SIZE; ++slot, ++slotData)
...
inline void CGraphLinkManager::CollectGarbage()
{
	...
	for (int i = 0, count = static_cast<int>(m_buckets.size()); i < count; ++i)
	{
	...
	for (int i = 0, count = static_cast<int>(bucketsToDelete.size()); i < count; ++i)
	{
		delete[] m_buckets[bucketsToDelete[i]];
		m_buckets[bucketsToDelete[i]] = nullptr;
	}
}
