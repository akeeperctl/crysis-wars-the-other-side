...
//
//--------------------------------------------------------------------------------------------------
int CGraph::Rearrange(ListNodes& nodesList, const Vec3& cutStart, const Vec3& cutEnd)
{
	int                 ndNumber = 0;

	for (auto it = nodesList.begin(); it != nodesList.end(); ++it, ndNumber++)
	{
		int        counter = 0;
	...
	const int counter = ProcessMegaMerge(nodesList, cutStart, cutEnd);
	return counter;
}

//
//--------------------------------------------------------------------------------------------------
int CGraph::ProcessMegaMerge(ListNodes& nodesList, const Vec3& vCutStart, const Vec3& vCutEnd)
{
	ListNodes  leftSideNodes;
	ListNodes  rightSideNodes;
	constexpr int  counter = 0;
	const Vec3 cutDirection = vCutEnd - vCutStart;
	...
	Vec3 cutN = cutDirection.normalized();
	...
	for (it = nodesList.begin(); it != nodesList.end(); ++it)
	{
		...
		if (std::find(rightSideNodes.begin(), rightSideNodes.end(), curNode) != rightSideNodes.end()) // already in list - skip
			continue;
		if ((curNode->vertex[0] == cutStart) || (curNode->vertex[1] == cutStart) || (curNode->vertex[2] == cutStart) || (curNode->vertex[0] == cutEnd) || (curNode->vertex[1] == cutEnd) || (curNode->vertex[2] == cutEnd))
		...
		{
			for (auto li = rightSideNodes.begin(); li != rightSideNodes.end() && !bHasPointOnEdge; ++li)
			{
		...
		const int otherVertexIdx1 = 2 - vIdx;
		const int otherVertexIdx2 = 3 - (otherVertexIdx1 + vIdx);

		Vec3 theOtherVertex = m_pAISystem->m_VertexList.GetVertex(curNode->vertex[otherVertexIdx1]).vPos - vCutStart;
		...
		theOtherVertex.normalize();
		const float cross2 = cutN.x * theOtherVertex.y - cutN.y * theOtherVertex.x;
		...
		/*
					Vec3	theOtherVertex = m_pAISystem->m_VertexList.GetVertex(
	...
	for (it = leftSideNodes.begin(); !leftSideNodes.empty() && it != leftSideNodes.end(); ++it)
	{
		...
		Disconnect(curNode);
		const ListNodes::iterator itList = std::find(nodesList.begin(), nodesList.end(), curNode);
	...
	for (it = nodesList.begin(); it != nodesList.end(); ++it)
	{
		...
		for (vi = curNode->link.begin(); vi != curNode->link.end(); vi++)
		{
			if ((*vi).fMaxRadius < 0.0f ||
				...
				//				)
				(curNode->vertex[(*vi).nStartIndex] == cutStart && curNode->vertex[(*vi).nEndIndex] == cutEnd) || (curNode->vertex[(*vi).nEndIndex] == cutStart && curNode->vertex[(*vi).nStartIndex] == cutEnd))
...
bool CGraph::CreateOutline(const ListNodes& insideNodes, ListNodes& nodesList, ObstacleIndexList& outline)
{
	ListNodes::iterator     it;
	VectorOfLinks::iterator vi;
	GraphNode*              curNode;
		...
		for (vi = curNode->link.begin(); vi != curNode->link.end(); vi++)
		{
			if ((*vi).IsNewLink()) // it's on the edge - it's a new node
				if ((curNode->vertex[(*vi).nStartIndex] == (*outline.begin())) || (curNode->vertex[(*vi).nEndIndex] == (*outline.begin())))
				{
					...
					//					Vec3	theOtherVertex = m_pAISystem->m_VertexList.GetVertex(theOtherVertexIdx).vPos;
...
void CGraph::TriangulateOutline(ListNodes& nodesList, ObstacleIndexList& outline, bool orientation)
{
	...
	while (outline.size() > 3)
	{
		...
		auto  candidateRemove = outline.end();
		float candidateDegeneracy = -1.0f;

		candidateNode = CreateNewNode();

		int cntr = 1;
		for (auto curItr = outline.begin(); curItr != outline.end(); ++curItr, cntr++)
		{
			auto vItr1 = curItr;
			auto vItr2 = curItr;
			...
			for (auto tmpItr = originalOutline.begin(); tmpItr != originalOutline.end() && !notGood; ++tmpItr)
			{
				if (
						//					IsEquivalent()
				...
				if (PointInTriangle(m_pAISystem->m_VertexList.GetVertex(*tmpItr).vPos, &tmpNode))
					notGood = true;
			}
			...
			const float dVal = tmpNode.GetDegeneracyValue();
...
//
//--------------------------------------------------------------------------------------------------
int CGraph::ProcessRearrange(ListNodes& nodesList, const Vec3& cutStart, const Vec3& cutEnd)
{
	return 0;
	/*
	int counter = 0;
...
bool CGraph::ProcessRearrange(GraphNode* node1, GraphNode* node2, ListNodes& nodesList)
{
	...
	VectorOfLinks::iterator vi;	
		...
		Vec3	newEdge = node1->vertex[other1VertexIdx].vPos - node2->vertex[other2VertexIdx].vPos;
		Vec3	v1 = node1->vertex[(*vi).nStartIndex].vPos - node2->vertex[other2VertexIdx].vPos;
		Vec3	v2 = node1->vertex[(*vi).nEndIndex].vPos - node2->vertex[other2VertexIdx].vPos;
...
float GraphNode::GetDegeneracyValue()
{
	...
	Vec3 firstE = pVList->GetVertex(vertex[1]).vPos - pVList->GetVertex(vertex[0]).vPos;
	Vec3 secondE = pVList->GetVertex(vertex[2]).vPos - pVList->GetVertex(vertex[0]).vPos;
...
void GraphNode::MakeAntiClockwise()
{
	...
	Vec3 one = pVList->GetVertex(od2).vPos - pVList->GetVertex(od1).vPos;
	Vec3 two = pVList->GetVertex(od3).vPos - pVList->GetVertex(od2).vPos;
...
bool GraphNode::IsAntiClockwise()
{
	...
	const int od1 = vertex[0];
	const int od2 = vertex[1];
	const int od3 = vertex[2];
...
//
//--------------------------------------------------------------------------------------------------
float GraphNode::GetCross(const Vec3& vCutStart, const Vec3& vDir, const GraphLink& theLink)
{
	Vec3 theOtherVertex = GetAISystem()->m_VertexList.GetVertex(vertex[3 - (theLink.nStartIndex + theLink.nEndIndex)]).vPos - vCutStart;
	theOtherVertex.normalize();
