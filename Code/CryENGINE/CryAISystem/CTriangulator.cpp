...
int CTriangulator::AddVertex(float x, float y, float z, void* pForeign, bool bImportant)
{
	...
	const VARRAY::iterator i = std::find(m_vVertices.begin(), m_vVertices.end(), newvertex);
...
bool CTriangulator::IsAntiClockwise(const Tri* who) const
{
	const Vtx v1 = m_vProcessed[who->v[0]];
	const Vtx v2 = m_vProcessed[who->v[1]];
	const Vtx v3 = m_vProcessed[who->v[2]];
	...
	const float f = vec1.x * vec2.y - vec2.x * vec1.y;
...
bool CTriangulator::Calculate(Tri* pTri) const
{
...
bool CTriangulator::IsPerpendicular(const Vtx& v1, const Vtx& v2, const Vtx& v3) const
{
	const double yDelta_a = v2.y - v1.y;
	const double xDelta_a = v2.x - v1.x;
	const double yDelta_b = v3.y - v2.y;
	const double xDelta_b = v3.x - v2.x;
	...
	if (fabs(yDelta_a) <= 0.0000001)
		return true;
	if (fabs(yDelta_b) <= 0.0000001)
		return true;
	if (fabs(xDelta_a) <= 0.000000001)
		return true;
	if (fabs(xDelta_b) <= 0.000000001)
		return true;
	return false;
}

void CTriangulator::CalcCircle(const Vtx& v1, const Vtx& v2, const Vtx& v3, Tri* pTri) const
{
	const double yDelta_a = v2.y - v1.y;
	const double xDelta_a = v2.x - v1.x;
	const double yDelta_b = v3.y - v2.y;
	...
	// calc center
	pTri->center.x = static_cast<float>((aSlope * bSlope * (v1.y - v3.y) + bSlope * (v1.x + v2.x) - aSlope * (v2.x + v3.x)) / (2.f * (bSlope - aSlope)));
	pTri->center.y = static_cast<float>(-(pTri->center.x - (v1.x + v2.x) / 2.f) / aSlope + (v1.y + v2.y) / 2.f);
	pTri->center.z = v1.z;
...
bool CTriangulator::PrepForTriangulation(void)
{
	...
	// bounding rectangle
	for (const auto& m_vVertice : m_vVertices)
	{
		const Vtx current = m_vVertice;
		if (current.x < min.x)
			min.x = current.x;
		if (current.y < min.y)
			min.y = current.y;
	...
	*/
	...
	const float xsize = max.x - min.x;
	const float ysize = max.y - min.y;
...
void CTriangulator::FinalizeTriangulation(void)
{
	...
	for (auto dr = m_vTriangles.begin(); dr != m_vTriangles.end();)
	{
		const Tri* now = (*dr);
...
bool CTriangulator::TriangulateNew(void)
{
	...
	for (const auto& m_vVertice : m_vVertices)
	{
		...
		{
			const Tri* triangle = (*ti);

			const float dist = (current.x - triangle->center.x) * (current.x - triangle->center.x) + (current.y - triangle->center.y) * (current.y - triangle->center.y);
		...
		// add new triangles	
		const int pos = m_vProcessed.size();
