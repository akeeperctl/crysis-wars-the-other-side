...
// CNavPath
//====================================================================
class CNavPath final : public INavPath
{
public:
	CNavPath();
	~CNavPath() override;
	...
	void SetVersion(int v) override
	{
		m_version.v = v;
	}

	/// The original parameters used to generate the path will be stored
	void SetParams(const SNavPathParams& params) override
	{
		m_params = params;
	}

	const SNavPathParams& GetParams() const override
	{
		return m_params;
	}

	SNavPathParams& GetParams() override
	{
		return m_params;
	}
	...
	float GetPathLength(bool b2D) const override;
	...
	bool GetPathEndIsAsRequested() const override
	{
		return m_pathEndIsAsRequested;
	}

	void SetPathEndIsAsRequested(const bool val) override
	{
		m_pathEndIsAsRequested = val;
	}
	...
	const PathPointDescriptor* GetLastPathPoint() const override
	{
		return m_pathPoints.empty() ? nullptr : &m_pathPoints.back();
	}

	/// previous path point - returns 0 if not available
	const PathPointDescriptor* GetPrevPathPoint() const override
	{
		return m_pathPoints.empty() ? nullptr : &m_pathPoints.front();
	}

	/// next path point - returns 0 if not available
	const PathPointDescriptor* GetNextPathPoint() const override
	{
		return m_pathPoints.size() > 1 ? &(*(++m_pathPoints.begin())) : nullptr;
	}

	/// next but 1 path point - returns 0 if not available
	const PathPointDescriptor* GetNextNextPathPoint() const override
	{
		return m_pathPoints.size() > 2 ? &(*(++(++m_pathPoints.begin()))) : nullptr;
	}

	/// Gets the position of next path point. Returns defaultPos if the path is empty
	const Vec3& GetNextPathPos(const Vec3& defaultPos = Vec3Constants<float>::fVec3_Zero) const override
	{
		const PathPointDescriptor* ppd = GetNextPathPoint();
		return ppd ? ppd->vPos : defaultPos;
	}

	/// Gets the end of the path. Returns defaultPos if the path is empty
	const Vec3& GetLastPathPos(const Vec3& defaultPos = Vec3Constants<float>::fVec3_Zero) const override
	{
		const PathPointDescriptor* ppd = GetLastPathPoint();
		return ppd ? ppd->vPos : defaultPos;
	}
	...
	bool GetPosAlongPath(Vec3& posOut, float dist = 0.f, bool b2D = false, bool bExtrapolateBeyondEnd = false, IAISystem::ENavigationType* pNextPointType = nullptr) const override;
	...
	const TPathPoints& GetPath() const override
	{
		return m_pathPoints;
	}

	void SetPathPoints(const TPathPoints& points) override
	{
		m_pathPoints = points;
		++m_version.v;
	}
	...
	/// well.
	bool GetPathPropertiesAhead(float distAhead, bool twoD, Vec3& posOut, Vec3& dirOut, float* invROut, float& lowestPathDotOut, bool scaleOutputWithDist) const override;

	/// Serialise the current request
	void Serialize(TSerialize ser);

	const Vec3& GetEndDir() const override
	{
		return m_endDir;
	}

	void SetEndDir(const Vec3& endDir) override
	{
		m_endDir = endDir;
	}
	...
	bool UpdateAndSteerAlongPath(Vec3& dirOut, float& distToEndOut, float& distToPathOut, bool& isResolvingSticking, Vec3& pathDirOut, Vec3& pathAheadDirOut, Vec3& pathAheadPosOut, Vec3 currentPos, const Vec3& currentVel, float lookAhead, float pathRadius, float dt, bool resolveSticking, bool twoD) override;
	...
	float GetDiscardedPathLength() const override
	{
		return m_fDiscardedPathLength;
	}
...
private:
	...
	void CopyTo(INavPath* pRecipient) const override
	{
		*dynamic_cast<CNavPath*>(pRecipient) = *this;
	}

	INavPath* Clone() const override
	{
	...
	struct SVersion
	{
		...
		void operator=(const SVersion& other)
		{
			v = other.v + 1;
		}
