...
constexpr auto PUPPETSTATE_NONE = 0;
constexpr auto PUPPETSTATE_ATTACKING = 1;
constexpr auto PUPPETSTATE_RETREATING = 2;
constexpr auto PUPPETSTATE_IDLING = 3;
constexpr auto PUPPETSTATE_WANDERING = 4;
...
public:
	...
	bool PointAudible(const Vec3& pos, float fRadius);
	void Forget(CAIObject* pDummyObject) override;
	void OnObjectRemoved(CAIObject* pObject) override;
	bool m_bDryUpdate{};
	void RequestPathTo(const Vec3& pos) override;
	void Navigate(CAIObject* pTarget) override;
	bool PointVisible(const Vec3& pos);
	...
	void Devalue(CAIObject* pObject, bool bDevaluePuppets) override;
...
public:
	...
	void CreateFormation(const char* szName) override;
	void Reset(void) override;

	bool ReleaseFormation() override;
	// removes a goal from the active goals and reshuffles the active goals
	//void RemoveActiveGoal(int nOrder);
	Vec3 GetIndoorHidePoint(int nMethod, float fSearchDistance, bool bSameOK);
...
public:
	...
	Vec3 FindHidePoint(float fSearchDistance, int nMethod, bool bIndoor = false, bool bSameOk = false) override;
	...
	Vec3 GetOutdoorHidePoint(int nMethod, float fSearchDistance, bool bSameOk);
	...
	void Load_PATCH_1(CStream& stm) override;
};
