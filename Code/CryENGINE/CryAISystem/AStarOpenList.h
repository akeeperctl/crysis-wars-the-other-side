...
public:
	OpenListMonitor(const NodeInfoMap& mNodeInfoMap, float sMin, float sMax, float sAvg, int sNumSamples, float sMinFrames, float sMaxFrames, float sAvgFrames)
		: m_nodeInfoMap(mNodeInfoMap),
		sMin(sMin),
		sMax(sMax),
		sAvg(sAvg),
		sNumSamples(sNumSamples),
		sMinFrames(sMinFrames),
		sMaxFrames(sMaxFrames),
		sAvgFrames(sAvgFrames) {}
...
private:
	...
	void UpdateStats(float t, int frames)
	{
		...
		if (t < sMinFrames)
			sMinFrames = static_cast<float>(frames);
		if (t > sMaxFrames)
			sMaxFrames = static_cast<float>(frames);
...
public:
	...
	AStarSearchNode* GetAStarNode(uint32 index)
	{
		...
		if (it.second)
		{
			auto& node = const_cast<AStarSearchNode&>(*it.first);
