...
public:
	...
	~CPathObstacle() override;
	void           Free();
	CPathObstacle& operator=(const CPathObstacle& other);
	CPathObstacle(const CPathObstacle& other);

	EPathObstacleType GetType() const
	{
		return m_type;
	}

	SPathObstacleShape2D& GetShape2D()
	{
		AIAssert(m_type == ePOT_Shape2D);
		return *static_cast<SPathObstacleShape2D*>(m_pData);
	}

	const SPathObstacleShape2D& GetShape2D() const
	{
		AIAssert(m_type == ePOT_Shape2D);
		return *static_cast<SPathObstacleShape2D*>(m_pData);
	}

	SPathObstacleCircle2D& GetCircle2D()
	{
		AIAssert(m_type == ePOT_Circle2D);
		return *static_cast<SPathObstacleCircle2D*>(m_pData);
	}

	const SPathObstacleCircle2D& GetCircle2D() const
	{
		AIAssert(m_type == ePOT_Circle2D);
		return *static_cast<SPathObstacleCircle2D*>(m_pData);
	}
...
public:
	...
	/// Gets the result
	const TPathObstacles& GetCombinedObstacles() const
	{
		return m_combinedObstacles;
	}
	...
	virtual bool IsPathIntersectingObstacles(NavigationMeshID meshID, const Vec3& start, const Vec3& end, float radius) const override;
...
private:
	...
	struct SPathObstaclesInfo
	{
		...
		SPathObstaclesInfo(TPathObstacles& _obstacles, const AgentMovementAbility& _movementAbility, const CNavPath* _pNavPath, const CAIActor* _pAIActor, float _maxPathDeviation)
			: minAvRadius(0.0f),
			maxSpeed(0.0f),
	...
	static constexpr int               s_maxDynamicObstacleFlags = 256;
