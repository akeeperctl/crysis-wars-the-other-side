п»ї// Copyright 2001-2016 Crytek GmbH / Crytek Group. All rights reserved.
...
/*************************************************************************
   ...
   -------------------------------------------------------------------------
   History:
   - 15:1:2009   18:01 : Created by MпїЅrcio Martins
...
struct NavigationBlocker
{
	...
	NavigationBlocker()
		...
		costMultMod(0)
	{
		assert("Should never get called");
	}
...
struct PathPointDescriptor
{
	...
	explicit PathPointDescriptor(IAISystem::ENavigationType _navType = IAISystem::NAV_UNSET, const Vec3& _vPos = ZERO)
		: vPos(_vPos),
		navType(_navType),
		navTypeCustomId(0),
		iTriId(0),
	...
	explicit PathPointDescriptor(const Vec3& _vPos)
		: vPos(_vPos),
...
struct PathfindRequest
{
	...
	explicit PathfindRequest(ERequestType type)
		: type(type),
		startIndex(0),
		endIndex(0),
		bSuccess(false),
...
{
	PathfindingHeuristicProperties(const AgentPathfindingProperties& properties, const IAIPathAgent* pAgent = nullptr)
...
{
	SNavPathParams(const Vec3& start = Vec3Constants<float>::fVec3_Zero, const Vec3& end = Vec3Constants<float>::fVec3_Zero, const Vec3& startDir = Vec3Constants<float>::fVec3_Zero, const Vec3& endDir = Vec3Constants<float>::fVec3_Zero, int nForceBuildingID = -1, bool allowDangerousDestination = false, float endDistance = 0.0f, bool continueMovingAtEnd = false, bool isDirectional = false)
		: start(start),
		end(end),
		startDir(startDir),
		endDir(endDir),
...
public:
	...
	virtual bool                                       GetPosAlongPath(Vec3& posOut, float dist, bool twoD, bool extrapolateBeyondEnd, IAISystem::ENavigationType* nextPointType = nullptr) const = 0;
	...
	bool ArePathsEqual(const INavPath& otherNavPath) const
	{
		...
		const TPathPoints::const_iterator path1It = path1.begin();
		const TPathPoints::const_iterator path1End = path1.end();
		const TPathPoints::const_iterator path2It = path2.begin();

		typedef std::pair<TPathPoints::const_iterator, TPathPoints::const_iterator> TMismatchResult;

		const TMismatchResult result = std::mismatch(path1It, path1End, path2It, PathPointDescriptor::ArePointsEquivalent);
...
struct IPathObstacles
{
	...
	virtual bool IsPathIntersectingObstacles(NavigationMeshID meshID, const Vec3& start, const Vec3& end, float radius) const = 0;
...
public:
	...
	virtual bool CheckWalkability(const Vec2* path, size_t length) const = 0;
	...
	virtual void SetAllowCuttingCorners(bool allowCuttingCorners) = 0;
	...
	virtual bool IsRemainingPathAffectedByNavMeshChange(NavigationMeshID affectedMeshID, MNM::TileID affectedTileID) const = 0;
	// </interfuscator:shuffle>
};
...
public:
	...
	virtual int  RequestRawPathTo(const Vec3& start, const Vec3& end, float passRadius, IAISystem::tNavCapMask navCapMask, unsigned& lastNavNode, bool allowDangerousDestination, float endTol, const PathfindingExtraConstraints& constraints, IAIPathAgent* pReference = nullptr) = 0;
