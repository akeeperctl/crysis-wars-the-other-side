...
bool CCompatibilityAlienMovementController::RequestMovement(CMovementRequest& request)
{
	...
	//vLookTargetPos, vAimTargetPos, vShootTargetPos, fDistanceToPathEnd, fDesiredSpeed and stance getting to "os" from "request"
	...
	if (request.HasMoveTarget())
	{
		...
		if (distanceToEnd > 0.001f)
			os.fDistanceToPathEnd = distanceToEnd;*/
	}
	...
	//Dont work because aliens dont use this
	/*if (request.HasDeltaMovement())
		m_currentMovementRequest.AddDeltaMovement(request.GetDeltaMovement());

	if (request.HasDeltaRotation())
		m_currentMovementRequest.AddDeltaRotation(request.GetDeltaRotation());*/
