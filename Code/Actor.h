/*************************************************************************
 ...
 -------------------------------------------------------------------------
  History:
  - 7:10:2004   14:46 : Created by M�rcio Martins
...
#define SER_VALUE(valName) ser.Value(#valName, valName)
#define SER_VALUE_ENUM(valName, nfirst, nlast) ser.EnumValue(#valName, valName, nfirst, nlast)
#define SER_VALUE_TYPE(funcName,serType) ser.Value(#funcName, funcName, #serType)

static const float ALIEN_MAX_ENERGY = 200.0f;
...
struct SActorParams
{
	...
	Vec3	vLimitDir;
	float	vLimitRangeH;//horizont
	float	vLimitRangeV;
	float vLimitRangeVUp;//up limit form center
	float vLimitRangeVDown;//down limit form center
...
struct STOSVehicleStats
{
	...
	void Serialize(TSerialize ser)
	{
		...
		{
			ser.Value("lastOperatedVehicleId", lastOperatedVehicleId);
			ser.Value("lastTimeEnter", lastTimeEnter);
			ser.Value("lastTimeExit", lastTimeExit);
		}
	}
};

struct STOSGrabStats
{
	STOSGrabStats()
	{
		reset();
	...
	std::vector<EntityId> grabbedIds;
	EntityId grabberId;
	bool isGrabbed;

	float lastTimeGetDropped;
	float lastTimeGetGrabbed;
	...
	void reset()
	{
		...
		lastTimeGetDropped = 0;
		lastTimeGetGrabbed = 0;
	...
	void Serialize(TSerialize ser)
	{
		if (ser.GetSerializationTarget() != eST_Network)
		{
			ser.Value("grabberId", grabberId);
			ser.Value("isGrabbed", isGrabbed);
			ser.Value("lastTimeGetDropped", lastTimeGetDropped);
			ser.Value("lastTimeGetGrabbed", lastTimeGetGrabbed);
			ser.Value("lastTimeGrab", lastTimeGrab);
			ser.Value("lastTimeDrop", lastTimeDrop);
			ser.Value("grabbedIds", grabbedIds);
		}
	}

	const bool IsGrabbedBy(EntityId id) const
	{
		return isGrabbed && grabberId == id;
	}
};
//~TheOtherSide
...
public:
	...
	struct PlayActionParams
	{
		...
		PlayActionParams() {};
		PlayActionParams(string _animName, string _typeName)
		{
			animName = _animName;
			typeName = _typeName;
		}
		...
		void SerializeWith(TSerialize ser)
		{
			ser.Value("playanim_animName", animName, 'stab');
			ser.Value("playanim_typeName", typeName, 'stab');
		}
	};

	struct PlayerHideParams
	{
		PlayerHideParams() : bHide(0) {};
		PlayerHideParams(bool _bHide) : bHide(_bHide) {};
		bool bHide;
		void SerializeWith(TSerialize ser)
		{
			ser.Value("hide", bHide, 'bool');
		}
	};

	struct SNetSpawnParticleParams
	{
		string effectName;
		Vec3 pos;
		bool bIndependent;

		SNetSpawnParticleParams() {};
		SNetSpawnParticleParams(bool _bIndependent, Vec3 _pos, string _effectName)
		{
			bIndependent = _bIndependent;
			pos = _pos;
		...
		void SerializeWith(TSerialize ser)
		{
			ser.Value("netparticle_effectName", effectName, 'stab');
			ser.Value("netparticle_position", pos, 'wrld');
			ser.Value("netparticle_bIndependent", bIndependent, 'bool');
		}
	};

	struct SNetCamShakeParams
	{
		float angle;
		float shift; 
		float duration;
		...
		SNetCamShakeParams() {};
		SNetCamShakeParams(float _angle, float _shift, float _duration, float _frequency, Vec3 _pos)
		{
			angle = _angle;
			shift = _shift;
			duration = _duration;
		...
		void SerializeWith(TSerialize ser)
		{
			ser.Value("angle", angle, 'iii');
			ser.Value("shift", shift, 'tod');
			ser.Value("duration", duration, 'tod');
			ser.Value("frequency", frequency, 'tod');
			ser.Value("pos", pos, 'wrld');
		}
	};

	DECLARE_CLIENT_RMI_NOATTACH(ClCameraShake, SNetCamShakeParams, eNRT_ReliableOrdered);
	DECLARE_SERVER_RMI_NOATTACH(SvRequestCameraShake, SNetCamShakeParams, eNRT_ReliableOrdered);

	DECLARE_CLIENT_RMI_NOATTACH(ClSpawnEffect, SNetSpawnParticleParams, eNRT_ReliableOrdered);
	DECLARE_SERVER_RMI_NOATTACH(SvRequestSpawnEffect, SNetSpawnParticleParams, eNRT_ReliableOrdered);

	DECLARE_CLIENT_RMI_NOATTACH(ClPlayAction, PlayActionParams, eNRT_ReliableOrdered);
	DECLARE_SERVER_RMI_NOATTACH(SvRequestPlayAction, PlayActionParams, eNRT_ReliableOrdered);
	//DECLARE_CLIENT_RMI_PREATTACH(ClBla, SetSpectatorHealthParams, eNRT_ReliableUnordered);
	DECLARE_CLIENT_RMI_PREATTACH(ClHidePlayer, PlayerHideParams, eNRT_ReliableOrdered);
	DECLARE_SERVER_RMI_PREATTACH(SvRequestHidePlayer, PlayerHideParams, eNRT_ReliableOrdered);
	//TheOtherSide
...
protected:
	...
	//TheOtherSide
private:
	EntityId m_ownerId; //контролирующий игрок
	EntityId m_slaveId; //контролируемый актёр
	string m_equipmentPackName;
public:

	const char* GetEquipmentPackName();

	//Checking the player if he is controlling someone
	void		SetSlaveId(const EntityId id);
	EntityId	GetSlaveId() const; //returns controlled actor of this player
	IActor* GetSlaveActor(); //returns controlled actor of this player

	void	 SetOwnerId(const EntityId id);
	EntityId GetOwnerId() const;
	IActor* GetOwnerActor();

	//bones
	Vec3& GetBoneWorldPos(const char* bonename);

	//Returns true when the owner of this actor is a local player
	bool	IsLocalOwner() const;

	//Returns true when this actor is having owner
	bool	IsHaveOwner() const;

	//Returns true when this actor is having a slave (having a controlled actor)
	bool	IsHaveSlave() const;

	void	OnGrabbed(EntityId grabberId);
	void	OnDropped(EntityId grabberId);

	void	OnDrop(EntityId dropId);
	void	OnGrab(EntityId grabId);
	STOSGrabStats* GetGrabStats();

	IGrabHandler* m_pGrabHandler; //moved from protected
	STOSGrabStats m_grabStats;
	STOSVehicleStats m_vehicleStats;
	//~TheOtherSide
