...
bool CPlayerMovementController::UpdateNormal(float frameTime, SActorFrameMovementParams& params)
{
	...
	{
		//CryLogAlways("%s pAnimTarget->preparing", m_pPlayer->GetEntity()->GetName());
...
void CPlayerMovementController::UpdateMovementState(SMovementState& state)
{
	...
	//TheOtherSide not used by Cryteks
...
bool CPlayerMovementController::CIdleChecker::Process(SMovementState& movementState, CMovementRequest& currentReq, CMovementRequest& newReq)
{
	if (m_pMC->m_pPlayer->IsPlayer())
		return false;
