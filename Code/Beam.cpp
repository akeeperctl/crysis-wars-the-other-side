		...
		if (hitValid)
		{
			...
			if (m_tickTimer > 0.0f)
			{
				...
				if (m_tickTimer <= 0.0f)
				{
					...
					{
						auto mfx = gEnv->pGame->GetIGameFramework()->GetIMaterialEffects();
						int matID = mfx->GetDefaultSurfaceIndex();

						const string bulletImpactName = m_fireparams.hit_type;
						TMFXEffectId effectId = mfx->GetEffectId(bulletImpactName.c_str(), matID);

						string effectName = m_beamparams.hit_effect.c_str();

						SMFXResourceListPtr pList = mfx->GetResources(effectId);
						if (pList && pList->m_particleList)
							effectName = pList->m_particleList->m_particleParams.name;

						auto pParticleEffect = gEnv->p3DEngine->FindParticleEffect(effectName);
						if (pParticleEffect)
