...
void CWeapon::UpdateCrosshair(float frameTime)
{
	...
	bool bUpdateControlHUD = IsSelected() && GetOwnerActor() && GetOwnerActor() == g_pControlSystem->GetLocalControlClient()->GetControlledActor();

	if (g_pControlSystem->GetLocalEnabled())
	{
		if (bUpdateControlHUD)
		{
...
void CWeapon::GetAmmos(TAmmoMap& map)
{
	map = m_ammo;
}
