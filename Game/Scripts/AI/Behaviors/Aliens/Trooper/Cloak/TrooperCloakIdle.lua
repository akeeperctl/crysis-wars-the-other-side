...
--	- 6/1/2021		: Added AIPARAM_PERCEPTIONSCALE_VISUAL condition by Akeeper (TheOtherSide mod)
--------------------------------------------------------------------------
AIBehaviour.TrooperCloakIdle = {
	Name = "TrooperCloakIdle",
	Base = "TROOPERDEFAULT",
	alertness = 0,-- removed commentary
	...
	OnSomethingSeen = function( self, entity )
		...
		--~TheOtherSide;
			...
			AIBehaviour.TrooperCloakIdle:OnInterestingSoundHeard(entity);
		end
	end,
...
--		if(AI.Hostile(entity.id,data.id) and (entity.AI.lastEnemyDamageTime==nil or (_time - entity.AI.lastEnemyDamageTime > 1))) then 
	...
	OnNearMiss = function ( self, entity, sender,data)
		...
		--AIBehaviour.TrooperCloakIdle:OnBulletRain(entity,sender,data);
	end,
	...
	OnSomebodyDied = function( self, entity, sender)
		if(entity.Properties.Perception.sightrange > 0 and AI.GetAIParameter( entity.id, AIPARAM_PERCEPTIONSCALE_VISUAL ) > 0.0) then
