...
--
--------------------------------------------------------------------------
AIBehaviour.Trooper2ShootOnSpot = {
	Name = "Trooper2ShootOnSpot",
	Base = "TROOPERDEFAULT",
	alertness = 2,
	...
	OnCloseContact= function(self,entity,target)
		...
		if(diff>-1 and diff<-1) then 
			if(entity.AI.spotReached) then 
				AIBehaviour.Trooper2AttackSwitchPosition:OnCloseContact(entity,target); 
...
--		AI.SetPFBlockerRadius( entity.id, PFB_REF_POINT, 8);
	...
	OnTargetNavTypeChanged= function(self,entity,sender,data)
		AIBehaviour.Trooper2AttackSwitchPosition:OnTargetNavTypeChanged(entity,sender,data);
	end,
