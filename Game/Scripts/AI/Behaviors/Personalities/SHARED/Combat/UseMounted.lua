	...
	StartUsingMountedWeapon = function(self,entity)
  	...
  	AI.SetPFBlockerRadius(entity.id, PFB_DEAD_BODIES, radius);
   	...
   	if(entity.Properties.aicharacter_character=="TrainSoldier") then
   		AI.BeginGoalPipe("use_this_mounted_weapon_on_train");
		...
		AI.EndGoalPipe();
   		entity:SelectPipe(0, "use_this_mounted_weapon_on_train");
   	else
		...
		if(data and data.id) then 
			if(AI.Hostile(entity.id,data.id)) then
				...
				if(target and (target.id == data.id)) then 
					if(entity.Properties.aicharacter_character=="TrainSoldier" and AI.GetAttentionTargetDistance(entity.id)>2.0) then
						return;
					else
