...
--		AI.SetAimOffset(entity.id, 0,0,0);
	...
	OnPlayerSeen = function( self, entity, senda )
		...
		local weapon = entity.AI.current_mounted_weapon;
		local minDist = 7;
		if(entity.Properties.aicharacter_character=="TrainSoldier") then
			minDist = 4;
		end
		if(weapon and AI.IsMountedWeaponUsableWithTarget(entity.id,weapon.id, minDist)) then
			entity.AI.keepMG = 1;
...
--			entity:SelectPipe(0,"mg_short_reaction_leave");
	...
	OnEnemyDamage = function( self, entity, senda, data )
		...
		if(shooter) then 
			...
			if(weapon and AI.IsMountedWeaponUsableWithTarget(entity.id,weapon.id,minDist,shooter:GetPos()) ) then
				entity.AI.keepMG = 1;
				AI.Signal(SIGNALFILTER_SENDER, 1, "TO_USE_MOUNTED",entity.id);
				return;
			end
...
--		AI.RecComment(entity.id, "hostile="..tostring(AI.Hostile(entity.id, sender.id)));
		if(AI.Hostile(entity.id, sender.id)) then
			...
			if(weapon and AI.IsMountedWeaponUsableWithTarget(entity.id,weapon.id,minDist,sender:GetPos()) ) then
				entity.AI.keepMG = 1;
				AI.Signal(SIGNALFILTER_SENDER, 1, "TO_USE_MOUNTED",entity.id);
			else
				entity.AI.keepMG = nil;
