...
AIBehaviour.VtolFlyTOS = {
	...
	ACT_DUMMY = function( self, entity, sender, data )
		self:VtolFlyTOS_MAIN( entity, data );
	end,
	...
	--------------------------------------------------------------------------
	VtolFlyTOS_MAIN = function( self, entity, data )
		...
		AI.CreateGoalPipe("VtolFlyTOS");
		AI.PushGoal("VtolFlyTOS","waitsignal", 1, "VtolFlyTOS_END", nil, 1000.0 );
		AI.PushGoal("VtolFlyTOS","signal",1,1,"TO_HELI_IDLE",SIGNALFILTER_SENDER);
		entity:InsertSubpipe(AIGOALPIPE_SAMEPRIORITY,"VtolFlyTOS",nil,data.iValue);
		...
		if ( entity.AI.flyPathName ~=nil ) then
			...
			Script.SetTimerForFunction( minUpdateTime *1000, "AIBehaviour.VtolFlyTOS.VtolFlyTOS_SUB_PATH", entity );

		else
			entity.AI.heliTimer4 = 1;
			Script.SetTimerForFunction( minUpdateTime *1000, "AIBehaviour.VtolFlyTOS.VtolFlyTOS_SUB_POINT", entity );
		end
	...
	----------------------------------------------------------------------------------------------------------------------------------------------------
	VtolFlyTOS_SUB_POINT = function( entity )

	--System.LogAlways(entity:GetName().." VtolFlyTOS_SUB_POINT block("..tonumber(entity.AI.bLock)..")");
	...
	--------------------------------------------------------------------------
		...
		if ( entity.AI.bLock == 1 ) then
			...
			FastScaleVector( vTmp, entity.AI.vDirectionRsv, 500.0 );
			...
			local dist = DistanceVectors(entity.AI.vReinforcementSetPosition, vMyPos);
	
			local threshold = 120
			if(entity.AI.flyStopDistance) then
				threshold = entity.AI.flyStopDistance
			end

			--System.LogAlways(entity:GetName().." dist("..dist..") and treshold("..threshold..")");
			...
			if (dist < threshold) then
				...
				--System.LogAlways(entity:GetName().." VtolFlyTOS_END");
	...
	----------------------------------------------------------------------------------------------------------------------------------------------------
	VtolFlyTOS_SUB_PATH = function( entity )
