function PipeManager:OnInitTheOtherSide()

	AI.BeginGoalPipe("locate_refpoint");
		AI.PushGoal("locate",1,"refpoint");	
	AI.EndGoalPipe();

    AI.BeginGoalPipe("setup_combat_idle");	
	    AI.PushGoal("firecmd",0,0);	
	    AI.PushGoal("bodypos",0,BODYPOS_STAND);	
	    AI.PushGoal("run",0,0);
    AI.EndGoalPipe();

    AI.BeginGoalPipe("simple_goto");
    ...
    AI.BeginGoalPipe("squad_search_enemy"); --for scout(0.5sec) -- на скауте с группой не проверял
        AI.PushGoal("+firecmd",0,0);
        AI.PushGoal("+bodypos",0,BODYPOS_STEALTH);
        AI.PushGoal("branch", 1, "START", BRANCH_ALWAYS);	
        AI.PushLabel("START");
    ...
    AI.BeginGoalPipe("ord_cooldown"); --for scout(0.5sec) -- на скауте с группой не проверял
        ...
        AI.PushLabel("END");
            ...
            AI.PushGoal("clear",1,1); -- clear всегда нужно ставить в конец goalpipe
    AI.EndGoalPipe();

    AI.BeginGoalPipe("ord_cooldown_trooper");--for trooper(1.5sec) --не работает нормально с группой
        ...
        AI.PushLabel("END");
            ...
            --AI.PushGoal("clear",1,0); -- clear всегда нужно ставить в конец goalpipe
            --AI.PushGoal("clear",1,1); -- clear всегда нужно ставить в конец goalpipe
            --AI.PushGoal("clear",1,1);
    AI.EndGoalPipe();

    AI.CreateGoalPipe("rush_ai");
    AI.PushGoal("rush_ai","locate",0,"refpoint");
    AI.PushGoal("rush_ai","run",0,2);
    AI.PushGoal("rush_ai","timeout",1,0.2,0.8);
    AI.PushGoal("rush_ai","firecmd",1,1);
    AI.PushGoal("rush_ai","approach",1,5);
    AI.PushGoal("rush_ai","firecmd",1,0);
    AI.PushGoal("rush_ai","signal",1,1,"STOP_RUSH",0);
end