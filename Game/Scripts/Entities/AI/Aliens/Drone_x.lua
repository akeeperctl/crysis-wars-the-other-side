...
Drone_x =
{
	...
	PropertiesInstance =
	{
		aibehavior_behaviour = "DroneIdle",
		bAutoDisable = 1,
	},
	--AI related	over
	...
	Properties =
	{
		...
		accuracy = 1.0,
		species = 2,
		aicharacter_character = "Drone",
		soclasses_SmartObjectClass = "Drone",
		fileModel = "Objects/characters/alien/C3/drone/dronebody.chr",
	...
	gameParams =
	{
		...
		destruct_charge_sound = "sounds/alien_drone:oneshot:ping_charge",
		destruct_explode_sound = "sounds/alien_drone:oneshot:explosion",
		destruct_gound_sound = "sounds/alien_drone:oneshot:drone_hit_ground",
		death_explode_sound = "sounds/alien_drone:oneshot:explosion",
...
-- 		},
	...
	AIMovementAbility =
	{
		...
		maxTurnRadius = 4,	-- meters
		optimalFlightHeight = 3.0,
		minFlightHeight = 2.0,
		maxFlightHeight = 10.0,
    	pathFindPrediction = 0.05,
		-- The alien scout air space is divided into three zones, each the height of the attackZoneHeight.
		attackZoneHeight = 3,	-- meters
...
function Drone_x:RegAI(bPlayer)
	if bPlayer then
		...
		-- end
		--System.LogAlways("registered for PUPPET");
	end
end

function Drone_x:ControlCancelShootAt()
	entity:CancelSubpipe(90000);
end

function Drone_x:OnResetCustom()

	self.iAutoDestructTimer = 0;
	--
	--self.gEnergy = self.Properties.Damage.shieldEnergy;
	--self.isUseShield = self.Properties.Damage.bUseShield;
...
function Drone_x.Client:OnUpdate(frameTime)
...
function Drone_x:UpdateEnergy(frameTime)

	--if(self.isUseShield ~= 0)then
		self:ActivateOutput("Energy",self.actor:GetAlienEnergy())
	--end

	-- if(self.gEnergy < self.Properties.Damage.shieldEnergy) then
	-- 	self.gEnergy = self.gEnergy + 0.2
	-- end

end

function Drone_x:OnShoot()
...
function Drone_x:ResetAnimation()
	self:StopAnimation(0,3);
end

function Drone_x:DoPlayerSeen()
...
function Drone_x:DoShootWeapon()
	--self:PushEvent("moac_attack");
end

function Drone_x:DoShootWeaponLong()
	--self:PushEvent("moac_attack_long");
end

function Drone_x:DoInvestigate()
...
function Drone_x:DoPickupTruck()
	--self:StopAnimation(0,3);
	--self:StartAnimation(0, "truck_arrive", 3, 0.3, 0.5, false,true);
end

function Drone_x:DoHoldTruck()
	--self:StopAnimation(0,3);
	--self:StartAnimation(0, "truck_idle", 3, 0.15, 1.0, true,true);
end

function Drone_x:DoDropTruck()
	--self:StopAnimation(0,3);
	--self:StartAnimation(0, "truck_take_off", 3, 0.15, 1.0, false,true);
end

function Drone_x:Kill(ragdoll, shooterId, weaponId)
	--Log ("Drone_x:Kill() called");

		local health = self.actor:GetHealth();
	if ( health > 0 ) then
		Log("ERROR:DRONE is Killed though he has a health still.");
	end
...
--	Script.SetTimer(3000, Drone_x.OnDeath, self);
	...
	if ( bSpecialDir == true ) then
		...
		self.lastImpulseAmt = stats.mass*1.0;
		self.reImpulse = 0.1;

	else

		local stats = self:GetPhysicalStats();
		self.lastImpulseAmt = stats.mass*lastSpeed *0.2;
		self.reImpulse = randomF(1.0,3.0);

	end

	--partId 10 is the head
	self:AddImpulse(2,{x=0,y=0,z=0},self.lastImpulseDir,self.lastImpulseAmt,1);

	--explosion
	--Particle.SpawnEffect("alien_special.scout.ScoutCrashA", pos, g_Vectors.v001, 1.0);
...
function Drone_x:ScriptEvent(event,value,str)
...
function Drone_x:GetDamageMultiplier(hit)
...
function Drone_x.Client:OnHit(hit, remote)
...
function Drone_x.Server:OnHit(hit)--(shooterId, weaponId, matName, damage, hitType, pos, normal, partId, radius)
	...
	if(shooter~=nil) then
		...
		if (self.Properties.species ~= shooter.Properties.species) then
			if(self == g_localActor or self.actor:IsLocalOwner()) then
	...
	local health = self.actor:GetHealth();
	if(shooter~=nil) then
		if (self.Properties.species ~= shooter.Properties.species or self.id == shooter.id) then

			-- if(self.isUseShield ~= 0) then
			-- 	local shieldMult = self.Properties.Damage.shieldBulletMult;

			-- 	damage = damage * shieldMult;
			-- 	self.actor:SetAlienEnergy(self.actor:GetAlienEnergy() - damage);
			-- 	if (self.actor:GetAlienEnergy() < 1.0) then
			-- 		self.actor:SetAlienEnergy(0);
...
function Drone_x:GrabEntity(entityname)
...
function Drone_x:InitiateAutoDestruction()
	System.LogAlways("Drone_x:InitiateAutoDestruction");

	if ( self.iAutoDestructTimer == 0 ) then
	  self.iAutoDestructTimer = 1;
	  Script.SetTimerForFunction( self.warmupAutoDestructionTime*1000, "Drone_x.WarmupAutoDestruct",self);
	end
end

function Drone_x:WarmupAutoDestruct()
	System.LogAlways("Drone_x:WarmupAutoDestruct");
	...
	if ( self.iAutoDestructTimer == 1 ) then
		...
		if ( self.waterCrash ~= nil) then
			...
			if ( self.waterCrash > 3000 ) then
				...
				Particle.SpawnEffect("alien_special.Drone.Explosion", g_Vectors.temp_v1, g_Vectors.v001, 1);
				self:PlaySoundEvent(self.gameParams.destruct_charge_sound, g_Vectors.v000, g_Vectors.v010, 0, SOUND_SEMANTIC_EXPLOSION);
				
				System.LogAlways("scoutCrashA 1038");
				
				self.iAutoDestructTimer = 2;
				Script.SetTimerForFunction(self.autoDestructionTime*1000,"Drone_x.AutoDestruct",self);
			else
			  Script.SetTimerForFunction( 200, "Drone_x.WarmupAutoDestruct",self);
			end
		elseif ( self:GetSpeed() < 1.0  ) then
			...
			System.LogAlways("scoutCrashA 1050");
...
function Drone_x:AutoDestruct()
	System.LogAlways("Drone_x:AutoDestruct");
	if ( self:GetSpeed() == nil or self:GetPos() == nil ) then
		self:RemoveActor();
		return;
	end
...
function Drone_x:ReApplyImpulse()
...
function Drone_x:PushEntities(hit)
	...
	if (e) then
		...
		if(e.actor)then
			e.actor:Fall(hit.pos);
		end
		--~TheOtherSide
	end
end

function Drone_x.Client:OnCollision(hit)
...
--   end
	...
	if (hit.target_mass and hit.target_mass > 0) then
    	return true; -- don't process collisions with light objects
  	end
	--~TheOtherSide
	...
	-- local e = System.GetEntity(hit.target_id);
	-- if(e)then
	-- 	System.LogAlways("OnCollision "..e:GetName());
	-- end
	
	--Static object deconstruction on explosion
	local radius = 1;
	local damage = self:GetSpeed() * 9.81;
	g_gameRules:CreateExplosion(self.id,self.id, damage,hit.pos,nil,radius);
	--(shooterId,weaponId,damage,pos,dir,radius,angle,pressure,holesize,effect,effectScale, minRadius, minPhysRadius, physRadius)

	self:PushEntities(hit);

	-- the code below is supposed to use for the dead scout

	if (not self:IsDead() and self.actor:IsHaveOwner()) then
		--System.LogAlways("Scout have owner and OnCollision");
		...
		System.LogAlways("ScoutCrashB_snow");
		
		System.LogAlways("scoutCrashB 1087");
		
	--	g_gameRules:ClientViewShake(pos,90,0.5,0.5,0.05);
	end
	...
	if (vel>8*8) then
		...
		local spawned = false;
		if (hit.materialId and hit.materialId >= 0) then
			local effectId = MaterialEffects.GetEffectId("player_fx", hit.materialId);
		  if (effectId ~= MFX_INVALID_EFFECTID) then
		    spawned = MaterialEffects.ExecuteEffect(effectId, {pos=hit.pos});
		  end
		end
...
--		  Log("fallback: ScoutCrashB_snow");
		...
		if (self.firstCrash) then
		 ...
		 -- g_gameRules:ClientViewShake(pos,30,1.5,0.25,0.05);
		  ...
		  System.LogAlways("scoutCrashB 1223");
	...
	--		self:PlaySoundEvent(self.gameParams.death_explode_sound, g_Vectors.v000, g_Vectors.v010, 0, SOUND_SEMANTIC_EXPLOSION);
			...
			System.LogAlways("scoutCrashA 1236");
...
-----------------------------------------------------------------------------------------------------
function Drone_x:MakeParalyzed( dir, strength )
...
function Drone_x.OnDeath( entity )
--	AI.LogEvent(" >>>> Drone_x.OnDeath "..entity:GetName());
...
function Drone_x:GrabObject(object, query)
...
------------------------------------------------------------------------------
function Drone_x:EnableSearchBeam(bEnable)
  --self.actor:CreateCodeEvent({event="searchBeam", enable=bEnable});
end

------------------------------------------------------------------------------
function Drone_x:SetSearchBeamDir(worldDir)
  --self.actor:SetSearchBeam(worldDir);
end


function Drone_x:GrabMultiple()
	...
	-- --System.Log("Drone_x:GrabMultiple() for "..self:GetName().."/"..numGrabbed)
...
function Drone_x:DropObject(throw,throwVec,throwDelay)
...
function Drone_x:DropMultiple(throw, throwVec, throwDelay)
	-- if ( self.objects_to_be_grabbed ==nil ) then
	-- 	return;
	-- end
...
function Drone_x:OnSave(save)

	--save.isUseShield = self.isUseShield
	...
	save.idleSound = self.idleSound;
end

function Drone_x:OnLoad(save)

	--self.isUseShield = save.isUseShield
	...
	-- end
	self.idleSound = saved.idleSound;
end
