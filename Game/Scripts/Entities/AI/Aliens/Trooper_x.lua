...
--Script.SetTimerForFunction(1000,"Trooper_x.MoveEnergyRegen", self);
	...
	Properties = 
	{
	  ...
	  bCanSelfDestruct = 0,
		...
		Damage =
		{
			...
			dieEffect="alien_special.Trooper.death_explosion",
...
TROOPER_ENERGY_REGENERATION_TIMER = 2000;
TROOPER_HEALTH_REGENERATION_TIMER = 2000;

TROOPER_CONV_IDLE = 0;
TROOPER_CONV_REQUESTING = 1;

function Trooper_x:RegAI(bPlayer)
	if bPlayer then
...
function Trooper_x:Event_Energy(sender, value)
...
function Trooper_x:noDamageImpulse(bValue)
	value = bValue;
	self.AI.noDamageImpulse = value;
end

function Trooper_x:Mel(EntityId)
	
	--self.Behaviour:Destructor(self);
	--AI.SoundEvent(self:GetWorldPos(), 15, AISE_MOVEMENT, self.id)
	local target = System.GetEntity(EntityId);
		if(target) then
			AI.meleeTarget = target;
			local vel = g_Vectors.temp;
			target:GetVelocity(vel);
			local speed = target:GetSpeed();
			if(speed > 1) then
				local y = dotproduct2d( self:GetDirectionVector(1), vel);
				if(y>=0) then 
					return;
				end
			end
			AI.lastMeleeTime = curTime;
			self:MeleeAttack(target);
--			entity:SelectPipe(0,"do_nothing");
-- --			entity:SelectPipe(0,"tr_melee_timeout");
			-- self:InsertSubpipe(AIGOALPIPE_NOTDUPLICATE,"tr_do_melee");
			entity:SelectPipe(0,"do_nothing");
		end
		AI.Animation(self.id,AIANIM_SIGNAL,"meleeAttack");
			--self:SelectPipe(0,"tr_melee");
end	

function Trooper_x:WeaponSound()
	 AI.SoundEvent(self:GetWorldPos(), 45, AISE_WEAPON, self.id)	
end	

function Trooper_x:Event_GetEnergy(sender)
	
	local tk = true
	if (tk) then	
		self:ActivateOutput("Energy",self.energy )
		tk = false
	end
			
end	

-- function Trooper_x:RegenEnergy()
...
-- 		self:SetTimer(TROOPER_ENERGY_REGENERATION_TIMER,10)	
-- 		if (self.energy<200) then			
-- 				self.energy = self.energy + (0.1 * scale)
-- 		end
-- 	end	
	
-- end

-- function Trooper_x:RegenHealth()
	
-- 	local maxHealth = self.Properties.Damage.health;
-- 	local currentHealth = self.actor:GetHealth();
-- 	local scale = maxHealth / 100;
	
-- 	if(self.energy == 200)then
-- 		self:SetTimer(TROOPER_HEALTH_REGENERATION_TIMER,3000)
-- 		if(maxHealth == currentHealth) then
-- 			return;
-- 		elseif(maxHealth > currentHealth) then	
...
function Trooper_x.Client:OnUpdate(frameTime)
	...
	if (self.idleSound ) then	 	 	  
  	...
  	local rpmDecel = 0.2;
		if(speed ~= 0) then
			AI.SoundEvent(self:GetWorldPos(), self.Properties.fsoundRadius, AISE_MOVEMENT, self.id)
		end
...
function Trooper_x:PlayAccelerationSound()
	if( not (self.accelerationSound and Sound.IsPlaying(self.accelerationSound)))	then
		...
		--AI.SoundEvent(self:GetWorldPos(), 15, AISE_MOVEMENT, self.entity.AI);
--		if (sound) then
...
function Trooper_x:Kill(ragdoll, shooterId, weaponId)
	...
	--System.LogAlways("Trooper_x:Kill")
...
function Trooper_x:ResetTimers()
	...
	self:KillTimer(TROOPER_ENERGY_REGENERATION_TIMER); 
	self:KillTimer(TROOPER_HEALTH_REGENERATION_TIMER);
...
function Trooper_x.AutoDestruct(entity)--,timerid)
	...
	--System.LogAlways("[LUA] Trooper_x.AutoDestruct Damage "..damage);
	...
	Particle.SpawnEffect(entity.Properties.Damage.dieEffect,pos,g_Vectors.v000,entity.id);	
	entity:RemoveActor();
end
...
function Trooper_x:MeleeAttackById(entityId)
	if(not self.AI.meleeImpulse) then 
		self.AI.meleeImpulse = {x=0,y=0,z=0};
		CopyVector(self.AI.meleeImpulse,self:GetDirectionVector(1));
	end
	local unit = System.GetEntity(entityId);
	self.AI.meleeTarget = unit;
	self:SetGrabbable(0);
	self:SetTimer(TROOPER_END_MELEE_TIMER,1500);
end
...
function Trooper_x:Ctrl_DoMelee(impulse,targetId)
...
--				end
				...
				elseif( entity.Properties and entity.Properties.bNanoSuit==1) then 
					hit.damage	= melee.damage * melee.damageMultiplier ;					
				end
...
--					local distance = math.sqrt(distance2);
	        ...
	        if (energy ~= 0) then	          
	          entity.actor:SetNanoSuitEnergy(energy-0.2*NANOSUIT_ENERGY);
				...
				elseif(entity.vehicle and entity:IsEntityOnVehicle(entity.id)) then
					-- player is on the meleed vehicle
					-- TO DO: improve with linear shake rather than angular
					entity.actor:CameraShake(random(20,30), 0.2, 0.13, g_Vectors.v000);
			  end		  
...
function Trooper_x.Client:OnTimer(timerId,mSec)
	...
	--elseif(timerId == TROOPER_ENERGY_REGENERATION_TIMER) then 
	--	self:RegenEnergy();
	elseif(timerId == TROOPER_HEALTH_REGENERATION_TIMER) then 
		self:RegenHealth();
	end
end
...
function Trooper_x:Event_Beam(target)
...
function Trooper_x:Event_EnableRagdoll(params)
	self.actor:SetPhysicalizationProfile("ragdoll");
end

function Trooper_x:Event_DisableRagdoll(params)
...
Trooper_x.FlowEvents =
{
	Inputs =
	{
		...
		Beam = { Trooper_x.Event_Beam, "int" },
		Energy = { Trooper_x.Event_Energy, "int" },--
		GetEnergy = { Trooper_x.Event_GetEnergy, "bool" },
		EnableRagdoll = { Trooper_x.Event_EnableRagdoll, "bool" },
		DisableRagdoll = { Trooper_x.Event_DisableRagdoll, "bool" },
		EnableLamLights = { Trooper_x.Event_EnableLamLights, "bool" },
		DisableLamLights = { Trooper_x.Event_DisableLamLights, "bool" },
	},
	Outputs =
	{
		EnableCloaked = "bool",
