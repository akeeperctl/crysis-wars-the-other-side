...
Pinger_x = 
{
	...
	Properties = 
	{
	  bExactPos = 0,
		...
		fileModel = "Objects/Characters/Alien/C3/Pinger/Pinger.chr",	
		...
		Shield = 
		{
			explosionDamageMult = 0.3,
			bulletDamageMult = 0.1,
			regenTime = 100,
		},
...
--				"melee_attack_02",
	...
	IKLimbs = 
	{
		{0,"RightTentacle","Right_leg_1","","Right_leg_finger_2_3",0},
		{0,"LeftTentacle","Left_leg_1","","Left_leg_finger_2_3",0},
		{0,"BackTentacle","Back_leg_1","","Back_leg_finger_2_3",0},
...
function Pinger_x:CreateAttachments()
	...
	self:CreateBoneAttachment(0, "Generator", "death_effect_1");
...
------------------------------------------------------
-- function Pinger_x:AbilityHunterThrow(grabId, throwPos, scheme)

-- 	local hunter = System.GetEntity( self.id );
-- 	local grab =   System.GetEntity(grabId);
...
-- 	--grab:GetWorldPos(throwVec);
-- 	--hunter:GetWorldPos(hunterPos);
-- 	--SubVectors(throwPos,throwPos,hunterPos);
	
-- 	--local len = LengthVector(throwPos)*1;
-- 	--NormalizeVector(throwPos);
	
-- 	--throwPos.z = throwPos.z - 1;
-- 	--NormalizeVector(throwPos);
	
-- 	--FastScaleVector(throwPos,throwPos,len);
-- --}
-- --~TheOtherSide
-- 	hunter:SetGrabbingScheme(scheme);
	
-- 	--Log(Vec2Str(throwPos));	
-- 	hunter:GrabObject(grab,nil);	
-- 	hunter:DropObject( true, throwPos, 2 );
-- end

function Pinger_x:RegAI(bPlayer)
	if bPlayer then
...
-----------------------------------------------------------------------------------------------------
function Pinger_x:OnResetCustom()
...
--function Pinger_x.Client:OnAnimationEvent(animation,strPar,intPar)
--	Log("animation event on "..animation.." ("..intPar..")");
--end

------------------------------------------------------------------------------
function Pinger_x.Client:OnUpdate(frameTime)
...
-- function Pinger_x:UpdateEnergy(frameTime)
...
------------------------------------------------------------------------------
-- function Pinger_x:UpdateGlow(frameTime)
...
------------------------------------------------------------------------------
-- function Pinger_x:SetGlow(glowtype, goal, speed, initial)
...
-- 	if (weapon) then
--   	if (weapon.class == "SingularityCannon" or weapon.class == "SingularityCannon_PLAY") then
--   	  self:SetGlow(HUNTER_GLOW_SINGULARITY, 1.0, 0.27);  	  
--   	elseif (weapon.class == "MOAR" or weapon.class == "MOAR_PLAY") then
...
-- function Pinger_x.SetHunterGlowLevel( self, level )	
-- 	--Log("Glow = %.3f", level);	
-- 	local Glow = self.Glow;
...
------------------------------------------------------------------------------
function Pinger_x:CanHoldObject(mass,volume,query)
...
------------------------------------------------------------------------------
function Pinger_x:DoPlayerSeen()
	
  	--System.Log ("Pinger_x:DoPlayerSeen() called");	
	--BasicAlien.DoPlayerSeen(self);
end

------------------------------------------------------------------------------
function Pinger_x:DoShootWeapon()

	-- System.LogAlways("Pinger_x:DoShootWeapon() called");	
...
------------------------------------------------------------------------------
function Pinger_x:StopEverything(stopAnimation)
...
------------------------------------------------------------------------------
-- function Pinger_x:TakeOffDropObjectAtPoint(targetPos)
...
------------------------------------------------------------------------------
function Pinger_x:GetDamageMultiplier(hit)
...
-- function Pinger_x:ResetDetachableSpots()
...
------------------------------------------------------------------------------
--FIXME:should be in basicalien?
--function Pinger_x:ProcessDetachableSpots(weaponId, matName, damage, hitType, pos, normal, partId)
-- function Pinger_x:ProcessDetachableSpots(hit)
...
------------------------------------------------------------------------------
function Pinger_x.Server:OnHit(hit)

	--Log("Pinger_x.Server:OnHit");

	if (hit.shooterId == self.id and hit.type ~= "event") then
		return false;
	end
	
	local shieldExploMult = 1;
	...
	if (hit.explosion) then
		...
		shieldExploMult = self.Properties.Shield.explosionDamageMult;
	end
	...
	--if(self.bCanNotDie == 0) then 
		if(self.actor:IsShieldProjected()) then
			local shieldMult = self.Properties.Shield.bulletDamageMult;
	
			fDamage = fDamage * shieldMult * shieldExploMult;
			self.actor:SetAlienEnergy(self.actor:GetAlienEnergy() - fDamage);
...
------------------------------------------------------------------------------
-- function Pinger_x:OnVulnerableHit(partId, materialType)
...
------------------------------------------------------------------------------
function Pinger_x.Client:OnTimer(timerId,mSec)
...
------------------------------------------------------------------------------
-- function Pinger_x:ProcessHighPose (hit, damageMult)
...
------------------------------------------------------------------------------
function Pinger_x:Kill(ragdoll, shooterId, weaponId)
...
------------------------------------------------------------------------------
function Pinger_x.Client:OnHit(hit, remote)
...
--function Pinger_x:Event_GoPatrol( params )
--	AI.Signal(0, 1, "GO_PATROL", self.id);
--end	
--
--function Pinger_x:Event_GoSearch( params )
--	AI.Signal(0, 1, "GO_SEARCH", self.id);
--end	
--
--function Pinger_x:Event_GoEscort( params )
--	AI.Signal(0, 1, "GO_ESCORT", self.id);
--end	

------------------------------------------------------------------------------
-- function Pinger_x:GrabEntity(entityname)
...
------------------------------------------------------------------------------
function Pinger_x.Client:OnCollision(hit)
...
------------------------------------------------------------------------------
function Pinger_x:ScriptEvent(event,value,str)
...
------------------------------------------------------------------------------
function Pinger_x:AnimationEvent(event,value)
	...
	--   	if (weapon.class == "SingularityCannon" or weapon.class == "SingularityCannon_PLAY") then
	--   	  self:SetGlow(HUNTER_GLOW_SINGULARITY, 1.0, 0.27);  	  
	--   	elseif (weapon.class == "MOAR" or weapon.class == "HunterSweepMOAR" or weapon.class == "MOAR_PLAY" or weapon.class == "HunterSweepMOAR_PLAY") then
	--   	  self:SetGlow(HUNTER_GLOW_FREEZE_BEAM, 0.45, 0.25);
	--   	end	  
	-- 	end
	if ( event == "DeadHitGroundNow" ) then
...
------------------------------------------------------------------------------
function Pinger_x:GetWeaponDir(weapon)
...
------------------------------------------------------------------------------
function Pinger_x.OnDeath( entity )
	Log(" >>>> Pinger_x.OnDeath "..entity:GetName());
	entity:TriggerEvent(AIEVENT_AGENTDIED);
end 

------------------------------------------------------------------------------
-- function Pinger_x:SetGrabbingScheme(scheme)
...
function Pinger_x:GrabObject(object, query)
...
------------------------------------------------------
-- function Pinger_x:Event_GrabVTOL(sender)
...
------------------------------------------------------
-- function Pinger_x:Event_ReleaseVTOL(sender)
...
----------------------------------------------------
function Pinger_x:HealthChanged()	
...
Pinger_x.FlowEvents =
{
	Inputs =
	{
		-- GrabVTOL = { Pinger_x.Event_GrabVTOL, "bool" },
		-- ReleaseVTOL = { Pinger_x.Event_ReleaseVTOL, "bool" },		
	},
	Outputs =
	{
