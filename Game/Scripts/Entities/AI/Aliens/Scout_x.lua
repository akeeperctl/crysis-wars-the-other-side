...
-----------------------------------------------------------------------------------------------------
--TheOtherSide
function Scout_x:GrabEntityInTentacle(entityId)
	local GrabObj = System.GetEntity( entityId );
	if ( GrabObj ) then
	else
		return;
	end

	--Define object to get grabbed
	local index = table.getn(self.objects_to_be_grabbed)+1;	
	self.objects_to_be_grabbed[index] = GrabObj.id;

	--Do grab defined object
	self:GrabMultiple()

	--if (GrabObj.actor and not GrabObj.isAlien)	then
		--GrabObj:TurnRagdoll(1);
		--GrabObj.actor:Fall(GrabObj:GetPos());
	--end
end

function Scout_x:DropEntitiesFromTentacles(point)
	self:PlaySoundEvent("Sounds/alien:scout:drop_trooper", self.AI.vZero, self:GetDirectionVector(1), SOUND_DEFAULT_3D, SOUND_SEMANTIC_AI_READABILITY);
...
function Scout_x:SetAnimationInput(input, value)
	self.actor:SetAnimationInput(input, value);
end

function Scout_x:RegAI(bPlayer)
	if bPlayer then
...
function Scout_x:UpdateEnergy(frameTime)
...
function Scout_x.Client:OnHit(hit, remote)
	...
	if ( damage > maxhealth * 0.25 ) then
		...
		if ( math.abs( dotproduct3d( vFwd, vDir ) ) > math.cos( 3.1416 * 30.0 / 180.0 ) ) then
			self:SetAnimationInput("Signal","largeHit");
		else
			if ( dotproduct3d( vWng, vDir ) > 0 ) then
				self:SetAnimationInput("Signal","HeavyDamageRight");
			else
				self:SetAnimationInput("Signal","HeavyDamageLeft");
			end
		end
	else
		self:SetAnimationInput("Signal","smallHit");
	end
...
function Scout_x.Server:OnHit(hit)--(shooterId, weaponId, matName, damage, hitType, pos, normal, partId, radius)
	--System.LogAlways("Scout_x.Server:OnHit: type = "..hit.type);
	--System.LogAlways("Scout_x.Server:OnHit: typeid = "..hit.typeId);
	...
	local health = self.actor:GetHealth();
	if(shooter~=nil) then
		local sameSpecies = self.Properties.species == shooter.Properties.species;
		local meShooter = self.id == shooter.id;
		local dudeShooter = shooter.id == g_localActorId;

		--if ((sameSpecies and dudeShooter) or (not sameSpecies or meShooter)) then
		if (meShooter or not sameSpecies) then
	
			if(self.isUseShield == 1 and (not meShooter)) then
				...
				--System.LogAlways("Scout use shield, hitType: "..hit.type);
			else
				health = health - damage;
				if (health < 1.0) then
					health = 0;
				...
				self.actor:SetHealth(health);
				self:HealthChanged();
				--System.LogAlways("Scout not use shield, hitType: "..hit.type);
			end

			--if (self.actor:IsHaveOwner()) then
				--System.LogAlways(self:GetName().." is damaged: "..damage.." with hittype: "..hit.type);
			--end
...
function Scout_x:PushEntities(hit)
	...
	if (e) then
		...
		if (e.Properties.species == self.Properties.species) then
			return;
		end

		--TheOtherSide
		local scoutPos = self:GetWorldPos();
		local targetPos = e:GetWorldPos();
		local dir = targetPos;
		SubVectors(dir,dir,scoutPos);

		local length = LengthVector(dir);
		...
		if (e.vehicle) then
			e:AddImpulse(-1, targetPos, dir, self:GetSpeed() * 9.81 * 1, 1);
		else
			e:AddImpulse(-1, targetPos, dir, self:GetSpeed() * 9.81, 1);
		end
...
function Scout_x.Client:OnCollision(hit)
	...
	local radius = 1;
	local damage = (self:GetSpeed() / 2) * 9.81;
...
function Scout_x:DropObject(throw,throwVec,throwDelay)
	--System.LogAlways("[LUA][SCOUT DropObject][START]")
...
function Scout_x:DropMultiple(throw, throwVec, throwDelay)
	...
	--System.LogAlways("[LUA][SCOUT DropMultiple][START]")

	if ( self.objects_to_be_grabbed ==nil ) then
		return;
	end

	--System.LogAlways("[LUA][SCOUT DropMultiple][CONTINUE] objects_to_be_grabbed")
	...
	--System.LogAlways("[LUA][SCOUT DropMultiple][CONTINUE] numGrabbed "..numGrabbed)
	...
	for i=1, numGrabbed do
		...
		--System.LogAlways("[LUA][SCOUT DropMultiple][CONTINUE] OnDropped")
		AI.Signal(SIGNALFILTER_SENDER,0,"OnDropped",self.objects_to_be_grabbed[i]);
	end

	--System.LogAlways("[LUA][SCOUT DropMultiple][CONTINUE] preEvent")
	...
	--System.LogAlways("[LUA][SCOUT DropMultiple][CONTINUE] after Event")
end
