...
--TheOtherSide
function BasicAlien:OnWeaponStartFire(weaponId)
	--BasicActor:OnWeaponStartFire(weaponId)

	if (self.actor:IsEMP() or self.actor:IsHaveOwner() or self.actor:GetHealth() < 0) then
		return;
	end

	local target = AI.GetAttentionTargetEntity( self.id );
	local isHostile = target and AI.Hostile( self.id, target.id )

	if ((self.IsHaveCloak == true and self.cloaked == 1) and isHostile) then
		self:Cloak(0);
	end

	--System.LogAlways("BasicAlien:OnWeaponStartFire");

end

function BasicAlien:OnWeaponStopFire(weaponId)
	--BasicActor:OnWeaponStopFire(weaponId)
	...
	if ((self.IsHaveCloak == true and self.cloaked == 0) and isHostile) then
		self:Cloak(1);
	end
end

function BasicAlien:SetGlowing(nSlot,nValue)
	--slot 0 - body, slot 1 - head
	self:SetMaterialFloat( 0,nSlot,"glow",nValue);

end

function BasicAlien:GetGlowing(nSlot)
	--slot 0 - body, slot 1 - head
	return self:GetMaterialFloat( 0,nSlot,"glow");
...
function BasicAlien.Server:OnHit(hit)
	...
	local shield=self.actor:GetShieldEnergy();
	...
	if (shooter==nil or self.Properties.species ~= shooter.Properties.species or hit.explosion or hit.type == "smartbomb") then
	
		if (shield > 0 and self.actor:IsShieldProjected()) then
			local newshield=__max(shield-damage, 0);
	-- Had to modify the shield as a deflector operating as long as there it has energy
			local newdamage=0;--__max(damage-shield, 0);
			--LogWarning("<%s> shield: %.1f / new shield: %.1f / damage: %.1f / reduced damage: %.1f", self:GetName(), shield, newshield, damage, newdamage);
			damage=newdamage;
			self.actor:SetShieldEnergy(newshield);
		end	
		
		health=math.floor(health-damage);
		
		--LogWarning("<%s> damage %.1f / newHealth %.0f", self:GetName(), damage, health);
	
		if (not self.actor:IsRageMode()) then
...
function BasicAlien:Cloak(cloak)
	...
	if (cloaked ~= (cloak or 0)) then
		...
		if (self.cloaked == 1) then
			AI.ChangeParameter(self.id,AIPARAM_CAMOSCALE,0.05);
			--self:TriggerEvent(AIEVENT_PLAYER_STUNT_CLOAK);
			self:PlayIdleSound(self.voiceTable.idleCloak);
		else
			--self:TriggerEvent(AIEVENT_PLAYER_STUNT_UNCLOAK);
		...
		if(cloak==1) then 
			AI.ChangeParameter(self.id,AIPARAM_CAMOSCALE,0.05);
			--self:TriggerEvent(AIEVENT_PLAYER_STUNT_CLOAK);
			self:PlaySoundEvent("sounds/alien:trooper:cloak", g_Vectors.v000, g_Vectors.v010, SOUND_DEFAULT_3D, SOUND_SEMANTIC_AI_READABILITY);
			--self:Readibility("CLOAK",1,100);
			
		else
			AI.ChangeParameter(self.id,AIPARAM_CAMOSCALE,1);
			--self:TriggerEvent(AIEVENT_PLAYER_STUNT_UNCLOAK);
			self:PlaySoundEvent("sounds/alien:trooper:uncloak", g_Vectors.v000, g_Vectors.v010, SOUND_DEFAULT_3D, SOUND_SEMANTIC_AI_READABILITY);
...
--TheOtherSide
function BasicAlien:EnableShield(bEnable)
	self.actor:CreateCodeEvent({event="shieldProjection", enable=bEnable});
	--System.LogAlways("PlayerTrooper_x:EnableLamLights");
end

function BasicAlien:Event_EnableShield(sender)
	self:EnableShield(true);
	--System.LogAlways("PlayerTrooper_x:Event_EnableLamLights");
end

function BasicAlien:Event_DisableShield(sender)
	self:EnableShield(false);
	--System.LogAlways("PlayerTrooper_x:Event_EnableLamLights");
end
--~TheOtherSide
...
BasicAlien.FlowEvents =
{
	...
	{	
	  ...
	  EnableShield = { BasicAlien.Event_EnableShield, "bool" },
	  DisableShield = { BasicAlien.Event_DisableShield, "bool" },
	},
	Outputs =
	{			
	  Cloak = "bool",
