...
--TheOtherSide
function Player:RegisterPlayerAI()
	if AI then
...
function Player:SetSpecies(value)
	if (value) then
		self.Properties.species = value;
		--BasicActor.PhysicalizeActor(self);
	end
end
...
function Player:SetOnUseData(entityId, slot)
	--System.LogAlways("SetOnUseData "..tostring(entityId).." "..tostring(slot))
...
function Player:OnAction(action, activation, value)
	...
	-- if(self.actor:GetPlayerSlaveId() ~= 0) then
	-- 	return;
