...
--TheOtherSide
function BasicActor:ActorChangeSeat(seatIndex, isAnimationEnabled)
	self.AI.theVehicle.vehicle:ChangeSeat( self.id, seatIndex, isAnimationEnabled);
end
function BasicActor:OnWeaponStartFire(weaponId)
	--System.LogAlways("BasicActor:OnWeaponStartFire");
end
function BasicActor:OnWeaponStopFire(weaponId)
	--System.LogAlways("BasicActor:OnWeaponStopFire");
end
function BasicActor:RegaWithAI()
	AI.RegisterWithAI(self.id, AIOBJECT_PUPPET, self.Properties, self.PropertiesInstance, self.AIMovementAbility,self.melee);
end
function BasicActor:EquipPack()
	if (self.Properties) then		
		local equipmentPack=self.Properties.equip_EquipmentPack;
		if (equipmentPack and equipmentPack ~= "") then
			ItemSystem.GiveItemPack(self.id, equipmentPack, false, false);
		end
...
function BasicActor:RemoveActor()
	...
	--TheOtherSide
	local isConquest = g_gameRules.game:IsConquestGamemode();

	if (System.IsEditor() or isConquest) then
...
function BasicActor:StartBleeding()
	self:DestroyAttachment(0, "wound");
	self:CreateBoneAttachment(0, "Bip01 Spine", "wound");
	
	local effect=self.bloodFlowEffect;
	local pos = self:GetBonePos("Bip01 Spine", g_Vectors.temp_v1);
...
function BasicActor.Client:OnHit(hit)
	...
	if (self == g_localActor or (g_localActor.actor:IsHaveSlave() and self.id == g_localActor.actor:GetSlaveId())) then
...
function BasicActor:SetActorModel(isClient)
	...
	if (self.currItemModel ~= self.Properties.fpItemHandsModel) then
		...
		--System.LogAlways("[LUA][self.currItemModel: ]"..self.currItemModel.." != [self.Properties.fpItemHandsModel:"..self.Properties.fpItemHandsModel.."]");
...
function BasicActor:DropObject(throw,throwingVec,throwingDelay)

	local dropTable =
	{
		event = "dropObject",
		throwVec = {x=0,y=0,z=0},
		throwDelay = throwingDelay or 0,
	};
		
	if (throwingVec) then
		CopyVector(dropTable.throwVec,throwingVec);
	end
...
function BasicActor:RemoveRagdoll(frameTime)
	if (not g_localActor or g_gameRules.game:IsConquestGamemode()) then
		return false;
	end	
