...
function ForbiddenArea:PunishPlayer(player, time)
	...
	if (warning<=0) then
		...
		--System.LogAlways("Player have warning ");
		local slaveEntity = System.GetEntity(player.actor:GetSlaveId());

		if (slaveEntity) then
			g_gameRules:CreateHit(slaveEntity.id,slaveEntity.id,slaveEntity.id,self.dps*(time/1000),nil,nil,nil,"punish");
			--System.LogAlways("Hit created to "..slaveEntity:GetName().." with 100000 damage");
			--g_gameRules:CreateHit(slaveId,slaveId,slaveId,self.dps*(time/1000),nil,nil,nil,"punish");
		else
			
			--System.LogAlways("Player not have controlled ");
