...
--TheOtherSide
--------------------------------------------------------------------------
function VehicleBase:DriverSelectWeaponByIndex(index)
...
--------------------------------------------------------------------------
function VehicleBase:GetSeatWeaponCount(seatId)
	local seat = self.Seats[seatId];
	if (not seat) then
...
--------------------------------------------------------------------------
function VehicleBase:RequestGunnerSeat(userId)
	for i,seat in pairs(self.Seats) do
...
function VehicleBase.Server:OnHit(hit)
  ...
  local direction = hit.dir;
	...
	local dmgDone = self.vehicle:OnHit(targetId, hit.shooterId, hit.damage, hit.pos, hit.radius, hitType, explosion);
	--if(hit.type ~= "fire" and dmgDone > 0.0 and hit.shooterId~=self:GetDriverId() and not self.vehicle:IsDestroyed()) then
		--g_gameRules.game:SendHitIndicator(hit.shooterId, hit.explosion or false);
	--end

		-- send damage indicator to all players in this vehicle
	if (g_gameRules and hit.targetId) then
		if(not hit.shooterId == nil or (dmgDone > 0.0)) then
		  if (self.Seats) then
  			for i,seat in pairs(self.Seats) do
  				if(seat:GetPassengerId()) then
  					g_gameRules.game:SendDamageIndicator(seat:GetPassengerId(), hit.shooterId or NULL_ENTITY, hit.weaponId or NULL_ENTITY);
  				end	
  			end
			end
		end
