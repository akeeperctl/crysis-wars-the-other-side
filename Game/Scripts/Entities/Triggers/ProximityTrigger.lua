...
function ProximityTrigger:IsValidSource( entity )
	...
	if(Properties.bOnlyControlActor ~= 0 and not entity.actor:IsHaveOwner()) then
		return false;
	end

	if(Properties.bOnlyMyControlActor ~= 0 and not entity.actor:IsLocalOwner()) then
		return false;
	end
