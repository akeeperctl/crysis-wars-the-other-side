...
function Hazard:HandleEntity(ent)
	System.LogAlways("[LUA][Hazard][HandleEntity]");
	...
	if(ent~=nil and ent.actor~=nil)then
		if(self.Properties.Damage.bOnlyPlayer==1 and not ent.actor:IsPlayer())then
			System.LogAlways("[LUA][Hazard][HandleEntity][Not Player]");
			return;
		end;

		local dmg=self.Properties.Damage.fDamage;
		local slaveId = ent.actor:GetSlaveId();
		...
		if(dmg>0)then
			--TheOtherSide
			--System.LogAlways("[LUA][Hazard][HandleEntity][dmg>0]");
			if (slaveId~=0) then

				--System.LogAlways("[LUA][Hazard][HandleEntity][ent.actor:GetSlaveId()!=0]");
				local slaveEntity = System.GetEntity(slaveId);

				if(self.dmg=="frost")then
					if (slaveEntity.isAlien) then
						--System.LogAlways("[LUA][Hazard][HandleEntity][slaveEntity.isAlien][Frost]");
						return;
					end
					
				elseif(self.dmg=="electricity")then
					if (slaveEntity.isAlien) then
						--System.LogAlways("[LUA][Hazard][HandleEntity][slaveEntity.isAlien][electricity]");

						--regen alien health 
						local health = slaveEntity.actor:GetHealth();
						local addHealth = dmg;

						health = health + addHealth;

						slaveEntity.actor:SetHealth(health);
						slaveEntity:ResetDamageEffects();
					end
				elseif(self.dmg=="fire")then
					g_gameRules:CreateHit(slaveEntity.id,self.id,self.id,dmg,nil,nil,nil,"fire");
				end
			--~TheOtherSide
			else
...
function Hazard:AddScreenEffect(ent)
	if(ent.actor:IsPlayer() or ent.actor:IsHaveOwner())then
...
Hazard.Server.Deactivated=
{
	OnBeginState = function(self)
		for i,v in pairs(self.ents) do
			if(v.actor and (v.actor:IsPlayer() or v.actor:IsHaveOwner()))then
				self.onClient:RemoveScreenEffect(v.actor:GetChannel(), v.id);
			end;
		end;
	end,
