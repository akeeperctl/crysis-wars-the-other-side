...
--- режимы поиска укрытий
HM_NEAREST = 0                                    --Найти ближайшее укрытие.
HM_NEAREST_TO_ME = 16                             --Найти ближайшее укрытие ко мне.
HM_FARTHEST_FROM_TARGET = 1                       --Найти укрытие, максимально удаленное от цели.
HM_NEAREST_TO_TARGET = 2                          --Найти укрытие, максимально близкое к цели.
HM_NEAREST_BACKWARDS = 13                         --Найти ближайшее укрытие позади меня.
HM_NEAREST_TOWARDS_TARGET = 12                    --Найти ближайшее укрытие, находящееся в направлении цели.
HM_FARTHEST_FROM_GROUP = 3                        --Найти укрытие, максимально удаленное от группы.
HM_NEAREST_TO_GROUP = 4                           --Найти укрытие, максимально близкое к группе.
HM_LEFTMOST_FROM_TARGET = 5                       --Найти укрытие, максимально слева от цели.
HM_RIGHTMOST_FROM_TARGET = 6                      --Найти укрытие, максимально справа от цели.
HM_RANDOM = 7                                     --Найти случайное укрытие.
HM_FRONTLEFTMOST_FROM_TARGET = 8                  --Найти укрытие, максимально слева от цели и спереди.
HM_FRONTRIGHTMOST_FROM_TARGET = 9                 --Найти укрытие, максимально справа от цели и спереди.
HM_NEAREST_TO_FORMATION = 10                      --Найти укрытие, максимально близкое к формации.
HM_FARTHEST_FROM_FORMATION = 11                   --Найти укрытие, максимально удаленное от формации.
HM_NEAREST_TO_LASTOPRESULT = 512                  --Найти укрытие, максимально близкое к результату последнего действия.
HM_RANDOM_AROUND_LASTOPRESULT = 519               --Найти случайное укрытие вокруг результата последнего действия.
HM_USEREFPOINT = 30                               --Использовать точку “refpoint” как укрытие.
HM_ASKLEADER = 31                                 --Спросить у лидера о подходящем укрытии.
HM_ASKLEADER_NOSAME = 32                          --Спросить у лидера о подходящем укрытии, отличном от того, которое использовал лидер.
HM_BEHIND_VEHICLES = 29                           --Найти укрытие за транспортным средством.
HM_NEAREST_PREFER_SIDES = 14                      --Найти ближайшее укрытие, отдавая предпочтение боковым позициям.
HM_NEAREST_TOWARDS_TARGET_PREFER_SIDES = 17       --Найти ближайшее укрытие, находящееся в направлении цели, отдавая предпочтение боковым позициям.
HM_NEAREST_TOWARDS_TARGET_LEFT_PREFER_SIDES = 18  --Найти ближайшее укрытие, находящееся в направлении цели, отдавая предпочтение левым боковым позициям.
HM_NEAREST_TOWARDS_TARGET_RIGHT_PREFER_SIDES = 19 --Найти ближайшее укрытие, находящееся в направлении цели, отдавая предпочтение правым боковым позициям.
HM_NEAREST_TOWARDS_REFPOINT = 20                  --Найти ближайшее укрытие, находящееся в направлении точки “refpoint”.
HM_AROUND_LASTOP = 512                            --Включить в поиск укрытия около точки результата последней GoalOp
HM_INCLUDE_SOFTCOVERS = 64                        --Включить в поиск мягкие укрытия
...
TOS_AI = {
    ...
    ---@param targetId? userdata (Опционально)  Идентификатор цели подканала (она должна быть ИИ), которая при вызове функции сохраняется в `LastOpResult`
    ...
    ---Изменяет позицию опорной точки сущности.
    ---@param entityId    userdata идентификатор сущности, для которой нужно получить позицию опорной точки
    ---@param pos Vec3 позиция в 3Д пространстве
    SetRefPointPosition = function(entityId, pos)
        return AI.SetRefPointPosition(entityId, pos)
    end,
